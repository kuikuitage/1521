...
evt_xsm_t evt_osd_xsm[5];
u8 osd_cnt = 0, j_record = 0, i_record = 0;

extern BOOL manage_get_preview_rect(u8 root_id,
                             u16 *left,
                             u16 *top,
                             u16 *width,
                             u16 *height);

extern u8 fw_get_focus_id(void);
...
static void on_dvb_msg(handle_t handle, os_msg_t *p_msg)
{
  ...
  switch(p_msg->content)
  {
    ...
    }
    {
      ...
      if ((p_this->pmt_version != p_pmt->version) || (p_this->crc_32 != p_pmt->crc_32))
      {
        p_this->pmt_version = p_pmt->version;
        p_this->crc_32 = p_pmt->crc_32;
 ...
 if(SUCCESS == cas_events_get(p_this, &event))
{
    if(manage_get_preview_rect(fw_get_focus_id(),
                             ...
                             &width, &height)) //!full
    {
        OS_PRINTF("\n%s:LINE:%d,is not fullscreen!\n", __func__, __LINE__);
        is_fullscreen = FALSE;
    }
    else
    {
        OS_PRINTF("\n%s:LINE:%d,yes fullscreen!osd_cnt=[%d],i_record=[%d],j_record=[%d]\n", \
                __func__, __LINE__, osd_cnt, i_record, j_record);
        if(osd_cnt)
        {
            if(evt_osd_xsm[i_record].flag == 1)
            {
                evt_osd_xsm[i_record].evt_xsm.data2 = (u32)&evt_osd_xsm[i_record].msg_info;
                
                OS_PRINTF("\n%s:LINE:%d,\ndata1=[0x%x],\ndata2=[0x%x],addr=[0x%x],\ndata3=[0x%x]\n", \
                        __func__, __LINE__, \
                        evt_osd_xsm[i_record].evt_xsm.data1, \
                        evt_osd_xsm[i_record].evt_xsm.data2,\
                        addr,\
                        &evt_osd_xsm[i_record].evt_xsm);
                
                evt_osd_xsm[i_record].flag = 0;
                ap_frm_send_evt_to_ui(APP_CA, &evt_osd_xsm[i_record].evt_xsm);
                osd_cnt--;
    ...
    switch(cas_event)
    {
      ...
      case CAS_C_SHOW_OSD:
        ...
        {   
            OS_PRINTF("\n%s:LINE:%d,this is not a fullscreen!j_record=[%d]\n size=%d\n", __func__, __LINE__, j_record, sizeof(msg_info_t));
            memcpy(&(evt_osd_xsm[j_record].msg_info), (msg_info_t *)cas_param.p_param, sizeof(msg_info_t));

            evt_osd_xsm[j_record].evt_xsm.id = CAS_EVT_SHOW_OSD_MSG;
            evt_osd_xsm[j_record].evt_xsm.data1 = evt.data1;
            evt_osd_xsm[j_record].flag = 1;
            OS_PRINTF("\n%s:LINE:%d,\naddr=[0x%x],\ndata1=[0x%x],\ndata2=[0x%x]\n", \
                    __func__, __LINE__, addr, evt.data1, evt_osd_xsm[j_record].evt_xsm.data2);
	...
	*/
	...
	case CAS_C_CARD_NEED_UPDATE:
      ...
      case CAS_E_CARD_OVERDUE:
        ...
        OS_PRINTF("get mem addr 0x%x, LINE=%d\n", addr,__LINE__);
        cas_param.p_param = (void *)addr;
        remain_day = cas_io_ctrl(event.slot_id, CAS_IOCMD_CA_OVERDUE, 0);
        evt.id = CAS_EVT_OVERDUE;
        evt.data1 = (event.slot_id << 16) | cas_param.cam_id;
        evt.data2 = (u32)(&remain_day);
        break;
      default:
        evt.id = CAS_EVT_NOTIFY;
        evt.data1 = (event.slot_id << 16) | cas_param.cam_id;
