...
static cas_vender_str_t cas_vender[] =
{
  ...
  {CAS_ID_LX, {0x85,0x50,0x1,0x64,0x22,0x20}, 6}, /* LiXin*/
  ...
  {CAS_ID_SV, {0x26,0x90}, 2}, /* Sumavision */
  {CAS_ID_SV, {0x26,0xff}, 2}, /* Sumavision */
  {CAS_ID_SV, {0xee,0x72}, 2}, /* Sumavision */
  {CAS_ID_SV, {0x1f,0x72}, 2}, /* Sumavision */
  #endif
  ...
  {CAS_ID_XG, {0x86,0x48}, 2}, 
  {CAS_ID_XSM, {0x11}, 1}, /* XSM*/
  {CAS_ID_XSM, {0x20}, 1}, /* XSM*/
...
cas_dmx_chan_t  cas_si_chan[CAS_DMX_CHANNEL_COUNT];
cas_mail_headers_t mail_header;
u16 g_mail_node_id[MAIL_MAX_NODE_NUM] = {0};
u16 g_stb_serial[SERIAL_MAX_NODE_NUM] = {0};
...
static u32 _cam_identify(u8 *p_atr_hb, u32 size)
{
  ...
  for(i = 0; i < size; i++)
  {
   // if((p_atr_hb[i] >= 'a' && p_atr_hb[i] <= 'z')
   //   || (p_atr_hb[i] >= 'A' && p_atr_hb[i] <= 'Z'))
    {
  ...
  if (event_status & CAS_EVT_MAIL_MASK)
  {
    ...
    else
    {
      ...
      mail_header.max_num++;
        ...
        mail_header.total_email_num++;
        OS_PRINTF("use mail_header.total_email_num = %d\n",mail_header.total_email_num);
      #endif
...
static RET_CODE _hotplug_handle(void *p_param, u32 event_status)
{
  ...
  else
  {
    ...
    //g_cas_priv.cam_id[slot] = CAS_UNKNOWN;
    if(cam_id == CAS_UNKNOWN)
    {
...
static cas_dmx_chan_t *_search_free_channel(void)
{
  u32 i = 0;

  for (i = 0; i < CAS_DMX_CHANNEL_COUNT; i ++)
  {
    if (cas_si_chan[i].handle == DMX_INVALID_CHANNEL_ID)
    {
      cas_si_chan[i].chan_index = i;
      return cas_si_chan + i;
    }
  }
...
u8 cas_compare_mail(u8 *p_data, cas_mail_compare_type_t type)
{
  ...
  for(num = 0; num < (mail_header.total_email_num+1); num ++)
  {
    OS_PRINTF("mail_header.total_email_num = %d\n",mail_header.total_email_num);
    OS_PRINTF("mail_header.p_mail_head[%d].special_id = %d\n",num,mail_header.p_mail_head[num].special_id);
  }
...
void cas_del_mail_data(cas_module_id_t module_id, u32 mail_id)
{
  ...
  #if(CONFIG_CAS_ID == CONFIG_CAS_ID_GS)
    special_id_temp = mail_header.p_mail_head[0].special_id;
  #endif
  ...
  #if(CONFIG_CAS_ID == CONFIG_CAS_ID_GS)
    mail_header.p_mail_head[mail_header.total_email_num].special_id = special_id_temp;
  #endif
...
void cas_del_all_mail_data(cas_module_id_t module_id)
{
  ...
  #if(CONFIG_CAS_ID == CONFIG_CAS_ID_GS)
    u32 special_id_temp[MAIL_MAX_NODE_NUM];
    u16 total_email_num_temp = 0;
  #endif

  #if(CONFIG_CAS_ID == CONFIG_CAS_ID_GS)
  for (i = 0; i < (mail_header.total_email_num+1); i ++)
  #else
  for (i = 0; i < mail_header.max_num; i ++)
  #endif
  {
  ...
  #if(CONFIG_CAS_ID == CONFIG_CAS_ID_GS)
  total_email_num_temp = mail_header.total_email_num;
  for (i = 0; i < mail_header.total_email_num; i ++)
  {
    special_id_temp[i] = mail_header.p_mail_head[i].special_id;
  }
  #endif
  ...
  #if(CONFIG_CAS_ID == CONFIG_CAS_ID_GS)
  mail_header.total_email_num= total_email_num_temp;
  for (i = 0; i < (mail_header.total_email_num+1); i ++)
  {
    mail_header.p_mail_head[i].special_id = special_id_temp[i];
  }
  #endif
  ...
  for (i = 0; i < total_num; i++)
  {
    ...
    switch(type)
    {
      case MAIL_DATA_TYPE:
        ...
        #if(CONFIG_CAS_ID == CONFIG_CAS_ID_GS)
          mail_header.total_email_num++;
          OS_PRINTF("use1 mail_header.total_email_num = %d\n",mail_header.total_email_num);
        #endif
       ...
       u16 pid,u8 *p_match,u8 *p_mask, u8 filter_mode)
{
  ...
  if(dmx_ret != SUCCESS)
  {
	...
	p_chan->p_buf = p_buf = (u8 *)mtos_nc_malloc(CAS_DVB_SECTION_SIZE);
...
cas_dmx_chan_t *cas_ecm_request(void *p_dmx, u16 pid, u8 *value, u8 *mask, u8 filter_mode)
{
  cas_dmx_chan_t *p_chan = NULL;
  RET_CODE ret = ERR_FAILURE;

  p_chan = _search_free_channel();
  p_chan->tid = value[0];
  if(NULL == p_chan)
  {
  ...
  if(filter_mode & ECM_FILTER_CONTINUOUS_DISABLED)
      filter_mode = FILTER_CONTINOUS_DISABLED;
  else
      filter_mode = FILTER_CONTINOUS_ENABLED;
  ret = cas_table_request(p_dmx, p_chan, pid, value, mask, filter_mode);
  MT_ASSERT(ret == SUCCESS);

  return p_chan;
}

cas_dmx_chan_t *cas_emm_request(void *p_dmx, u16 pid, u8 *value, u8 *mask, u8 filter_mode)
{
  cas_dmx_chan_t *p_chan = NULL;
  RET_CODE ret = ERR_FAILURE;
