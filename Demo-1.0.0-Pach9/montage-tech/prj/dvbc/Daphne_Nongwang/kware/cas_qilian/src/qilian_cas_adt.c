...
//  BU16 ProductCount = 0;
  ...
  switch(cmd)
  {
    ...
    case CAS_IOCMD_CARD_INFO_GET:
	  ...
	  if(STBCA_isDaughterCard(&pdwCardID))
	  {
		p_card_info->card_type = 1;
		sprintf((char *)(p_card_info->mother_card_id),"%ld",(u32)pdwCardID);
    ...
    case CAS_IOCMD_ENTITLE_INFO_GET:
	  ...
	  if(STBCA_GetProductCount(&pProductCount) == TRUE)
	  {
		CAS_QILIAN_PRINTF("QI LIAN TEST %s, %d get product success!\n", __FUNCTION__, __LINE__);
	  }
	  CAS_QILIAN_PRINTF("QI LIAN TEST %s, %d pProductCount=%d\n", __FUNCTION__, __LINE__, pProductCount);
        ...
        for(i = 0;i < CaMenu_product.CaMenu_number; i++)
        {
          p_entitles_info->p_entitle_info[i].product_id
            = (u32)CaMenu_product.Camenu_Unit[i].Product_ID;
					...
					__by_MjdToDate(CaMenu_product.Camenu_Unit[i].Purchase_start_time
            ,date);
          year = 2000 + date[0];
          month = date[1];
          day = date[2];
          ...
          __by_MjdToDate(CaMenu_product.Camenu_Unit[i].Purchase_end_time
            ,date);
          year = 2000 + date[0];
          month = date[1];
          day = date[2];
    ...
    case CAS_IOCMD_MAIL_HEADER_GET:
      ...
      for(i = 0; i < email_count; i++)
      {
        ...
        month = date[1];
        day = date[2];
        p_mail_headers->p_mail_head[i].send_date[0] = (u8)(year / 100);
        p_mail_headers->p_mail_head[i].send_date[1] = (u8)(year % 100);
        p_mail_headers->p_mail_head[i].send_date[2] = month;
        p_mail_headers->p_mail_head[i].send_date[3] = day;
        p_mail_headers->p_mail_head[i].send_date[4] = email.SendTime[2];
        p_mail_headers->p_mail_head[i].send_date[5] = email.SendTime[3];
        p_mail_headers->p_mail_head[i].send_date[6] = email.SendTime[4];
      }
    ...
    case CAS_IOCMD_FINGER_MSG_GET:
     ...
     // OS_PRINTF("\nGET=%s\n", msgFinger.data);
      //OS_PRINTF(" CAS_RCV_IOCMD---> CAS_IOCMD_FINGER_GET\n");
      break;
