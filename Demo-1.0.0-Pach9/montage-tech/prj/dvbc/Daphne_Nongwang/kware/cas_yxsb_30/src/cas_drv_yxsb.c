                                     ...
                                     const SCDCAIppvBuyInfo* pIppvProgram  )
{
    ...
    YXSB_DRV_DEBUG("IPPV Program buy information: EcmPid = %x,Type(ppv & ppt:2?) = %d \n", wEcmPid, byMessageType);
    ...
    YXSB_DRV_DEBUG("price_num(price points) = %d\n", IppvBuyInfo.price_num);
                                    ...
                                    CDCA_U16  wCurtainCode)
{
    u32 event;
    u32 CurtainCode = 0;
    OS_PRINTF("CDSTBCA_ShowCurtainNotify, wEcmPID = %d, wCurtainCode = %d\n", wEcmPID, wCurtainCode);
...
void CDSTBCA_ShowFingerMessage(CDCA_U16 wEcmPID,CDCA_U32 dwCardID)
{
  ...
  OS_PRINTF("Finger Msg EcmPid = %d, dwCardId = %d \n",
        wEcmPID, dwCardID);
                                       ...
                                       CDCA_U8 byMark )
{
    ...
    OS_PRINTF("CDSTBCA_ShowProgressStrip, byProgress = %d, byMark = %d, ProgressStripPara = %d\n", 
                                        byProgress, byMark, ProgressStripPara);
    while(!(p_priv->card_update_inited))
    {
        mtos_task_sleep(100);
    }
    OS_PRINTF("CDSTBCA_ShowProgressStrip\n");
