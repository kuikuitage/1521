...
static RET_CODE cas_yxsb_io_ctrl(u32 cmd, void *param)
{
    ...
    switch(cmd)
    {
        case CAS_IOCMD_START_CAS:
            YXSB_ADT_DEBUG("[YXSB] CAS_IOCMD_START_CAS \n");
            p_priv->card_update_inited = 1;
            break;
...
static RET_CODE cas_yxsb_table_process(u32 t_id, u8 *p_buf, u32 *p_result)
{
    ...
    //YXSB_ADT_DEBUG("[YXSB]%s:LINE:%d, get in!t_id=%d\n", __FUNCTION__, __LINE__, t_id);

    p_priv = (s_cas_yxsb_priv_t *)g_cas_priv.cam_op[CAS_ID_TF].p_priv;

    if (CAS_TID_PMT == t_id)
    {
        //YXSB_ADT_DEBUG("[YXSB]pmt process\n");

        YXSB_ADT_ParsePMT(p_buf, p_priv);
        //YXSB_ADT_DEBUG("ecm_pid = 0x%x, audio_ecm_pid = 0x%x, video_ecm_pid = 0x%x, pro_id = %d, ver_num=%d, pmt_ver_mun=%d, s_id=%d\n",
                                //p_priv->cas_desc.ca_pid,
                                //p_priv->audio_desc.a_cas_desc.ca_pid,
                                //p_priv->vedio_desc.v_cas_desc.ca_pid,
                                //p_priv->program_id,
                                //p_priv->pmt_ver_mun,pmt_ver_mun,s_id);
