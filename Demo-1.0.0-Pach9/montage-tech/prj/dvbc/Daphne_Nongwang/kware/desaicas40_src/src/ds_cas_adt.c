    ...
    while (length > 0)
    {
        ...
        switch (descriptor_tag)
        {
            case 0x09:  /* DVB_DESC_CA */
            {
                ...
                {
                  CAS_DS_PRINTF("[DS] NOT DESAI CA_ID!!!! %#x\n", descriptor_tag);
                }
            }
                break;
            default:
      ...
      CAS_DS_PRINTF("[DS]%s:LINE:%d, get in, cmd =%d!\n", __FUNCTION__, __LINE__, cmd);
       ...
       switch(cmd)
       {
            ...
            case CAS_IOCMD_PIN_VERIFY:
             ...
             //only for test
             #if 0
             if(p_pin_modify->pin_spare_num == 0)
             {
          ...
          case CAS_IOCMD_ZONE_CHECK:
            DEBUG(DSCA4, INFO, "[DS] CAS_IOCMD_ZONE_CHECK \n");
			...
			DEBUG(DSCA4, INFO, "[DS], nim unlock:%d ! \n", p_nim_info->lock);
...
RET_CODE cas_ds_attach(cas_module_cfg_t *p_cfg, u32 *p_cam_id)
{
   s_cas_ds_priv_t *p_priv = NULL;
