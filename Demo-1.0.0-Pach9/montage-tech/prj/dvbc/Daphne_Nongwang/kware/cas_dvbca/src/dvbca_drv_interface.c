...
#define DVBCA_CAS_SECTION_TASK_PRIO        DRV_CAS_MODUL_TASK_PRIORITY_START
...
void dvbcas_stb_drv_careqlock(void)
{
	...
	if(ret == FALSE)
	  DEBUG(DVBCAS_PORTING,INFO,"[DVBCAS]SEM TAKE ERROR\n");
}
void dvbcas_stb_drv_carequnlock(void)
{
	...
	if(ret == FALSE)
		DEBUG(DVBCAS_PORTING,INFO,"[DVBCAS]SEM GIVE ERROR\n");
}
...
int DVBSTBCA_SCReset(void)
{
	...
	if(SUCCESS != ret)
	{
		DEBUG(DVBCAS_PORTING, INFO, "cas: reset card failed\n");
		return DVBCA_FAILED;
	}
						   ...
						   DVBCA_UINT16 usPid)
{
    ...
    DEBUG(DVBCAS_PORTING, INFO, "call in, usPid=0x%x, ucFilterNum = %d\n", 
		usPid,ucFilterNum);
	...
	for (i= 0; i < DVBCA_FILTER_MAX_NUM && i < ucFilterNum; i++)
    {
		...
		if (index < 0)
		{
			DEBUG(DVBCAS_PORTING,INFO,"request ecm filter failed!\n");
			return DVBCA_FAILED;
		}
	    ...
	    if (SUCCESS != ret)
	    {
	        DEBUG(DVBCAS_PORTING, ERR, "open chan failed, ret=0x%08x!!!!\n", ret);
	    ...
	    if (SUCCESS != ret)
	    {
	        DEBUG(DVBCAS_PORTING, ERR, "chan set buffer failed! ret=0x%08x!!!!\n", ret);
	        //dvbcas_stb_drv_carequnlock();
	        return DVBCA_FAILED;
	    }
...
#ifdef FILTER_DEBUG
		DEBUG(DVBCAS_PORTING,INFO,"value:\n");
		DUMP(DVBCAS_PORTING,INFO,DMX_SECTION_FILTER_SIZE,p_param.value,"%x");
		
		DEBUG(DVBCAS_PORTING,INFO,"mask:\n");
		DUMP(DVBCAS_PORTING,INFO,DMX_SECTION_FILTER_SIZE,p_param.mask,"%x");
#endif
	    ...
	    if (SUCCESS != ret)
	    {
	        DEBUG(DVBCAS_PORTING, ERR, "chan start failed! ret=0x%08x!!!!\n", ret);
	        //dvbcas_stb_drv_carequnlock();
	        return DVBCA_FAILED;
	    }
	    ...
	    DEBUG(DVBCAS_PORTING, INFO, "[DVBCA] ===ecm filter data:index=%d,ecm_filter[%d].pid=0x%x!\n",
	            index,index,priv->ecm_filter[index].pid);
		pucFilter++;
		priv->ecm_filter[index].valid = TRUE;
	    //dvbcas_stb_drv_carequnlock();
	}
	DEBUG(DVBCAS_PORTING, INFO, "DVBSTBCA_SetEcmFilter OK\n");
    return DVBCA_SUCESS;
}
						   ...
						   DVBCA_UINT16 usPid)
{
	...
	for (i= 0;i<DVBCA_FILTER_MAX_NUM && i<ucFilterNum;i++)
    {
		...
		if (index < 0)
		{
			DEBUG(DVBCAS_PORTING,INFO,"request emm filter failed!\n");
			return DVBCA_FAILED;
		}
	    ...
	    DEBUG(DVBCAS_PORTING, INFO, "[DVBCA] ===emm filter data:index=%d,dvbca_filter[%d].pid=0x%x!\n",
	            index,index,priv->emm_filter[index].pid);
		pucFilter++;
		priv->emm_filter[index].valid = TRUE;
		//dvbcas_stb_drv_carequnlock();
	}
	DEBUG(DVBCAS_PORTING, INFO, "DVBSTBCA_SetEmmFilter OK\n");
	return DVBCA_SUCESS;
}
					  ...
					  DVBCA_UINT8 ucKeyLen)
{
...
#ifdef FILTER_DEBUG
	DEBUG(DVBCAS_PORTING, INFO,"usEcmPid:%#x,pucKeyLen:%d\n",
		usEcmPid,ucKeyLen);
	DEBUG(DVBCAS_PORTING, INFO,
	...
	if (ret != SUCCESS)
	{
		DEBUG(DVBCAS_PORTING,ERR," get chan id by v_pid failed!!!!\n");
		return ;
	}
	...
	if (ret != SUCCESS)
	{
		DEBUG(DVBCAS_PORTING,ERR," get chan id by a_pid failed!!!!\n");
		return ;
	}
	...
	if ((a_channel != 0xffff) && (a_pid != 0x1fff))
	{
		...
		if (ret != SUCCESS)
		{
			DEBUG(DVBCAS_PORTING,ERR,"enable audio descrambler failed!!!!\n");
			//MT_ASSERT(0);
		}
	...
	{
		DEBUG(DVBCAS_PORTING,INFO,"No audio prpgram playing now!!!!\n");
	}
	...
	if (v_channel != 0xffff)
	{
		...
		if (ret != SUCCESS)
		{
			DEBUG(DVBCAS_PORTING,ERR,"enable video descrambler failed!!!!\n");
			//MT_ASSERT(0);
		}
	...
	{
		DEBUG(DVBCAS_PORTING,INFO,"No video prpgram playing now!!!!\n");
	}
					...
					OUT DVBCA_UINT8 *pucStatusWord)
{
	...
	if(pucCommand[0] == 0x74 && pucCommand[1] == 0xf5 && pucCommand[4] == 0x02 && usCommandLen == 5)
	{
		retry_times = 1;
	}
	...
	if(priv->reset_flag == 1)
	{
		...
		do
		{
			...
			if(ret == SUCCESS)
			{
				DEBUG(DVBCAS_PORTING, INFO, "T0 transaction success!\n");
				break;
			}
			retry_times--;
		}while (retry_times > 0);
	...
	if(ret == SUCCESS)
	{
		...
		{
		  DEBUG(DVBCAS_PORTING, INFO, "SendDataToSmartCard error! \n");
		  MT_ASSERT(0);
		  return DVBCA_FAILED;
		}
	...
	{
		DEBUG(DVBCAS_PORTING, ERR, "T0 transaction failed!\n");
		...
		if(card_update_flag == 1)
		{
			for (index = 0; index < 3; index ++)
			{
				...
				if (SUCCESS == ret)
				{
					DEBUG(DVBCAS_PORTING, INFO, "card reset ok: %d!\n", index);
		 			break;
				}
				else
				{
					DEBUG(DVBCAS_PORTING, INFO, "card reset failed!\n");
					return DVBCA_FAILED;
				}
			}
			card_update_flag = 0;
	...
	if(pucCommand[0] == 0x74 && pucCommand[1] == 0xf0 && pucCommand[4] == 0x01 && usCommandLen == 5)
	{
		mtos_task_delay_ms(200);
		DEBUG(DVBCAS_PORTING, INFO, "card upgrade start!\n");
	}
	if(pucCommand[0] == 0x74 && pucCommand[1] == 0xf5 && pucCommand[4] == 0x02 && usCommandLen == 5)
	{
		mtos_task_delay_ms(1000);
		DEBUG(DVBCAS_PORTING, INFO, "card upgrade end reset!\n");
		card_update_flag = 1;
	}
						...
						DVBCA_UINT32 uiLen)
{
	...
	DEBUG(DVBCAS_PORTING, INFO, "call in!\n addr:%#x,read len:%d\n",uiAddr,uiLen);
	DEBUG(DVBCAS_PORTING, INFO, "start address :%#x",priv->task_priv.flash_start_add);
	...
	{
		DEBUG(DVBCAS_PORTING, ERR, "Read flash out of range!\n");
		return;
	}
	...
	if (0 != length)
	{
		...
		if(SUCCESS != ret)
		{
			DEBUG(DVBCAS_PORTING, ERR, "Read data from nvram failed!\n");
		}
	}
	{
		int i;
...
{	u32 length = uiLen;
	...
	if ((start_address + length) > 
		(priv->task_priv.flash_start_add + priv->task_priv.flash_size))
	{
		DEBUG(DVBCAS_PORTING, ERR, "Write flash out of range!\n");
		return;
	}
	...
	if (0 != length)
	{
		...
		if(SUCCESS != ret)
		{
			DEBUG(DVBCAS_PORTING, ERR, "Write data to nvram failed!\n");
		}
	}
	{
		int i;
						...
						DVBCA_UINT32 uiLen)
{
	...
	if(ret != SUCCESS)
	{
		DEBUG(DVBCAS_PORTING, ERR, "format buffer error!\n");
	}
...
{
	cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	DEBUG(DVBCAS_PORTING,INFO,"call in usEcmPid = %x,ucDay = %d\n",usEcmPid,ucDay);
	if(priv->cur_ecm_pid == usEcmPid)
	{
		if(ucDay < 10)
		{
			cas_send_event(priv->slot, CAS_ID_DVBCA, CAS_C_PPC_LESS, 0);
		}
		mtos_task_delay_ms(5000);
						 ...
						 DVBCA_UINT8 ucMessageType)
{
	...
	cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	MT_ASSERT(priv != NULL);
	DEBUG(DVBCAS_PORTING,INFO,"-------------------usECMPID = %d--------ucMessageType = %d---------------------\n",usEcmPid,ucMessageType);
	DEBUG(DVBCAS_PORTING,INFO,"priv->cur_ecm_pid = %d>>>>>>\n",priv->cur_ecm_pid);
	DEBUG(DVBCAS_PORTING,INFO,"EcmPid:%#x msg[%s]\n",usEcmPid,_dvbcas_e2s_msg(ucMessageType/*usEcmPid*/));
	if(priv->cur_ecm_pid == usEcmPid)
	{
			...
			case DVBCA_MESSAGE_WORKTIME:/*不在工作时段内*/
				...
				break;
			default:
				DEBUG(DVBCAS_PORTING,INFO," ucMessageType :%s !\n", _dvbcas_e2s_msg(ucMessageType));
				MT_ASSERT(0);
	          	break;
			}
	}
						 ...
						 DVBCA_UINT8 ucMessageType)
{
	u32 event = 0;
	cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	MT_ASSERT(priv != NULL);	DEBUG(DVBCAS_PORTING,INFO,"-------------------usEcmPid = %d-------ucMessageType = %d---------------------------------!\n",usEcmPid,ucMessageType);
	if(priv->cur_ecm_pid == usEcmPid)
	{
		switch(ucMessageType)
		{
...
{   
	...
	cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	DEBUG(DVBCAS_PORTING,INFO," SHOW FINGER MESSAGE :call in usEcmPid = %x,ucDay = %x\n",
				usEcmPid,uiCardID);
	if(priv->cur_ecm_pid == usEcmPid)
	{
		dvbcas_stb_drv_careqlock();
...
void DVBSTBCA_HideFingerMessage(void)
{
	u32 event = 0;
	cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	DEBUG(DVBCAS_PORTING,INFO,"HIDE FINGER MASSAGE\n");
	event = CAS_C_HIDE_NEW_FINGER;
	cas_send_event(priv->slot, CAS_ID_DVBCA, event, 0);
}void DVBSTBCA_NewEmailNotify(DVBCA_UINT16 usEmailID)
{
	cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	DEBUG(DVBCAS_PORTING,INFO,"NEW email notify");
	if(usEmailID != 0)
	{
...
{
	cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	DEBUG(DVBCAS_PORTING,INFO,"DVBSTBCA_HideUrgent");
...
{
	cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	DEBUG(DVBCAS_PORTING,INFO,"DVBSTBCA_ShowUrgent");
	dvbcas_stb_drv_careqlock();
...
void DVBSTBCA_ShowOSDMessage(IN ST_DVBCA_MSG *pstAdvertize)
{
	...
	DEBUG(DVBCAS_PORTING,INFO,">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>show osd message\n");
	DEBUG(DVBCAS_PORTING,INFO,"msg1: %s\n",pstAdvertize->aucText);
	int ret;
	ret = DVBCASTB_GetCardID((DVBCA_UINT8 *)old_card_id);
	if(ret != 0)
	{
		DEBUG(DVBCAS_PORTING,INFO,"get old card id failed!!!\n");
	}
	else
	{
		DEBUG(DVBCAS_PORTING,INFO,"new card id = %s\n",old_card_id);
	}
	dvbcas_stb_drv_careqlock();
	...
	DEBUG(DVBCAS_PORTING,INFO,"pstAdvertize->ucDispPos = %d\n",pstAdvertize->ucDispPos);
	DEBUG(DVBCAS_PORTING,INFO,"pstAdvertize->ucDispCount = %d\n",pstAdvertize->ucDispCount);
	DEBUG(DVBCAS_PORTING,INFO,"pstAdvertize->ucDispWay = %d\n",pstAdvertize->ucDispWay);
	DEBUG(DVBCAS_PORTING,INFO,"pstAdvertize->ucDispDirection = %d\n",pstAdvertize->ucDispDirection);
	DEBUG(DVBCAS_PORTING,INFO,"pstAdvertize->ucIntervalTime = %d\n",pstAdvertize->ucIntervalTime);
...
{
	cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	DEBUG(DVBCAS_PORTING,INFO,"HIDE OSD MESSAGE\n");
...
void DVBSTBCA_ResumeOSDMessage(void)
{
	...
	if(ret != 0)
	{
		DEBUG(DVBCAS_PORTING,INFO,"get new card id failed!!!\n");
	}
	else
	{
		DEBUG(DVBCAS_PORTING,INFO,"new card id = %s\n",new_card_id);
	...
	{
		//cas_adt_dvbca_priv_t *priv = g_cas_priv.cam_op[CAS_ID_DVBCA].p_priv;
	    DEBUG(DVBCAS_PORTING,INFO,"&&&&&&&&&&&&&&&&&DVBSTBCA_ResumeOSDMessage\n");
		//cas_send_event(priv->slot, CAS_ID_DVBCA, CAS_C_DVB_SHOW_OSD, 0);
	}	
}
						 ...
						 DVBCA_UINT16 usStackSize)
{
	...
	{
		DEBUG(DVBCAS_PORTING, INFO, "too many task or invalid task priority!\n");
		return DVBCA_FAILED;
	}
	...
	if (!ret)
	{
		DEBUG(DVBCAS_PORTING, INFO, "[DVBCA]create task error = 0x%08x!\n", ret);
		priv->task_record[uIndex].is_busy = 0;
		return DVBCA_FAILED;
	}

	DEBUG(DVBCAS_PORTING, INFO,"[DVBCA]create task success! Name[%s], Priority[%d]\n", priv->task_record[uIndex].task_prio);
	return DVBCA_SUCESS;
}/*DVBSTBCA_RegisterTask*/

void DVBSTBCA_Sleep(DVBCA_UINT16 usMilliSeconds)
{
	//DEBUG(DVBCAS_PORTING,INFO, "[DVBCAS]DVBSTBCA_Sleep:%dms\n", usMilliSeconds);
						   ...
						   DVBCA_UINT32 uiInitValue)
{
	DVBCA_UINT32 i;
	os_sem_t p_sem;
	DEBUG(DVBCAS_PORTING,INFO, "call in! uiInitValue = %d\n", uiInitValue);

	if(TRUE != mtos_sem_create(&p_sem, 0))
	{
		DEBUG(DVBCAS_PORTING,INFO, "init semphore failed!\n");
		MT_ASSERT(0);
		return;
	}
	for (i=0;i<uiInitValue;i++)
	{
		...
		{
			DEBUG(DVBCAS_PORTING,INFO, "Signal semphore failed!\n");
			MT_ASSERT(0);
			return;
		}
	}
...
void DVBSTBCA_SemaphoreWait(INOUT DVBCA_SEMAPHORE *pSemaphore)
{
	...
	{
		DEBUG(DVBCAS_PORTING,INFO, "waite semphore failed!\n");
	}
...
void DVBSTBCA_SemaphoreDestory(INOUT DVBCA_SEMAPHORE *pSemaphore)
{
	...
	{
		DEBUG(DVBCAS_PORTING,INFO, "[DVBCAS]CAS destory semphore failed!\n");
	}
...
{
	void *p_buf = NULL;
	DEBUG(DVBCAS_PORTING,INFO,"call in size = %d\n",uiBufSize);
	p_buf = mtos_malloc(uiBufSize);
	MT_ASSERT(NULL != p_buf);
	DEBUG(DVBCAS_PORTING,INFO,"finish!\n");	
	return p_buf;
}/*DVBSTBCA_Malloc*/
...
void dvbca_stb_drv_ca_data_monitor(void *pData)
{
	...
	while(1)
	{
		//DEBUG(DVBCAS_PORTING,INFO,"--------------------delay_diver = %d----------------\n",delay_diver);
		...
		for(i=0;i<DVBCA_FILTER_MAX_NUM;i++)
		{
	        ...
	        if (priv->ecm_filter[i].valid)
	        {
				...
				{
				#ifdef FILTER_DEBUG
					DEBUG(DVBCAS_PORTING,INFO,"priv->ecm_filter[%d].pid=%x,uDataLen:%d\n", 
						i, priv->ecm_filter[i].pid,uDataLen);
					DUMP(DVBCAS_PORTING,INFO,16,p_GotData,"%x");
				#endif
				...
				{
					//DEBUG(DVBCAS_PORTING, ERR, "get ecm_filter[%d] data failed!\n", i);
				}
	        }
			//dvbcas_stb_drv_carequnlock();
		}
		for(i=0;i<DVBCA_FILTER_MAX_NUM;i++)
		{
	        ...
	        if (priv->emm_filter[i].valid)
	        {
				...
				{
				#ifdef FILTER_DEBUG
					DEBUG(DVBCAS_PORTING,INFO,"priv->emm_filter[%d].pid=%x,uDataLen:%d\n", 
				...
				{
					//DEBUG(DVBCAS_PORTING, ERR, "get emm_filter[%d] data failed!\n", i);
				}
	        }
			//dvbcas_stb_drv_carequnlock();
		}
	...
	DEBUG(DVBCAS_PORTING,INFO,"dvbca_stb_drv_ca_data_monitor>>>>>>>>>>>>>>>>>>>>>>>>>>>>!\n");
}
...
RET_CODE dvbca_stb_drv_adt_init(void)
{
	...
	DEBUG(DVBAD, INFO, "task_prio_monitor:%d\n", priv->monitor_priv.task_prio_monitor);
	...
	if (nRet == FALSE)
	{
		DEBUG(DVBCAS_PORTING,INFO,"Create data monitor task error = 0x%08x!\n", nRet);
		return ERR_FAILURE;
	}

	DEBUG(DVBCAS_PORTING,INFO,"****dvbca_stb_drv_adt_Init ok!\n");
	return SUCCESS;
}
