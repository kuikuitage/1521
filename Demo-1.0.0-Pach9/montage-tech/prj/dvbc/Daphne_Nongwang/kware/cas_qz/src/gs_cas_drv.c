...
#define GS_CAS_SECTION_TASK_PRIO        32
...
/*!
  abc
  */
unsigned short GS_crc16ccitt(char *pBuffer, unsigned long uBufSize)
{
...
/*!
  abc
  */
char GS_check_card_apdu(const char *stb_sn, char stb_sn_len,
                char *super_cas_id, char *smartcard_id, char *cw_mask)
{
  char in[128] = {0};
  char out[128] = {0};
  char cache[32] = {0};

  char check_reply_length = 19; /* 应答包长度 */
  ...
  if(r > 0)return GS_ERR_COMM;
  if(in[0] != GS_INS_CHECK_SMARTCARD_APDU)return GS_ERR_COMM;
  //process rest data
  if(byte_write - 5)
  {
  ...
  r = GS_serial_read((unsigned char *)in, 1, (unsigned int *)(&bytes_readed), 300);
  if(r > 0)return GS_ERR_COMM;
  if(in[0] != GS_INS_GET_STAGE_DATA)return GS_ERR_COMM;
  //process rest data
  if(byte_write - 5)
  {
    GS_serial_write((unsigned char *)(out + 5),
      byte_write - 5, (unsigned int *)(&bytes_written), 100);
    GS_serial_flush();
  }
...
/*!
  abc
  */
char GS_process_emm_apdu(const char *emm_data, char emm_len)
{
  char in[128] = {0};
  char out[128] = {0};
  static char emm_back[128] = {0};

  const char emm_reply = 2;
  unsigned int bytes_readed = 0;
  unsigned int bytes_written = 0;
  char r = 0;
  char byte_write = 0;
  ...
  byte_write = GS_apdu_package_v3((unsigned char *)out,
      GS_CLA, GS_INS_PROCESS_EMM, 0, 0,
  ...
  r = GS_serial_read((unsigned char *)in, 1, (unsigned int *)(&bytes_readed), 300);
  if(r > 0)return GS_ERR_COMM;
  if(in[0] != GS_INS_PROCESS_EMM)return GS_ERR_COMM;
  //process rest data
  if(byte_write - 5)
  {
    GS_serial_write((unsigned char *)(out + 5), byte_write - 5,
  ...
  if(bytes_readed != emm_reply)return GS_ERR_COMM;
  if((in[bytes_readed - 2] == 0x90) && (in[bytes_readed - 1] == 0x00))
  {
...
char GS_process_ecm_apdu(const char *ecm_data, char ecm_len,
          char *cw_mask, char *cw,unsigned short *err_code)
{
  char in[128] = {0};
  char out[128] = {0};
  static char ecm_back[128] = {0};

  const char ecm_reply = 20;
  unsigned int bytes_readed = 0;
  unsigned int bytes_written = 0;
  unsigned int crc = 0;
  char r = 0;
  ...
  if(ecm_len > 128)
  {
    OS_PRINTF("[err] ecm_len > 128\n");
    return GS_ERR_DATA_LENGTH;
  }
    
  CAS_GS_PRINTF("\nget new ecm crc_old_ecm=%d\n",crc_old_ecm);
  if(GS_memncmp((u8 *)ecm_back, (u8 *)ecm_data, (u8)ecm_len) == 0
    && crc_old_ecm != 0)
    return GS_ERR_OLD_DATA;//已经处理过的旧数据
  CAS_GS_PRINTF("\nget new ecm~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
  byte_write = GS_apdu_package_v3((unsigned char *)out,
    GS_CLA, GS_INS_PROCESS_ECM_APDU, 0, 0,
  ...
  if(r > 0)
  {
    OS_PRINTF("[err] 1111111\n");
    return GS_ERR_COMM;
  }
  if(in[0] != GS_INS_PROCESS_ECM_APDU)return GS_ERR_COMM;
  //process rest data
  if(byte_write - 5)
  {
    GS_serial_write((unsigned char *)(out + 5),
  ...
  r = GS_serial_read((unsigned char *)in, 2,
    (unsigned int *)(&bytes_readed), 600);
  if(r > 0)
  {
    OS_PRINTF("[err] 222222\n");
    return GS_ERR_COMM;  
  }
  ...
  if((in[bytes_readed - 2] != 0x90) || (in[bytes_readed - 1] != 0x00))
  {
    OS_PRINTF("[err] 333333 code= 0x%02x %02x\n", in[bytes_readed - 2], in[bytes_readed - 1]);
    return GS_ERR_ECM;
  }
...
/*!
  abc
  */
char GS_get_expire_date(char *expire_date)
{
  char in[128] = {0};
  char out[128] = {0};
  const char reply = 6;
  ...
  GS_serial_flush();
  //read ack
  r = GS_serial_read((unsigned char *)in, 1,
  (unsigned int *)(&bytes_readed), 300);
  if(r > 0)return GS_ERR_COMM;
  if(in[0] != GS_INS_GET_EXPIRE_DATE)return GS_ERR_COMM;
  r = GS_serial_read((unsigned char *)in, reply,
    (unsigned int *)(&bytes_readed), 600);
  if(r > 0)return GS_ERR_COMM;
...
/*!
  abc
  */
char GS_get_area_code(char *area_code)
{
  char in[128] = {0};
  char out[128] = {0};
  const char reply = 4;
  ...
  unsigned int bytes_written = 0;
  bytes_written = GS_apdu_package_v2((unsigned char *)out,
             GS_CLA, GS_INS_GET_AREA_CODE, 0, 0, (unsigned char)reply);
  GS_serial_write((unsigned char *)out, 5,
    (unsigned int *)(&bytes_written), 100);
  GS_serial_flush();
  //read ack
  r = GS_serial_read((unsigned char *)in, 1,
    (unsigned int *)(&bytes_readed), 300);
  if(r > 0)return GS_ERR_COMM;
  if(in[0] != GS_INS_GET_AREA_CODE)return GS_ERR_COMM;
  r = GS_serial_read((unsigned char *)in, reply,
    (unsigned int *)(&bytes_readed), 600);
  if(r > 0)return GS_ERR_COMM;
  if(bytes_readed != reply)return GS_ERR_COMM;
...
//读取母卡的对子卡授权信息
char GS_get_child_auth_info(char *auth_info, char* len)
{
	char in[128];
	char out[128];
	int i = 0;
    ...
    GS_serial_flush();
	//read ack
    ret = GS_serial_read((unsigned char *)in, reply, (unsigned int *)(&bytes_readed), 300);
	DEBUG(QZCA,INFO,"ret=%d bytes_readed = %d\n",ret,bytes_readed);
	for(i=0;i<bytes_readed;i++)
		OS_PRINTF("%02x  ",in[i]);
	OS_PRINTF("\n");
	...
	if(ret > 0)
		return GS_ERR_COMM;
    //if(in[0] != GS_INS_GET_CHILD_AUTH_INFO)
	//	return GS_ERR_COMM;
	
	ret = GS_serial_read((unsigned char *)in, reply, (unsigned int *)(&bytes_readed), 600);
	DEBUG(QZCA,INFO,"ret = %d print the in data,bytes_readed = %d (hex):\n",ret,bytes_readed);
	for(i=0;i<bytes_readed;i++) {
		OS_PRINTF("%02x  ",in[i]);
	}
	OS_PRINTF("\n");
...
char GS_set_child_auth_info(char *auth_info, char len)
{
	char in[128];
	char out[128];
	char reply = 2;
	...
	if(ret > 0)
		return GS_ERR_COMM;
    if(in[0] != GS_INS_SET_CHILD_AUTH_INFO)
		return GS_ERR_COMM;
	//process rest data
...
char read_child_expire_date(char *expire_date)
{
	char in[128];
	char out[128];
	const char reply = 6;
	...
	if(ret > 0)
		return GS_ERR_COMM;
    if(in[0] != GS_INS_READ_CHILD_AUTH_INFO)
		return GS_ERR_COMM;
	ret = GS_serial_read((unsigned char *)in, reply, (unsigned int *)(&bytes_readed), 600);
    if(ret > 0)
		return GS_ERR_COMM;
    ...
    for(uIndex = 0; uIndex < uDataNum; uIndex ++)
    {
      if(1 == s_gs_tData[uIndex].m_byReqId) //ecm
      {
          ...
          if(crc_old_ecm != crc_b)
          {
            ...
            crc_a = GS_crc16ccitt((char *)(s_gs_tData[uIndex].p_Data + 3),
            ...
            if(crc_b == crc_a)
            {
              ...
              err = GS_process_ecm_apdu((const char *)(s_gs_tData[uIndex].p_Data + 3),
                  (s_gs_tData[uIndex].m_uLength - 3),  gs_cw_mask,gs_cw, &ca_error_code);
              if(err == GS_ERR_COMM || err == GS_ERR_ECM)
              {
                drv_smc_reset();
                err = GS_process_ecm_apdu((const char *)(s_gs_tData[uIndex].p_Data + 3),
                  (s_gs_tData[uIndex].m_uLength - 3),  gs_cw_mask,gs_cw, &ca_error_code);
  ...
  gs_mail.content_length);
        ...
        else
        {
          ...
          if(crc_old_emm != crc_b)
          {
            ...
            crc_a =
              GS_crc16ccitt((char *)(s_gs_tData[uIndex].p_Data + 3),
                s_gs_tData[uIndex].p_Data[2] - 2);
             //CAS_GS_PRINTF("emm crc 2 \n");
            ...
            if(crc_b == crc_a)
            {
              ...
              err = GS_process_emm_apdu((const char *)(s_gs_tData[uIndex].p_Data + 3),
