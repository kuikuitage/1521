...
extern char GS_check_card_apdu(const char *stb_sn, char stb_sn_len,
...
extern char GS_get_area_code(char *area_code);
extern char GS_get_expire_date(char *expire_date);
...
extern char GS_get_child_auth_info(char *auth_info, char* len);
extern char GS_set_child_auth_info(char *auth_info, char len);
extern char read_card_type(char *type);
char  gs_stb_sn[20] = {0};
char  gs_cw_mask[8] = {0};
...
static RET_CODE cas_gs_card_reset(u32 slot, card_reset_info_t *p_info)
{
  ...
  _Gs_CaReqLock();
  err = GS_get_area_code((char *)(&gs_card_area_code));
  _Gs_CaReqUnLock();
...
static RET_CODE cas_gs_io_ctrl(u32 cmd, void *param)
{
  ...
  switch(cmd)
  {
	...
	case CAS_IOCMD_MON_CHILD_STATUS_GET:
			...
			ret = read_card_type((char *)&(p_feed_info->is_child));
			if(p_feed_info->is_child != 0x5A)
			{
	        	ret = GS_get_child_auth_info((char *)p_feed_info->feed_data, (char *)&(p_feed_info->length));
			}
	...
	case CAS_IOCMD_MON_CHILD_STATUS_SET:
	        p_feed_info = (chlid_card_status_info *)param;
	        ret = GS_set_child_auth_info((char *)p_feed_info->feed_data, (char)p_feed_info->length);
			DEBUG(QZCA,INFO,"p_feed_info->is_child=%d,ret=%d\n",p_feed_info->is_child,ret);
	        if(GS_ERR_OK != ret)
	        {
    ...
    case CAS_IOCMD_MAIL_HEADER_GET:
      ...
      CAS_GS_ADT_PRINTF(" total_email_num= %d\n",
        p_cas_mail_headers->max_num);
      for(i = 0;i < p_cas_mail_headers->max_num; i ++)
      {
        CAS_GS_ADT_PRINTF(" subject = %s m_id=0x%x read=%d\n",
    ...
    case CAS_IOCMD_MAIL_DELETE_BY_INDEX:
      CAS_GS_ADT_PRINTF("MAIL_DELETE_BY_INDEX  =%d\n",*(u32 *)param);
      cas_del_mail_data(CAS_ID_GS, *(u32 *)param);
      break;

    case CAS_IOCMD_MAIL_DELETE_ALL:
      CAS_GS_ADT_PRINTF("MAIL_DELETE_ALL\n");
    ...
    case CAS_IOCMD_MAIL_BODY_GET:
      ...
      CAS_GS_ADT_PRINTF("MAIL_BODY_GET  =%s\n",p_cas_mail_body->data);
      break;
