...
#else
#define CAS_DS_PRINTF(...) do{} while(0)
#endif
...
static RET_CODE ds_get_product_info(product_entitles_info_t *p_entitles_info)
{
   ...
   p_pstProductEntitles = (ST_PRODUCT_ENTITLE  *)mtos_malloc(p_entitles_info->max_num * sizeof(ST_PRODUCT_ENTITLE));
   ...
   ret = DSCA_GetProductInfo(p_entitles_info->operator_id, p_entitles_info->start_pos,
    ...
    if(ret != CA_ERROR_OK)
    {
        //CAS_DS_PRINTF("[DS]get product info error : %d! \n", ret);
        return ERR_FAILURE;
    }
...
static RET_CODE ds_get_operator_info(cas_operators_info_t *p_operators_info)
{
  ...
  //get operator id
   ...
   if(ret != CA_ERROR_OK)
   {
       //CAS_DS_PRINTF("[DS]get opreator id error : %d! \n", ret);
          return ERR_FAILURE;
    }
  ...
  //get operator info
   ...
   if(ret != CA_ERROR_OK)
   {
       //CAS_DS_PRINTF("[DS]get opreator info error : %d! \n", ret);
           return ERR_FAILURE;
    }
    ...
    while (length > 0)
    {
        ...
        switch (descriptor_tag)
        {
            case 0x09:  /* DVB_DESC_CA */
            {
                ...
                if(TRUE == DSCA_IsDSCa(ca_desc.ca_system_id))
                {
                    if(ds_ecm_pid == ca_desc.ca_pid)
                    {
                        return;
                    }
                    ds_ecm_pid = ca_desc.ca_pid;
    ...
    if(NULL != p_data)
    {
         ...
         if(section_length > 0)
         {
            ...
            //  GUOTONG_SetEcmPID(CA_INFO_CLEAR, NULL);
              ds_pmt_parse_descriptor(p_data, program_info_length,program_number);
    ...
    while (length > 0)
    {
        ...
        switch (descriptor_tag)
        {
            case 0x09:  /* DVB_DESC_CA */
            {
                ...
                {
                    //CAS_DS_PRINTF("[DS]%s:LINE:%d, [DS EMM] emm pid =0x%x,Desc.ca_system_id =0x%x\n",
                    //__FUNCTION__, __LINE__, emm_pid, ca_system_id);
                    DSCA_SetEmmPid((UINT16)emm_pid);
                }
                else
                {
                  //CAS_DS_PRINTF("[DS] NOT DESAI CA_ID!!!! \n");
                }
            }
                break;
            default:
...
static void ds_cas_parse_cat(u8 *p_buf)
{
    ...
    //CAS_DS_PRINTF("[DS]%s:LINE:%d, [CAT] length=%d,version_number=%d\n",
    //        __FUNCTION__, __LINE__, length, version_number);

    p_data += 8;
    ds_cat_parse_descriptor(p_data, length - 9);

    //CAS_DS_PRINTF("[DS]\r\n %s:LINE:%d, parsing cat over.\n", __FUNCTION__, __LINE__);
}

static void ds_cas_parse_nit(u8 *p_buf)
{
    u8 p_data[7] = {0};
    u8 descriptor_tag = 0;
    u8 descriptor_length = 0;
    ...
    //CAS_DS_PRINTF("[DS]%s:LINE:%d, get in!\n", __FUNCTION__, __LINE__);
    descriptor_tag = p_buf[10] ;
    descriptor_length = p_buf[11] ;

    if(descriptor_tag == 0xa5 && descriptor_length == 0x07)
    {
      memcpy(p_data, &p_buf[12], descriptor_length);
      DSCA_SetCardAddressCode(p_data);
      //CAS_DS_PRINTF("[DS]\r\n %s:LINE:%d, parsing nit over.\n", __FUNCTION__, __LINE__);
    }
    else
    {
      //CAS_DS_PRINTF("[DS]\r\n %s:LINE:%d, parsing nit error.\n", __FUNCTION__, __LINE__);
    }
...
static RET_CODE cas_ds_init()
{
    ...
    {
        task_prio = p_cas_ds_priv->task_prio;
    }
    else
    {
...
static RET_CODE cas_ds_identify(UINT16 ca_sys_id)
{
     ...
     if(ret != TRUE)
     {
             //CAS_DS_PRINTF("[DS]Not DeSai CA card ! \n");
             return ERR_FAILURE;
     }
...
static RET_CODE cas_ds_io_ctrl(u32 cmd, void *param)
  {
      ...
      EN_ERR_CAS ret = 0;
      u8 index = 0;
	  //OS_PRINTF("1--###################ENTER ds_cas_adt_v5.o.c###################\n");
      ...
      //CAS_DS_PRINTF("[DS]%s:LINE:%d, get in, cmd =%d!\n", __FUNCTION__, __LINE__, cmd);
       ...
       switch(cmd)
       {
           ...
           case CAS_IOCMD_SET_ECMEMM:
              ...
              for(index = 0; index < CAS_MAX_ECMEMM_NUM; index++)
              {
                ...
                else
                {
                  if(p_cas_table_info->ecm_info[index].ecm_id == 0x1FFF)
                  {
                    ds_ecm_pid=0x1FFF;
                    ds_service_id = p_cas_table_info->service_id;
                    //CAS_DS_PRINTF("[DS_SET_ECM]p_cas_table_info->ecm_info[index].ecm_id = 0x%x \n",p_cas_table_info->ecm_info[index].ecm_id);
                    DSCA_SetEcmPid(0x1FFF);
                  }
                }
              }
            ...
            case CAS_IOCMD_CARD_INFO_GET:
			  ...
			  //OS_PRINTF("############mother_card_id=%s############\n",p_card_info->mother_card_id); 
			 // p_card_info->mother_card_id = "12344";  //lxd
              ...
              if(ret != CA_ERROR_OK)
              {
                  //CAS_DS_PRINTF("[DS]get card id error : %d! \n", ret);
                  return ERR_FAILURE;
              }
              ...
              if(ret != CA_ERROR_OK)
              {
                  //CAS_DS_PRINTF("[DS]get card version error : %d! \n", ret);
                  return ERR_FAILURE;
              }
             ...
             if(ret != CA_ERROR_OK)
              {
                // p_card_info->area_len = 32;
                  //CAS_DS_PRINTF("[DS]get card region error : %d! \n", ret);
                  return ERR_FAILURE;
              }
             ...
             ret = DSCA_GetWorkTime(&pstWorkTime);
              ...
              {
                  //CAS_DS_PRINTF("[DS]get card WORKTIME error : %d! \n", ret);
                  return ERR_FAILURE;
              }
       ...
       //       if(ret != CA_ERROR_OK)
       //       {
       //           CAS_DS_PRINTF("[DS]get mother card id error : %d! \n", ret);
       //           return ERR_FAILURE;
       //       }
              //CAS_DS_PRINTF("[DS]get mother card id  : %s! \n", p_card_info->mother_card_id);
	   	     //OS_PRINTF("--cas_adt_v5.c----mother_card_id=%s--------\n",p_card_info->mother_card_id);
          // get local id
       ...
       //       }
            ...
            case CAS_IOCMD_RATING_SET:
              ...
              {
                if(ret != CA_CARD_SUCCESS)
                  CAS_DS_PRINTF("[DS]CAS_IOCMD_RATING_SET WRONG: %d \n", ret);
              }
            ...
            case CAS_IOCMD_CUR_PRG_NUM_GET:
              ...
              p_entitles_info->max_num = max_num_p;
            ...
            case CAS_IOCMD_PIN_VERIFY:
              ...
              {
                if(ret != CA_CARD_SUCCESS)
                  CAS_DS_PRINTF("[DS]CAS_IOCMD_PIN_VERIFY FAIL = %d", ret);
              }
              //CAS_DS_PRINTF("[DS]old_pin : %x-%x-%x ! \n", p_pin_modify->old_pin[0], p_pin_modify->old_pin[1], p_pin_modify->old_pin[2]);
            ...
            case CAS_IOCMD_BURSE_INFO_GET:
              ...
              for(index = 1; index < 5; index ++)
              {
                ...
                if(ret != CA_ERROR_OK)
                {
                  //CAS_DS_PRINTF("[DS]burse get wrong\n");
                    return ERR_FAILURE;
                }
            ...
            case CAS_IOCMD_MAIL_HEADER_GET:
              ...
              if(ret != CA_ERROR_OK)
              {
                //CAS_DS_PRINTF("[DS]get email number error : %d! \n", ret);
                   return ERR_FAILURE;
              }
              ...
              if(ret != CA_ERROR_OK)
              {
                //CAS_DS_PRINTF("[DS]get email list error : %d! \n", ret);
                    return ERR_FAILURE;
              }
           ...
           case CAS_IOCMD_MAIL_BODY_GET:
             ...
             if(ret != CA_ERROR_OK)
             {
               //CAS_DS_PRINTF("[DS]get email content error : %d! \n", ret);
                   return ERR_FAILURE;
             }
             //CAS_DS_PRINTF("[DS]get email id: %d! \n", p_mail_body->mail_id);
            ...
            case CAS_IOCMD_LANGUAGE_SET:
              ...
              {
                //CAS_DS_PRINTF("[DS]not support the language id: %d \n",*p_sys_language);
                ret = ERR_FAILURE;
              }
              break;
          ...
          case CAS_IOCMD_OSD_GET:
           ...
           case CAS_IOCMD_OSD_MSG_SHOW_END:
            ...
            case CAS_IOCMD_PURCHASE_SET:
              ...
              p_ippv_info = (ipps_info_t *)param;
              ret = DSCA_IppvBuy(1, p_ippv_info->p_ipp_info[0].operator_id);
              switch(ret)
              {
...
static RET_CODE cas_ds_table_process(u32 t_id, u8 *p_buf, u32 *p_result)
{
    ...
    if (CAS_TID_PMT == t_id)
    {
        //OS_PRINTF("[DS]desai: pmt process\n");
    ...
    {
        //OS_PRINTF("[DS]desai: cat process\n");
    ...
    {
        //OS_PRINTF("[DS]desai: nit process\n");
        ...
        ds_cas_parse_nit(g_cas_priv.nit_data);
    }
...
static RET_CODE cas_ds_card_reset(u32 slot, card_reset_info_t *p_info)
{
    ...
    if (NULL == p_info)
    {
        //CAS_DS_PRINTF("[DS]%s:LINE:%d, p_info is NULL!\n", __FUNCTION__, __LINE__);
        return ERR_FAILURE;
    }
...
RET_CODE cas_ds_attach_v5(cas_module_cfg_t *p_cfg, u32 *p_cam_id)
{
  ...
  p_priv = mtos_malloc(sizeof(s_cas_ds_priv_t));
