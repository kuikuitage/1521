...
MG_STATUS MG_STBPort_Send_Command(MG_U8 *MG_SendComm, MG_U8 MG_CommLen, MG_U8 *MG_RecvResp,MG_U16 *SW, MG_U8 * NumberRead)
{
  ...
  while(1)
  {
    ...
    status[0] = MG_RecvResp[opt_desc.bytes_in_actual - 2];
    status[1] = MG_RecvResp[opt_desc.bytes_in_actual - 1];
    mtos_task_unlock();
#endif
    CAS_MGEOS_PRINTF("STBPort: sw0[0x%x], sw1[0x%x]\n", status[0], status[1]);
    CAS_MGEOS_PRINTF("MG_SendComm[%d]:", MG_CommLen);
...
#else
      temp = (u32)MG_RecvResp + opt_desc.bytes_in_actual - 2;
#endif
...
#else
  *NumberRead = opt_desc.bytes_in_actual - 2;
#endif
  if(MG_ret == MG_FAILING)
  {
    *NumberRead  = 0;
    *SW = 0;
  }
  ...
  *SW = ((MG_U16)status[0] << 8) |status[1];  
  ...
  CAS_MGEOS_PRINTF("MG_RecvResp[%d]:", *NumberRead);
...
MG_STATUS MG_STBPort_Create_Semaphore(MG_VOID)
{
  BOOL ret;
...
MG_STATUS MG_STBPort_Obtain_Semaphore(MG_VOID)
{
  BOOL ret;
...
MG_STATUS MG_STBPort_Release_Semaphore(MG_VOID)
{
  BOOL ret;
...
MG_STATUS MG_STBPort_Delete_Semaphore(MG_VOID)
{
  BOOL ret;
