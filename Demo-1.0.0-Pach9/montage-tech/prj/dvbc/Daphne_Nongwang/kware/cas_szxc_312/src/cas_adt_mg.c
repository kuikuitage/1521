...
/**
  The message format
*/
#define MAX_OSD_MSG_BAK   5
typedef struct
{
	cas_osd_t cas_osd[MAX_OSD_MSG_BAK];		                                    /* OSD struct Down LOOP */
...
static u32 parse_event(MG_STATUS result)
{
  ...
  switch(result)
  {
    ...
    case MG_EMM_CHILD_UNLOCK:
      /* */
      OS_PRINTF(" EMM_SENT_EVENT---> MG_EMM_CHILD_UNLOCK\n");
      break;//return CAS_C_EMM_CHILD_UNLOCK;
    ...
    case MG_EMM_LOADER:
      /* */
      OS_PRINTF(" EMM_SENT_EVENT---> MG_EMM_LOADER\n");
      break;//return CAS_S_CARD_NEED_UPG;

    case MG_EMM_FIGEIDENT:
      /* show the card serial number */
      CAS_ADT_PRINTF(" EMM_SENT_EVENT---> MG_EMM_URGENT_SERVICE\n");
      return CAS_C_SHOW_NEW_FINGER;

    case MG_EMM_URGENT_SERVICE:
      /* show the urgent service */
#if 0
      MG_Get_ServiceID(MG_U8 * MG_ServiceID);
#endif
    ...
    case MG_EMM_UPDATE_COUT:
      ...
      CAS_ADT_PRINTF(" EMM_SENT_EVENT---> CAS_C_EMM_UPDATE_COUT\n");
    ...
    case MG_EMM_OSD:
      ...
      {
				int i;
				for (i=0;i<MAX_OSD_MSG_BAK;i++)
				{
					if (cas_osd_msg.cas_osd[i].has_read)
					{
						break;
					}
				}
				if (i == MAX_OSD_MSG_BAK)
				  return CAS_E_UNKNOWN_ERROR;
				
        cas_osd_msg.cas_osd[i].disp_mode = p_msg_data[0];
        cas_osd_msg.cas_osd[i].disp_cnt = p_msg_data[1];
        memcpy(&cas_osd_msg.cas_osd[i].cas_osd_data, ( p_msg_data+3), CAS_OSD_MSG_DATA_MAX_LEN);
        cas_osd_msg.cas_osd[i].has_read = FALSE;
        CAS_ADT_PRINTF(" RCV_MG_EMM_OSD[%d]---> disp_mode: %d disp_cnt: %d data: %s\n",
					i,p_msg_data[0], p_msg_data[1], p_msg_data+3);
    ...
    case MG_ERR_NOT_RECEIVED_SPID:
      /* */
      OS_PRINTF(" ECM_SENT_EVENT---> CAS_E_NOT_RECEIVED_SPID\n");
    ...
    case MG_ERR_GET_CARD_DATA:
      /* */
      OS_PRINTF(" ECM_SENT_EVENT---> MG_ERR_GET_CARD_DATA\n");
      break;//return CAS_E_CARD_DATA_GET;
    ...
    case MG_ERR_ILLEGAL_CARD:
      /* show illegal card */
      OS_PRINTF(" ECM_SENT_EVENT---> MG_ERR_ILLEGAL_CARD\n");
      break;//return CAS_E_ILLEGAL_CARD;
...
static RET_CODE cas_adt_mg_init()
{
  ...
  cas_mail.cas_mail_headers.has_read = 0xFF;
	...
	for (i=0;i<MAX_OSD_MSG_BAK;i++)
	{
		cas_osd_msg.cas_osd[i].has_read = TRUE;
	}
...
//  u32 system_time_tick =0x00;
  ...
  //OS_PRINTF("get card config, F[%d], Clk[%d]\n", smc_cfg.clk_f_rate, smc_cfg.clk_ref);
  ...
  {
      //system_time_tick = mtos_ticks_get();
      //CAS_ADT_PRINTF("set ECMFilter after SMC reset OK system_time: 0x%8x\n", system_time_tick);
      cas_adt_mg_ecm_resetFilterAll();
      cas_adt_mg_ecm_setFilterAll();
  }
#if 1
...
RET_CODE cas_adt_mg_ecm_process(u8 *p_ecm_buf, ecm_info_t *p_info, u32 *p_result)
{
	...
	//only for test PARTNER_ERROR
	//#define PARTNER_ERROR
	#ifdef PARTNER_ERROR
		event = parse_event(MG_ERR_PIN_AUTHEN);
		cas_send_event(p_priv->slot, CAS_ID_ADT_MG, event, 0);
		return SUCCESS;
	#endif
	//end test
...
static RET_CODE cas_adt_mg_io_ctrl(u32 cmd, void *param)
{
  ...
  switch(cmd)
  {
    case CAS_IOCMD_CARD_INFO_GET:
        ...
        CAS_ADT_PRINTF(" CAS_RCV_IOCMD---> CAS_IOCMD_CARD_INFO_GET\n");
        break;
    ...
    case CAS_IOCMD_IPPV_INFO_GET:
      ...
      if(1 == MG_Get_Program_Type(MG_Get_Current_Program()))
      {
          p_ipp_info->p_ipp_info[0].price = MG_Get_Program_Price(MG_Get_Current_Program());
          ...
          p_ipp_info->p_ipp_info[0].authed = MG_Get_Program_Auth(MG_Get_Current_Program());
    ...
    case  CAS_IOCMD_IPP_BUY_INFO_GET:
      ...
      p_purch_info->ipp_unit_time = MG_Get_Program_Unit(MG_Get_Current_Program());
    ...
    case CAS_IOCMD_PIN_ERR_CNT_GET:
      ...
      if(0x80 == (ret & 0x80))
      {
        ...
        {
          p_pin_err->err_code = CAS_PIN_ERR_UNDEF;
        }
      }
      else
      {
        p_pin_err->err_cnt = ret & 0x0F;
     ...
     case CAS_IOCMD_MON_CHILD_STATUS_GET:
      ...
      MG_Get_MSCard_Info(card_info);
      if((card_info[14] & 0x1) == 0) /* mon card */
      {
      ...
      else /* son card */
      {
          ...
          memcpy(p_feed_info->parent_card_sn, card_info, 4);
          memcpy(p_feed_info->child_card_authed_time, &card_info[4], 2);
          //memcpy(p_feed_info->parent_card_stb_sn, &card_info[6], 6);
          memcpy(p_feed_info->child_card_remain_time, &card_info[12], 2);
      }
      CAS_ADT_PRINTF(" CAS_RCV_IOCMD---> CAS_IOCMD_MON_CHILD_STATUS_GET\n");
      break;

#if 1 /* deal with V3.1.2 adapter layer process flash R/W */
    ...
    *(u8 *)param = p_priv->mail_cfg.max_mail_num;
    ...
    case CAS_IOCMD_MAIL_BODY_GET:
      ...
      CAS_ADT_PRINTF(" CAS_RCV_IOCMD---> CAS_IOCMD_MAIL_BODY_GET\n");
      break;
    ...
    case CAS_IOCMD_OSD_GET:
	      ...
	      if(i < MAX_OSD_MSG_BAK)
	      {
					if (cas_osd_msg.cas_osd[i].disp_mode == OSD_DISP_MODE_UPLOOP)
         	  p_msg_info->osd_display.location = 0;
					else
						p_msg_info->osd_display.location = 1;
					
          p_msg_info->osd_display.roll_value = cas_osd_msg.cas_osd[i].disp_cnt;
          p_msg_info->osd_display.roll_mode = 1;
          p_msg_info->data_len = CAS_OSD_MSG_DATA_MAX_LEN;
          memcpy(p_msg_info->data, cas_osd_msg.cas_osd[i].cas_osd_data, CAS_OSD_MSG_DATA_MAX_LEN);
					cas_osd_msg.cas_osd[i].has_read = TRUE;
	        CAS_ADT_PRINTF(" CAS_IOCMD_OSD_GET---> type: %d disp_mode: %d disp_cnt: %d data: %s\n", 
...
RET_CODE cas_adt_mg_attach(cas_module_cfg_t *p_cfg, u32 *p_cam_id)
{
  ...
  p_priv->pre_v_pid = INVALID_CHANNEL_ID;
  p_priv->pre_a_pid = INVALID_CHANNEL_ID;
