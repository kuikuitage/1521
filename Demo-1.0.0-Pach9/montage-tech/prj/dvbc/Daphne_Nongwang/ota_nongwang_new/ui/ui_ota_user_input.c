...
s32 open_ota_user_input(u32 para1, u32 para2)
{
  ...
  sys_static_read_ota_tp_info(&ota_tp_info);
  
  OS_PRINTF("in function %s\n", __FUNCTION__);
  ...
  ctrl_set_rstyle(p_cont, RSI_OTA_USER_INPUT_MENU, 
    RSI_OTA_USER_INPUT_MENU, RSI_OTA_USER_INPUT_MENU);
  ...
  for(i = 0; i < OTA_USER_INPUT_ITEM_CNT; i++)
  {
    ...
    {       
      ...
      case IDC_OTA_USER_INPUT_DPID_T:
        p_item[i] = ctrl_create_ctrl((u8*)CTRL_TEXT, (u8)(IDC_OTA_USER_INPUT_TPFREQ_T + i),
          OTA_USER_INPUT_ITEMLX, y, 
          OTA_USER_INPUT_ITEMRW, OTA_USER_INPUT_ITEMH,
          p_cont, 0);
        ctrl_set_rstyle(p_item[i], RSI_OTA_USER_INPUT_STC_SH, 
          RSI_OTA_USER_INPUT_STC_HL, RSI_OTA_USER_INPUT_STC_SH);
        text_set_font_style(p_item[i], FSI_OTA_USER_INPUT_SH, 
          FSI_OTA_USER_INPUT_SH, FSI_OTA_USER_INPUT_SH);
      ...
      case IDC_OTA_USER_INPUT_DPID:
        p_item[i] = ctrl_create_ctrl((u8*)CTRL_NBOX, (u8)(IDC_OTA_USER_INPUT_TPFREQ_T + i),
        ...
        ctrl_set_rstyle(p_item[i], RSI_OTA_USER_INPUT_STC_SH, 
          RSI_OTA_USER_INPUT_STC_HL, RSI_OTA_USER_INPUT_STC_SH);
        nbox_set_font_style(p_item[i], FSI_OTA_USER_INPUT_SH, 
          FSI_OTA_USER_INPUT_HL, FSI_OTA_USER_INPUT_SH);
        ...
        switch(i+1)
        {
          case IDC_OTA_USER_INPUT_TPFREQ:
            ...
            nbox_set_num_by_dec(p_item[i], ota_tp_info.upg_tp.tp_freq/1000);
            break;
          case IDC_OTA_USER_INPUT_SYM:
            ...
            nbox_set_num_by_dec(p_item[i], ota_tp_info.upg_tp.tp_sym);
            break;
          case IDC_OTA_USER_INPUT_DPID:
            nbox_set_range(p_item[i], OTA_DPID_MIN, OTA_DPID_MAX, OTA_DPID_BIT);
            nbox_set_focus(p_item[i], (OTA_DPID_BIT - 1));
            nbox_set_postfix_type(p_item[i], NBOX_ITEM_PREFIX_TYPE_STRID);
            nbox_set_num_by_dec(p_item[i], ota_tp_info.upg_tp.pid);
            break;
          default:
            MT_ASSERT(0);
            break;
      ...
      case IDC_OTA_USER_INPUT_HELP:
        ...
        ctrl_set_rstyle(p_item[i], RSI_OTA_USER_INPUT_STC_SH, 
          RSI_OTA_USER_INPUT_STC_HL, RSI_OTA_USER_INPUT_STC_SH);
        text_set_font_style(p_item[i], FSI_OTA_USER_INPUT_HL, 
          FSI_OTA_USER_INPUT_HL, FSI_OTA_USER_INPUT_HL);
        text_set_align_type(p_item[i], STL_LEFT | STL_VCENTER);
        text_set_content_type(p_item[i], TEXT_STRTYPE_STRID);
  ...
  ctrl_set_related_id(p_item[1],  0, 7, 0, 4);//tp freq
  ctrl_set_related_id(p_item[3],  0,  2, 0, 6);//sym
  ctrl_set_related_id(p_item[5],  0,  4, 0, 7);//dpid
  ctrl_set_related_id(p_item[6],  0,  6, 0, 2);//start


  p_title = ctrl_create_ctrl((u8*)CTRL_TEXT, IDC_OTA_USER_INPUT_TTL,
    OTA_USER_INPUT_TTLX, OTA_USER_INPUT_TTLY, 
    OTA_USER_INPUT_TTLW, OTA_USER_INPUT_TTLH,
    p_cont, 0);
  ctrl_set_rstyle(p_title, RSI_OTA_USER_INPUT_MENU, 
    RSI_OTA_USER_INPUT_MENU, RSI_OTA_USER_INPUT_MENU);

  text_set_font_style(p_title, FSI_OTA_USER_INPUT_SH,
    FSI_OTA_USER_INPUT_SH, FSI_OTA_USER_INPUT_SH);
	...
	control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  struct control *p_tpfreq, *p_symb;
