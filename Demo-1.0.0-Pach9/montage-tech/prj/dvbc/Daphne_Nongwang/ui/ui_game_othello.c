  ...
  u32 para1, u32 para2)
{
  ...
  {
    ret = manage_close_menu(ROOT_ID_OTHELLO, 0, 0);
  }
...
static RET_CODE on_othello_level_exit_all(control_t *p_ctrl, u16 msg,
  u32 para1, u32 para2)
{
  ui_close_all_mennus();
  return SUCCESS;
...
static RET_CODE on_othello_level_exit_menu(control_t *p_ctrl, u16 msg,
  u32 para1, u32 para2)
{
  ui_close_all_mennus();
  return SUCCESS;
...
RET_CODE open_game_othello(u32 para1, u32 para2)
{
  ...
  // create container
  p_cont = ctrl_create_ctrl((u8 *)CTRL_CONT, GAME_CONT_ID,
  ...
  //Chess board
  p_chessboard = ctrl_create_ctrl((u8 *)CTRL_MBOX, GAME_BOARD_ID,
  ...
  mbox_set_item_rect(p_chessboard, OTHELLO_BOARD_IL, OTHELLO_BOARD_IT,
    OTHELLO_BOARD_IR, OTHELLO_BOARD_IB, 0, 0);
  ...
  p_info = ctrl_create_ctrl((u8 *)CTRL_CONT, GAME_INFO_ID, OTHELLO_INFO_X,
  ...
  for (i = 0; i < OTHELLO_INFO_ITEM_COUNT; i++)
  {
    switch (i)
    {
      ...
      case 6:
      case 8:
        p_ctrl[i] = ctrl_create_ctrl((u8 *)CTRL_TEXT, (u8)(i + 1),
      ...
      case 2:
        ...
        ctrl_set_rstyle(p_ctrl[i], RSI_ITEM_3_SH, RSI_SELECT_ITEM_3,
        ...
        break;
      case 5:
        p_ctrl[i] = ctrl_create_ctrl((u8 *)CTRL_CBOX, (u8)(i + 1),
                                     OTHELLO_INFO_ITEM_X, item_y,
                                     OTHELLO_INFO_ITEM_W,
                                     OTHELLO_INFO_ITEM_H, p_info,
                                     0);
...
#else
  p_mask = spt_load_menu(ROOT_ID_OTHELLO);
  MT_ASSERT(p_mask != NULL);

  p_cont = ctrl_get_child_by_id(p_mask, GAME_CONT_ID);
  MT_ASSERT(p_cont != NULL);

  p_chessboard = ctrl_get_child_by_id(p_cont, GAME_BOARD_ID);
  ctrl_set_keymap(p_chessboard, game_board_keymap);
  ctrl_set_proc(p_chessboard, othello_board_proc);
  mbox_set_count(p_chessboard, OTHELLO_TOT_NUM,
                 OTHELLO_COL_NUM, OTHELLO_ROW_NUM);
  ...
  p_info = ctrl_get_child_by_id(p_cont, GAME_INFO_ID);
  ctrl_set_keymap(p_info, game_info_keymap);

  for (i = 0; i < OTHELLO_INFO_ITEM_COUNT; i++)
  {
    p_ctrl[i] = ctrl_get_child_by_id(p_info, (u8)(i + 1));
    switch (i)
...
{
  u16 focus;
  focus = (col_num * OTHELLO_ROW_NUM) + row_num;
...
void othello_draw_cursor(control_t *p_mbox)
{
  ...
  new_focus = (cur_col * OTHELLO_ROW_NUM) + cur_row;
