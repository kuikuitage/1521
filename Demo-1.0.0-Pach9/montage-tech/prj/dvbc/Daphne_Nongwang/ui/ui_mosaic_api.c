...
#define MOSACI_RECT_OFFSET_X (-25)
#define MOSACI_RECT_OFFSET_Y (-6)

static play_param_t g_mosaic_pb_info;// = {SYS_DVBC, 0, 0};
...
static void print_cur_mosaic_info()
{
  ...
  OS_PRINTF("mosaic_prog name        =%s\n",g_mosaic_prog.name);
  OS_PRINTF("mosaic_prog s_id        =%d\n",g_mosaic_prog.s_id);
  OS_PRINTF("mosaic_prog video_pid   =%d\n",g_mosaic_prog.video_pid);
  OS_PRINTF("mosaic_prog service_type=%d\n",g_mosaic_prog.service_type);
  OS_PRINTF("mosaic_prog mosaic_flag =%d\n",g_mosaic_prog.mosaic_flag);
  OS_PRINTF("mosaic_prog h_e_cells   =%d\n",g_mosaic_prog.mosaic.h_e_cells);
  OS_PRINTF("mosaic_prog v_e_cells   =%d\n",g_mosaic_prog.mosaic.v_e_cells);
  OS_PRINTF("mosaic_prog lg_cell_cnt =%d\n",g_mosaic_prog.mosaic.lg_cell_cnt);
  OS_PRINTF("mosaic_prog pre_svc_id  =%d\n",g_mosaic_prog.mosaic.pre_svc_id);
  OS_PRINTF("mosaic_prog nxt_svc_id  =%d\n",g_mosaic_prog.mosaic.nxt_svc_id);

  OS_PRINTF("mosaic_prog g_logic_cell_id  =%d\n",g_logic_cell_id);
  OS_PRINTF("mosaic_prog g_cur_cell_prog_id  =%d\n",g_cur_cell_prog_id);
}

//use to adjust the whole mosaic frame 
void mosaic_rect_ratio(rect_t *p_rc, s16 screen_h)
{
  av_set_t mosaic_avset;
  sys_status_get_av_set(&mosaic_avset);
  
  if(mosaic_avset.tv_ratio == TV_RATIO_16_9)//16:9
  {
    p_rc->top +=4;
    p_rc->bottom +=2;
    if(p_rc->top  < screen_h/2 )
    {
      p_rc->top = screen_h/2 - (screen_h/2 - p_rc->top)*9/12;
    }
    else if(p_rc->top  > screen_h/2 )
    {
      p_rc->top = screen_h/2 + (p_rc->top - screen_h/2)*9/12;
    }

    //bottom
    if(p_rc->bottom < screen_h/2 )
    {
      p_rc->bottom = screen_h/2 - (screen_h/2 - p_rc->bottom)*9/12;
    }
    else if(p_rc->bottom  > screen_h/2 )
    {
      p_rc->bottom = screen_h/2 + (p_rc->bottom - screen_h/2)*9/13;
    }
  }
}
...
static void mosaic_switch_pg(u16 pg_id)
{
  ...
  OS_PRINTF("switch masaic audio pid %d  type %d\n",
...
BOOL ui_init_mosaic()
{
  ...
  memset(&g_mosaic_pb_info, 0, sizeof(play_param_t));
  g_mosaic_pb_info.lock_mode = SYS_DVBC;
...
BOOL ui_mosaic_change_logic_cell_id(u16 msg)
{
  ...
  {
    OS_PRINTF("ui_mosaic_change_logic_cell_id ERROR");
    return FALSE;
  }
  ...
  switch(msg)
  {
    ...
    case MSG_FOCUS_DOWN:
      ...
      OS_PRINTF("MSG_FOCUS_UPDOWNLEFTRIGHT\n");
      break;
...
{
  u8 elementary_cell_w = 0;
  u8 elementary_cell_h = 0;
  rect_t sMosaicRc = {0};
  ...
  sMosaicRc.left = g_mosaic_prog.mosaic.screen_left;
  sMosaicRc.right = g_mosaic_prog.mosaic.screen_right;
  sMosaicRc.top = g_mosaic_prog.mosaic.screen_top;
  sMosaicRc.bottom = g_mosaic_prog.mosaic.screen_bottom;
  mosaic_rect_ratio(&sMosaicRc, 576);

  elementary_cell_w = (sMosaicRc.right - sMosaicRc.left)/g_mosaic_prog.mosaic.h_e_cells;
  elementary_cell_h = (sMosaicRc.bottom - sMosaicRc.top)/g_mosaic_prog.mosaic.v_e_cells;
