...
//  utc_time_t time = {{0}};
  ...
  u8 droplist_page[TIME_SET_ITEM_CNT] = {2, 6, 2, 0, 0};
  u16 y;
#endif

#ifndef SPT_SUPPORT
  p_cont =
    ui_comm_root_create(ROOT_ID_TIME_SET, RSI_TIME_SET_FRM,
                          TIME_SET_CONT_X, TIME_SET_CONT_Y,
                          TIME_SET_CONT_W, TIME_SET_CONT_H,
                          IDS_LOCAL_TIME_SET, RSI_TITLE_BG);
  if (p_cont == NULL)
  {
    return ERR_FAILURE;
  }
  ...
  p_info = ctrl_create_ctrl((u8 *)CTRL_TEXT, IDC_INFO, 
    TIME_SET_ITEM_X, y, (TIME_SET_ITEM_LW + TIME_SET_ITEM_RW),
    4 * (TIME_SET_ITEM_H + TIME_SET_ITEM_V_GAP), p_cont, 0);
  ...
  ui_comm_select_set_focus(p_ctrl[0], (u16)time_set.gmt_usage);//usage
  ui_comm_select_set_focus(p_ctrl[1], (u16)time_set.gmt_offset);//offset
  ui_comm_select_set_focus(p_ctrl[2], (u16)time_set.summer_time);//summer
                                     ...
                                     u32 para2)
{
  ...
  p_cont = ctrl_get_parent(ctrl_get_parent(p_ctrl));	
	...
	switch (ctrl_get_ctrl_id(ctrl_get_parent(p_ctrl)))
	{
      ...
      {
        fill_gmt_info(ctrl_get_child_by_id(p_cont, IDC_INFO), " ", is_onfocus);
      }
  ...
  u16 msg, u32 para1, u32 para2)
{
  ...
  //local time had been changed, so check book node again.
  OS_PRINTF("on_time_set_timedit_unselect \n");
  book_delete_overdue_node();
