...
static comm_help_data_t feed_help_data = //help bar data
{
  ...
  {
	IM_SN_ADWIN_LB, //QQ   IM_ICON_BACK,
	IM_SN_ADWIN_LB,  //QQ   IM_ICON_EXIT,
  },
};
*/
...
RET_CODE open_conditional_accept_feed(u32 para1, u32 para2)
{
  ...
  p_cont = ui_comm_root_create(ROOT_ID_CONDITIONAL_ACCEPT_FEED,
                               ...
                               IDS_FEED, RSI_ORANGE); //QQ   RSI_TITLE_BG
  if (p_cont == NULL)
  {
    return ERR_FAILURE;
  }
  ...
  //QQ  ctrl_set_rstyle(p_frm2, RSI_TIP_BOX, RSI_TIP_BOX, RSI_TIP_BOX);
 ...
 //QQ  ctrl_set_rstyle(p_ctrl, RSI_DLG_BTN_SH, RSI_DLG_BTN_HL, RSI_DLG_BTN_SH);
 ctrl_set_rstyle(p_ctrl, RSI_ORANGE, RSI_ORANGE, RSI_ORANGE);
  //text_set_font_style(p_ctrl, FSI_GREEN, FSI_GREEN, FSI_GREEN);
  text_set_align_type(p_ctrl, STL_CENTER | STL_VCENTER);
  text_set_content_type(p_ctrl, TEXT_STRTYPE_STRID);
 ...
 ctrl_set_rstyle(p_ctrl, RSI_ORANGE, RSI_ORANGE, RSI_ORANGE);
  ...
  //QQ  ui_comm_help_create(&feed_help_data, p_cont);
...
static RET_CODE on_conditional_accept_cont_ca_msg(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  ...
  {
 //QQ    text_set_content_by_strid(p_ctrl_tip,IDS_READ_MOTHER_CARD_FAILURE);
 text_set_content_by_strid(p_ctrl_tip,IDS_BACK);
    p_child_info = NULL;
  }
  else if(ret == CAS_EVT_NONE && g_feed_status == FEED_STATUS_READY)
  {
    p_child_info = (chlid_card_status_info *)para2;
  //QQ   text_set_content_by_strid(p_ctrl_tip,IDS_CA_INSERT_SON_CARD);
    text_set_content_by_strid(p_ctrl_tip,IDS_CA_ADDRES_INFO);
    g_feed_status = FEED_STATUS_MOTHER_SUCCESS;
  }
  else if(ret != SUCCESS && g_feed_status == FEED_STATUS_SON_SUCCESS)
  {
   //QQ  text_set_content_by_strid(p_ctrl_tip,IDS_FEED_FAILURE);
   text_set_content_by_strid(p_ctrl_tip,IDS_BACK);
  }
  else if(ret == SUCCESS && g_feed_status == FEED_STATUS_SON_SUCCESS)
  {
