  ...
  switch(e_show_mode)
  {
      ...
      case EMAIL_PIC_FLASH:
        if(!is_pic_flash)
        {
          ...
          ret = fw_tmr_create(ROOT_ID_EMAIL_PIC, MSG_PIC_FLASH, 2000, TRUE);
          is_pic_flash = TRUE;
        }
        break;
      default:
...
static RET_CODE on_email_pic_flash(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  else if(1 == us_flash_count)
  {
     ...
     bmap_set_content_by_id(p_bmp_ctrl, IM_SN_SCREEN_EMAIL);//QQ  IM_ICON_MAIL
  }
