...
#define SN_CONDITIONAL_ACCEPT_LEVEL_CHANGE_OK_H 	   25
	...
	for (i = 0; i<SN_CONDITIONAL_ACCEPT_LEVEL_LEVEL_CONTROL_ITEM_CNT; i++)
	{
	  switch(i)
	  {
		...
		case 1:
		  ...
		  ui_comm_select_set_content(p_ctrl, 9, IDS_CA_LEVEL10);
		  ui_comm_select_set_content(p_ctrl, 10, IDS_CA_LEVEL11);
		  ui_comm_select_set_content(p_ctrl, 11, IDS_CA_LEVEL12);
		  ui_comm_select_set_content(p_ctrl, 12, IDS_CA_LEVEL13);
		  ui_comm_select_set_content(p_ctrl, 13, IDS_CA_LEVEL14);
		  ui_comm_select_set_content(p_ctrl, 14, IDS_CA_LEVEL15);
		  ui_comm_select_set_content(p_ctrl, 15, IDS_CA_LEVEL16);
	      ui_comm_select_set_content(p_ctrl, 16, IDS_CA_LEVEL17);
		  ui_comm_select_set_content(p_ctrl, 17, IDS_CA_LEVEL18);
...
#if 0
  p_ctrl = ctrl_create_ctrl((u8 *)CTRL_TEXT, (u8)IDC_CA_PIN_WARN,
                              CONDITIONAL_ACCEPT_WARN_X, 
                              CONDITIONAL_ACCEPT_WARN_Y,
                              CONDITIONAL_ACCEPT_WARN_W, 
                              CONDITIONAL_ACCEPT_WARN_H,
                              p_ca_frm, 0);
  ctrl_set_rstyle(p_ctrl, RSI_SUB_BUTTON_SH, RSI_SUB_BUTTON_SH, RSI_SUB_BUTTON_SH);
  text_set_font_style(p_ctrl, FSI_COMM_TXT_N, FSI_COMM_TXT_HL, FSI_COMM_TXT_GRAY);
  text_set_align_type(p_ctrl, STL_CENTER | STL_VCENTER);
  text_set_content_type(p_ctrl, TEXT_STRTYPE_STRID);
  text_set_content_by_strid(p_ctrl, IDS_CA_CARD_LOCK);
#endif
  ...
  // QQ ui_comm_help_create(&ca_level_help_data, p_cont);
...
static RET_CODE on_conditional_accept_level_set(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  ...
  OS_PRINTF("level MODIFY\n");
  
  OS_PRINTF("ca_pin_info  new_pin :\n");
  //for(i = 0;i < 8;i++)
  //{
    //OS_PRINTF("%d",ca_pin_info->pin[i]);
  //}
  //OS_PRINTF("\n");
  //OS_PRINTF("ca_pin_info  pin_len : %d\n ",ca_pin_info->pin_len);
  //OS_PRINTF("ca_pin_info  pin_spare_num : %d\n ",ca_pin_info->pin_spare_num);
  //OS_PRINTF("ca_pin_info  rate : %d\n ",ca_pin_info->rate);
  OS_PRINTF("RETURN result : %d\n",para1);
  if(para1 == 0)
  {
    //text_set_content_by_strid(p_result, IDS_CA_RATING_CHANGED);//IDS_CA_RATING_CHANGED);
    
	level_data_t.text_strID = IDS_CA_RATING_CHANGED;
  ...
  else if(para1 == CAS_E_PARAM_ERR)
  {
	...
	level_data_t.text_strID = IDS_CA_PARAME_SET_ERROR;
	ui_comm_prompt_open(&level_data_t);
  }
  else if(para1 == CAS_E_PIN_INVALID)
  {
    //text_set_content_by_strid(p_result, IDS_MAINMENU);//IDS_CA_PIN_INVALID);
    
	level_data_t.text_strID = IDS_CA_PIN_INVALID;
	ui_comm_prompt_open(&level_data_t);
  }
  else if(para1 == CAS_E_WATCHRATING_INVALID)
  {
   // text_set_content_by_strid(p_result, IDS_CAS_E_IC_CMD_FAIL);
	
	level_data_t.text_strID = IDS_LEVEL_TOO_LOW;//IDS_CAS_E_IC_CMD_FAIL;
	ui_comm_prompt_open(&level_data_t);
  }
  else if(para1 == 9)
  {
  //  text_set_content_by_strid(p_result, IDS_CA_PASSWORD_PIN_NUMB);
	
	level_data_t.text_strID = IDS_CA_PASSWORD_PIN_NUMB;
	ui_comm_prompt_open(&level_data_t);
  }
  ...
  //  text_set_content_by_strid(p_result,IDS_CARD_LOCKED);
	...
	level_data_t.text_strID = IDS_CARD_LOCKED;
	ui_comm_prompt_open(&level_data_t);
  }
  else
  {
   // text_set_content_by_strid(p_result, IDS_INPUT_PASSWORD_ERROR);
...
static RET_CODE on_exit_conditional_accept_level(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  manage_close_menu(ROOT_ID_CONDITIONAL_ACCEPT_LEVEL, 0, 0);
  return SUCCESS;
}
...
BEGIN_MSGPROC(conditional_accept_level_cont_proc, ui_comm_root_proc)
  ON_COMMAND(MSG_CA_RATING_GET, on_conditional_accept_level_update)
  ON_COMMAND(MSG_CA_RATING_SET, on_conditional_accept_level_set)
  ON_COMMAND(MSG_EXIT, on_exit_conditional_accept_level)
