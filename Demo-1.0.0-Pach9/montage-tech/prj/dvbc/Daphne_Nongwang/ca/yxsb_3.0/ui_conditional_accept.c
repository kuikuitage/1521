	 ...
	 // control_t *p_mw = NULL;
	  ...
	  for(i = 0; i < YXSB_CONDITIONAL_ACCEPT_BTN_CNT; i++)
	  {
		  ...
		  else
		  {
			...
			y = SN_SUBMENU_BTN_Y + (((i - 1) - YXSB_CONDITIONAL_ACCEPT_BTN_CNT/2)*SN_SUBMENU_BTN_H);
		  }
...
static RET_CODE on_item_change_focus(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  switch(msg)
  {
    case MSG_FOCUS_RIGHT:
	  g_sub_curn_index = (g_sub_curn_index + YXSB_CONDITIONAL_ACCEPT_BTN_CNT/2 + 1) % YXSB_CONDITIONAL_ACCEPT_BTN_CNT;
...
static RET_CODE on_item_select(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  switch(ctrlID)
  {
    ...
    case IDC_BTN_CA_INFO2:
      manage_open_menu(ROOT_ID_CA_OPE, 0, 0);    
      break;
