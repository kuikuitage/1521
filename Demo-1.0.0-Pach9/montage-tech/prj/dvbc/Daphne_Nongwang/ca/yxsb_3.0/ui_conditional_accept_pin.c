...
#endif
 // full_screen_title_t title_data = {IM_SN_ICON_BRACKET_LEFT/*IM_COMMON_BANNER_SETTLE*/,IDS_CONDITIONAL_ACCEPT, RSI_IGNORE/*RSI_TITLE_BG*/, IDS_CARD_PASSWORD_SET};
/*
  p_cont = ui_comm_prev_root_create(ROOT_ID_CONDITIONAL_ACCEPT_PIN, RSI_IGNORE,//RSI_FULL_SCREEN_BG,
                          CONDITIONAL_ACCEPT_PIN_CONT_X, CONDITIONAL_ACCEPT_PIN_CONT_Y,
                          CONDITIONAL_ACCEPT_PIN_CONT_W, CONDITIONAL_ACCEPT_PIN_CONT_H,
                          &title_data);
*/
...
#endif
  //QQ ui_comm_help_create(&ca_pin_help_data, p_cont);
...
/*static RET_CODE on_conditional_accept_pin_ca_frm_exit(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  switch(msg)
  {
    case MSG_EXIT:
      manage_close_menu(ROOT_ID_CONDITIONAL_ACCEPT_PIN, para1, para2);
      break;
...
static RET_CODE on_conditional_accept_pin_ca_frm_ok(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
	u32 enable_psw_pinold, enable_psw_pinnew ,enable_psw_pinnew_confirm;
	 ...
	 DEBUG(DVBCAS_PORTING,INFO,"*old pin: %#x\n",pin_old_psw);
	 DEBUG(DVBCAS_PORTING,INFO,"*new pin: %#x\n",pin_new_psw);
	 DEBUG(DVBCAS_PORTING,INFO,"*cfm pin: %#x\n",pin_new_psw_confirm);
	 ...
	 if(DLG_RET_YES==ret)
	 {
		 ...
		 {
		   pin_data_t.text_strID = IDS_CA_PIN_INVALID;
		   ui_comm_prompt_open(&pin_data_t);
		 }
		 else if(pin_new_psw == pin_new_psw_confirm)
		 {
		 
		   DEBUG(DVBCAS_PORTING,INFO,"***********ui new pin: %#x\n",pin_new_psw);
...
static RET_CODE on_conditional_accept_pin_ca_modify(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  if(ret == SUCCESS)
  {
    //text_set_content_by_strid(p_result, IDS_CA_PIN_CHANGED);//IDS_CA_PIN_CHANGED);
  ...
  {
    //text_set_content_by_strid(p_result, IDS_CA_PIN_INVALID);//IDS_CA_PIN_INVALID);
...
BEGIN_MSGPROC(conditional_accept_pin_cont_proc, ui_comm_root_proc)
  //ON_COMMAND(MSG_FOCUS_UP, on_conditional_accept_pin_cont_focus_change)
  //ON_COMMAND(MSG_FOCUS_DOWN, on_conditional_accept_pin_cont_focus_change)  
