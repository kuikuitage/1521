...
rsc
  ...
  p_cont = ui_comm_root_create(ROOT_ID_CA_IPP,
                             ...
                             CA_IPP_X, CA_IPP_Y,
                             CA_IPP_W, CA_IPP_H,
                             RSC_INVALID_ID,RSC_INVALID_ID, RSI_TRANSPARENT);
  ...
  p_head = ctrl_create_ctrl((u8*)CTRL_CONT, (u8)(IDC_IPP_HEAD),
                            290, 20, 100, 35, p_cont, 0);  
  ctrl_set_rstyle(p_head, RSI_TIP_BOX, RSI_TIP_BOX, RSI_TIP_BOX);
  ...
  for(i=0; i <6; i++)
  {
    mbox_set_content_by_icon(p_mbox, i, IM_GAME_TETRIS_1, IM_GAME_TETRIS_5);
  }
...
static RET_CODE on_ca_ipp_buy_result(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  ...
  UI_PRINTF("on_ca_ipp_buy_result %d",para1);
  ...
  switch(para1)
  {
      ...
      case CAS_E_MEM_ALLOC_FAIL: /* No enough money */
        text_set_content_by_strid(p_result, IDS_NO_ENOUGH_MEM);
        ctrl_set_attr(p_result, OBJ_ATTR_ACTIVE);
        ctrl_paint_ctrl(p_result,  TRUE);
        break;
