  ...
  for (i = 0; i<SN_CONDITIONAL_ACCEPT_WORKTIME_WORK_ITEM_CNT; i++)
  {
    switch(i)
    {
     ...
     case 3:
        ...
        text_set_font_style(p_ctrl,FSI_DLG_BTN_N, FSI_COMM_TXT_HL, FSI_DLG_BTN_N);
...
static RET_CODE on_conditional_set_worktime_right(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  ...
  {
    work_data_t.text_strID = IDS_CA_PARAME_SET_ERROR;
	 ui_comm_prompt_open(&work_data_t);
  }
  else if(para1 == 1)
  {
	work_data_t.text_strID = IDS_CAS_E_IC_CMD_FAIL;
	 ui_comm_prompt_open(&work_data_t);
  }
  else if(para1 == 9)
  {
	work_data_t.text_strID = IDS_CA_PASSWORD_PIN_NUMB;
	 ui_comm_prompt_open(&work_data_t);
  }
  else if(para1 == 7)
  {
    ...
    switch(pin_spare_num){
		case 1:
			work_data_t.text_strID = IDS_CA_PIN_ERROR_ONE_CHANCES_LEFT;
			ui_comm_prompt_open(&work_data_t);
			break;
		case 2:
			work_data_t.text_strID = IDS_CA_PIN_ERROR_TWO_CHANCES_LEFT;
			ui_comm_prompt_open(&work_data_t);
			break;
    }
  }
