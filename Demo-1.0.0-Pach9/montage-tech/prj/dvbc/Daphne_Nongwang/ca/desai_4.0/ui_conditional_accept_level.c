...
static RET_CODE on_conditional_accept_level_set(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  ...
  {
	level_data_t.text_strID = IDS_CAS_E_IC_CMD_FAIL;
    ui_comm_prompt_open(&level_data_t);
  }
  else if(para1 == 9)
  {
  ...
  else if(para1 == 7)
  {
    ...
    switch(pin_spare_num){
		case 1:
			level_data_t.text_strID = IDS_CA_PIN_ERROR_ONE_CHANCES_LEFT;
			ui_comm_prompt_open(&level_data_t);
			break;
		case 2:
			level_data_t.text_strID = IDS_CA_PIN_ERROR_TWO_CHANCES_LEFT;
			ui_comm_prompt_open(&level_data_t);
			break;
    }
  }
