...
extern void g_set_mail_header_info(cas_mail_headers_t *p);
...
static RET_CODE plist_update_body(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  g_set_mail_header_info(p_mail_data_buf);
...
quanzhi
  ...
  if(CUSTOMER_RUIXIANG_QUANZHI== cfg.customer)
  {
    email_help_data.bmp_id[1] = IM_F2_2;
  }
#endif
    ...
    u16 i = 0;
  //quanzhi full_screen_title_t title_data = {IM_COMMON_BANNER_MESSAGE,IDS_EMAIL, RSI_TITLE_BG};
...
quanzhi
  ...
  p_sbar = ctrl_create_ctrl((u8 *)CTRL_SBAR, IDC_EMAIL_BAR, 
                              ...
                              p_cont, 0);
  ctrl_set_rstyle(p_sbar, RSI_WHITE_YELLOW, RSI_WHITE_YELLOW, RSI_WHITE_YELLOW);
  sbar_set_autosize_mode(p_sbar, 1);
  sbar_set_direction(p_sbar, 0);
  sbar_set_mid_rstyle(p_sbar, RSI_WHITE_YELLOW, RSI_WHITE_YELLOW, RSI_WHITE_YELLOW);
  ...
  p_ctrl = ctrl_create_ctrl((u8 *)CTRL_TEXT, IDC_EMAIL_NOTIC,
                           ...
                           p_cont, 0);
  ctrl_set_rstyle(p_ctrl, RSI_WHITE_YELLOW, RSI_WHITE_YELLOW, RSI_WHITE_YELLOW);
  ...
  ctrl_set_rstyle(p_ctrl_cont, RSI_DIG_BG, RSI_SN_EMAIL_CONTENT_BG, RSI_DIG_BG);
  ...
  //quanzhi ui_comm_help_create(&email_help_data, p_cont);
