...
extern RET_CODE cas_xsm_attach(cas_module_cfg_t *p_cfg, u32 *p_cam_id);
...
void on_cas_init(void)
{
  ...
  OS_PRINTF("cas_cfg.p_smc_drv[0] = 0x%x\n",cas_cfg.p_smc_drv[0]);
  ...
  cas_module_cfg.nvram_read = nvram_read1;
  cas_module_cfg.nvram_write = nvram_write1;
  ...
  cas_module_cfg.flash_size = CA_DATA_SIZE;//0;
  cas_module_cfg.machine_serial_get = get_stbid;
  //cas_module_cfg.area_limit_free = TRUE;  /**area_limit_free : TRUE:free FALSE:limit***/
#ifndef WIN32
  cas_xsm_attach(&cas_module_cfg, &cas_id);
  cas_module_init(CAS_ID_XSM);
#endif
