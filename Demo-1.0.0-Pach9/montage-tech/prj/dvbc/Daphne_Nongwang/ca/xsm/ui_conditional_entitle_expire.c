...
RET_CODE open_ca_entitle_expire_dlg(u32 para1, u32 para2)
{
  ...
  ctrl_set_rstyle(p_title, RSI_SN_BG, RSI_SN_BG, RSI_SN_BG);
  ...
  p_icon = ctrl_create_ctrl((u8 *)CTRL_BMAP, IDC_ICON,
                   ...
                   40, 40,
                   p_cont, 0);
  bmap_set_content_by_id(p_icon, IM_SN_F1);
  ...
  TIMER_FLAG = fw_tmr_create(ROOT_ID_CA_ENTITLE_EXP_DLG, MSG_CA_CLOSE_TIMER, 8000, FALSE);//wait 2s
  return SUCCESS;
}
