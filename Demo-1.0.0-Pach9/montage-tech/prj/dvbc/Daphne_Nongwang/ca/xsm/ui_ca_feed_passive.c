...
#define RSI_DLG_TITLE		(RSI_SN_BG)
#define RSI_HELP		(RSI_SN_BG)
#define IDS_CA_MATCH_CARD_BEGIN		(IDS_MAINMENU)
#define IDS_MATCH_WARN		(IDS_MAINMENU)
  ...
  p_frm2 = ctrl_create_ctrl((u8*)CTRL_CONT, (u8)(IDC_CA_CARD_FEED_FRM2),
                            ...
                            p_cont, 0);
  ctrl_set_rstyle(p_frm2, RSI_DLG_TITLE, RSI_DLG_TITLE, RSI_DLG_TITLE);
  ...
  text_set_content_by_strid(p_ctrl, IDS_CA_MATCH_CARD_BEGIN);
  ...
  p_ctrl = ctrl_create_ctrl((u8*)CTRL_TEXT, (u8)(IDC_CA_CARD_FEED_WARN),
                            ...
                            p_cont, 0);
  ctrl_set_rstyle(p_ctrl, RSI_HELP, RSI_HELP, RSI_HELP);
  text_set_align_type(p_ctrl, STL_CENTER | STL_VCENTER);
  text_set_content_type(p_ctrl, TEXT_STRTYPE_STRID);
  text_set_content_by_strid(p_ctrl, IDS_MATCH_WARN);
...
static RET_CODE on_conditional_accept_cont_ca_msg(control_t *p_cont, u16 msg, u32 para1, u32 para2)
{
  ...
  {
    text_set_content_by_strid(p_ctrl_tip,IDS_CAS_C_MASTERSLAVE_PAIRED_OK);
    g_feed_status = FEED_STATUS_SUCCESS;
    g_ca_set_feed_status(&g_feed_status);
    fw_tmr_create(ROOT_ID_CONDITIONAL_ACCEPT_FEED_PASSIVE, MSG_CA_CLOSE_TIMER, 5000, FALSE);
  }
...
static RET_CODE close_menu_match_card(void)
{
   g_feed_status = FEED_STATUS_INVALID;
   manage_close_menu(ROOT_ID_CONDITIONAL_ACCEPT_FEED_PASSIVE, 0, 0);
   return ERR_NOFEATURE;
}
                             ...
                             u32 para1, u32 para2)
{
  ...
  if(g_feed_status == FEED_STATUS_NULL)
  {
    ...
    close_menu_match_card();
  return SUCCESS;
}

BEGIN_MSGPROC(conditional_accept_feed_passive_cont_proc, ui_comm_root_proc)
  ON_COMMAND(MSG_CA_JUDGE_MOTHER_CARD, on_judge_card_is_mon_child)
  ...
  ON_COMMAND(MSG_CA_CARD_INFO, on_judge_card_is_mon_child)
