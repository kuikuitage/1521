...
static list_xstyle_t email_item_rstyle =
  ...
  static list_field_attr_t email_plist_attr[EMAIL_LIST_FIELD] =
  {
    ...
    EMAIL_NEW_W, (EMAIL_NUMB_W +10), 0, &email_plist_field_rstyle,  &emial_plist_field_fstyle},

    { LISTFIELD_TYPE_UNISTR | STL_CENTER| STL_VCENTER,
    EMAIL_NAME_W, (EMAIL_NUMB_W + EMAIL_NEW_W+10), 0, &email_plist_field_rstyle,  &emial_plist_field_fstyle },

    { LISTFIELD_TYPE_UNISTR | STL_CENTER| STL_VCENTER,
    EMAIL_SENDER_W-40, EMAIL_NUMB_W + EMAIL_NEW_W+EMAIL_NAME_W+10, 0, &email_plist_field_rstyle,  &emial_plist_field_fstyle },
...
//  control_t *p_item = NULL;
  ...
  p_ctrl = ctrl_create_ctrl((u8*)CTRL_TEXT, IDC_EMAIL_FDELETE,
                            EMAIL_NEW_X-10, SN_EMAIL_HEAD_Y,
                            EMAIL_NEW_W+20, EMAIL_NEW_H,
                            p_menu, 0);
  text_set_font_style(p_ctrl, FSI_WHITE, FSI_WHITE, FSI_WHITE);
  ...
  p_ctrl = ctrl_create_ctrl((u8*)CTRL_TEXT, IDC_EMAIL_NAME,
                            EMAIL_NAME_X+10, SN_EMAIL_HEAD_Y,
                            EMAIL_NAME_W-40, EMAIL_NAME_H,
                            p_menu, 0);
  text_set_font_style(p_ctrl, FSI_WHITE, FSI_WHITE, FSI_WHITE);
  text_set_align_type(p_ctrl, STL_CENTER | STL_VCENTER);
  text_set_content_type(p_ctrl, TEXT_STRTYPE_STRID);
  ...
  p_ctrl = ctrl_create_ctrl((u8*)CTRL_TEXT, IDC_EMAIL_SENDER,
                           EMAIL_SENDER_X-10, SN_EMAIL_HEAD_Y, EMAIL_SENDER_W-40,EMAIL_SENDER_H, p_menu, 0);
  text_set_font_style(p_ctrl, FSI_WHITE, FSI_WHITE, FSI_WHITE);
  text_set_align_type(p_ctrl, STL_CENTER|STL_VCENTER);
  text_set_content_type(p_ctrl, TEXT_STRTYPE_STRID);
...
static RET_CODE on_email_list_select(control_t *p_list, u16 msg, u32 para1, u32 para2)
{
  ...
  OS_PRINTF("on_email_list_select : m_id == %d\n", p_mail_data_buf->p_mail_head[index].m_id);
  index = list_get_focus_pos(p_list);
  #ifndef WIN32
  if(ui_is_smart_card_insert() == TRUE)
  	ui_ca_get_info(CAS_CMD_MAIL_BODY_GET, p_mail_data_buf->p_mail_head[index].m_id, 0);
  else
