...
static RET_CODE on_ppv_ca_orde_select(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  if (p_btn != NULL)
  {
    if(ctrl_get_ctrl_id(p_btn) == IDC_TEXT_BIT_YES)
    {
      ...
      ui_ca_get_info((u32)CAS_CMD_IPP_BUY_SET, (u32)p_buy_info,0);
    }
    else
    {
...
static RET_CODE on_ppv_ca_burse_info(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  if(p_burse->p_burse_info[p_burse->index].cash_value >= 0)
  {
    ...
    get_ppv_str2(uni_str, IDS_CA_CA_RMB, burse_str);
...
static RET_CODE on_ppv_ca_ppv_bug_info(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  get_ppv_str2(uni_str, IDS_CA_PPV_RMB, ipp_str);
  ui_comm_static_set_content_by_unistr(p_text, uni_str);
  ctrl_paint_ctrl(p_ctrl,TRUE);
  return SUCCESS;
}
...
static RET_CODE on_ppv_ca_ppv_show_status(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  {
    ui_comm_static_set_content_by_strid(p_text, IDS_CA_HAS_ORRDERED);
  }
  else
  {
    ui_comm_static_set_content_by_strid(p_text, IDS_CA_NOT_ORDER);
  }
  ctrl_paint_ctrl(p_ctrl, TRUE);
  return SUCCESS;
}
...
#define SN_PPV_TEXT_TITLE_H              35
  ...
  p_attr1 = ctrl_create_ctrl((u8 *)CTRL_CONT, (u8)(IDC_PPV_CONT1),
                        PPV_CONT_TITLE_X,
                        PPV_CONT_TITLE_Y,
                        PPV_CONT_TITLE_W, PPV_CONT_TITLE_H,
                        p_cont, 0);
  ctrl_set_rstyle(p_attr1, RSI_DIG_BG, RSI_DIG_BG, RSI_DIG_BG);//RSI_CA_PPV_N
  ctrl_set_keymap(p_attr1, ca_ppv_cont1_keymap);
  ctrl_set_proc(p_attr1, ca_ppv_cont1_proc);
  
  ctrl_set_attr(p_attr1, OBJ_ATTR_ACTIVE);
  
  p_ctrl = ctrl_create_ctrl((u8 *)CTRL_TEXT, (u8)(IDC_TEXT_TITLE1),
                        PPV_TEXT_TITLE_X,
                        PPV_TEXT_TITLE_Y,
                        PPV_TEXT_TITLE_W, PPV_TEXT_TITLE_H,
                        p_attr1, 0);
  ctrl_set_rstyle(p_ctrl, RSI_SN_BG, RSI_SN_BG, RSI_SN_BG);//RSI_CA_PPV_N
  text_set_font_style(p_ctrl, FSI_CA_PPV_SH, FSI_CA_PPV_SH, FSI_CA_PPV_SH);
  text_set_align_type(p_ctrl, STL_CENTER | STL_VCENTER);
  text_set_content_type(p_ctrl, TEXT_STRTYPE_STRID);
  text_set_content_by_strid(p_ctrl, IDS_CA_PPV_ICON);

  p_attr2 = ctrl_create_ctrl((u8 *)CTRL_CONT, (u8)(IDC_PPV_CONT2),
                        PPV_CONT_TITLE2_X,
                        PPV_CONT_TITLE2_Y,
                        PPV_CONT_TITLE2_W, PPV_CONT_TITLE2_H,
                        p_cont, 0);
  ctrl_set_rstyle(p_attr2, RSI_SN_BG, RSI_SN_BG, RSI_SN_BG);//RSI_CA_PPV_N
  ctrl_set_keymap(p_attr2, ca_ppv_cont2_keymap);
  ctrl_set_proc(p_attr2, ca_ppv_cont2_proc);
  
  ctrl_set_attr(p_attr2, OBJ_ATTR_HIDDEN);
  

  p_ctrl2 = ctrl_create_ctrl((u8 *)CTRL_TEXT, (u8)(IDC_TEXT_TITLE2),
                        PPV_TEXT_TITLE_X,
                        PPV_TEXT_TITLE_Y,
                        PPV_TEXT_TITLE_W, PPV_TEXT_TITLE_H,
                        p_attr2, 0);
  ctrl_set_rstyle(p_ctrl2, RSI_SN_BG, RSI_SN_BG, RSI_SN_BG);//RSI_CA_PPV_N
  text_set_font_style(p_ctrl2, FSI_CA_PPV_SH, FSI_CA_PPV_SH, FSI_CA_PPV_SH);
  ...
  {
    x = PPV_TEXT_BIT_X;
    p_bit = ctrl_create_ctrl((u8 *)CTRL_TEXT, (u8)(IDC_TEXT_BIT_YES+i),
                        (PPV_TEXT_BIT_X+(PPV_TEXT_BIT_W+40)*i),
                        PPV_TEXT_BIT_Y,
                        PPV_TEXT_BIT_W, PPV_TEXT_BIT_H,
                        p_attr1, 0);
    ctrl_set_rstyle(p_bit, SN_RSI_BNT_TEXT_SH, SN_RSI_BNT_TEXT_HL, SN_RSI_BNT_TEXT_SH);//RSI_CA_PPV_BTN_SH, RSI_CA_PPV_BTN_HL, RSI_CA_PPV_BTN_SH
  ...
  #ifndef WIN32
  ui_ca_get_info((u32)CAS_CMD_GET_IPP_BUY_INFO, 0,0);
  ui_ca_get_info((u32)CAS_CMD_BURSE_INFO_GET, 0,0);
  ui_ca_get_info((u32)CAS_CMD_IPPV_INFO_GET, 0,0);
  #endif
  return SUCCESS;
}
