...
RET_CODE open_ca_card_info_adt_mg(u32 para1, u32 para2)
{
...
//#define SN_SUBMENU_CNT 	    6//8
#define SN_SUBMENU_BTN_W 	((SN_SUBMENU_W-20)/2)
#define SN_SUBMENU_BTN_H 	61
#define SN_SUBMENU_BTN_X1 	0
#define SN_SUBMENU_BTN_Y 	20//0
//#define SN_SUBMENU_CONT_H          (((SN_SUBMENU_CNT/2)*SN_SUBMENU_BTN_H)+42+52)
#define SN_SUB_MENU_HELP_RSC_CNT	15

#define SN_CARD_INFO_ADT_BTN_V_GAP     50
  ...
  u16 i,x,y;
  u16 item_str[SN_CARD_ADT_CNT] =
  {
  	  IDS_SMARTCARD_INFO,
	  IDS_CARD_MANAGE,//IDS_CA_AUTHOR_PROG_INFO,
	  IDS_CAS_C_CHARA_CODE,
  };
  ...
  DEBUG_ENABLE_MODE(CARD_INFO,INFO);
  ...
  // create 
  for(i = 0; i < SN_CARD_ADT_CNT; i++)
  {
        x = (SN_SUBMENU_W-(SN_SUBMENU_BTN_W))/2;
        y = SN_SUBMENU_BTN_Y + i*(SN_SUBMENU_BTN_H + SN_CARD_INFO_ADT_BTN_V_GAP);
    ...
    switch(ctrlID)
      {
		...
		case IDC_DISPLAY_SET:
		  manage_open_menu(ROOT_ID_CA_MOTHER, 0, 0);
		  break;
		
		case IDC_SMARTCARD_INFO:
		  manage_open_menu(ROOT_ID_CHARACTER_CODE, 0, 0);//ROOT_ID_CA_IPPV_PPT_DLG,ROOT_ID_CHARACTER_CODE,ROOT_ID_CA_FINGER,ROOT_ID_CA_PPV_ICON
		  break;
