...
static void conditional_accept_info_set_content(control_t *p_cont, cas_operators_info_t *p_card_info)
{
  ...
  u8 asc_str[CONDITIONAL_ACCEPT_INFO_CA_INFO_MAX_LENGTH];
  int i = 0;
  DEBUG(DVBCAS_PORTING,INFO,"****&&&&& p_card_info->p_operator_info[0].operator_id = %d", p_card_info->p_operator_info[0].operator_id);
  DEBUG(DVBCAS_PORTING,INFO,"****&&&&&operator_name = %s",p_card_info->p_operator_info[0].operator_name);
  DEBUG(DVBCAS_PORTING,INFO,"****&&&&&usNetworkID = %d",p_card_info->dvbca_operator_info[0].usNetworkID);
  DEBUG(DVBCAS_PORTING,INFO,"%02d-%02d-%02d %02d:%02d:%02d ",(p_card_info->dvbca_operator_info[0].stExpiredTime.ucYear), (p_card_info->dvbca_operator_info[0].stExpiredTime.ucMonth)
  ...
  else
  {
  	for(i = 0;i < 5;i++)
	{
		...
		{
			DEBUG(DVBCAS_PORTING,ERR,"conditional_accept_info_set_content failed\n");
		}
	}
  }
}
...
RET_CODE open_dvb_ca_operator_info(u32 para1, u32 para2)
{
  ...
  { 
    IDS_CA_SP_ID, IDS_CA_SP_NAME, IDS_CA_NET_ID,IDS_CA_CARD_DUETIME,IDS_CA_CARD_DUETIME
  };
