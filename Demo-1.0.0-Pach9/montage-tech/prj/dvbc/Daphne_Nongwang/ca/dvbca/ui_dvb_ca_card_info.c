...
static void conditional_accept_info_set_content(control_t *p_cont, cas_card_info_t *p_card_info)
{
 ...
 // control_t *p_cas_satue = ctrl_get_child_by_id(p_frm, IDC_CA_CA_STATUS);
  ...
  control_t *p_stb_pair_info = ctrl_get_child_by_id(p_frm, IDC_STB_PAIR_INFO); 
...
//  u8 card_state = p_card_info->paterner_stat;
  ...
  else
  {
   ...
   ui_comm_static_set_content_by_ascstr(p_card_id, asc_str);
	...
	//stb pair info
    sprintf((char *)asc_str, "%s", (char *)p_card_info->stb_pair_id);
...
RET_CODE open_dvb_ca_card_info(u32 para1, u32 para2)
{
//CA frame
...
#define SN_ACCEPT_INFO_CA_FRM_H                   360
  ...
  {
    IDS_CA_PROVIDER, IDS_CA_CONDITION_VER,IDS_CA_CARD_SERIAL,IDS_CA_CARD_VERSION,
    IDS_CA_WATCH_LEVEL,IDS_CA_WORK_TIME_AREA,IDS_STB_CARD_PAIR,
  };
...
#define 	SN_CA_INFO_ITEM_V_GAP    20
  ...
  DEBUG(CA_CARD_INFO,INFO,"open_dvb_ca_card_info now\n");
