...
RET_CODE open_ca_urgent(dvbcas_urgent *p_ca_urgent)
{
	control_t *p_cont = NULL,*p_title = NULL,*p_content = NULL;
	u16 uni_str[DVBCA_NOTIFY_CONTENT_SIZE + 1];
	DVBCA_UINT8 *title = (DVBCA_UINT8*)p_ca_urgent->Title;
	DVBCA_UINT8 *content = (DVBCA_UINT8*)p_ca_urgent->Content;
#ifndef WIN32
	...
	p_title = ctrl_create_ctrl((u8 *) CTRL_TEXT, IDC_URGENT_TITLE, 
	...
	p_content = ctrl_create_ctrl((u8 *) CTRL_TEXT, IDC_URGENT_CONTENT, 
