...
#define SN_CHARACTER_CODE_LXD_H					  30 // 360
	...
	p_cont1 = ctrl_create_ctrl((u8 *)CTRL_CONT, (u8)IDC_CHARACTER_CONT,
					    0, 0,
					    SN_CHARACTER_CODE_CONT_W, SN_CHARACTER_CODE_CONT_H-30,
					    p_cont, 0);
	...
	//card character code
	p_ctrl = ctrl_create_ctrl((u8 *)CTRL_TEXT, (u8) IDC_CHARACTER_NUMB,
	...
	//TEXT
	p_ca_frm= ctrl_create_ctrl((u8 *)CTRL_TEXT, (u8)(IDC_CHARACTER_CODE),
	...
	text_set_font_style(p_ca_frm, FSI_COMM_BTN, FSI_COMM_BTN, FSI_GRAY);
	text_set_align_type(p_ca_frm, STL_LEFT | STL_TOP);
	text_set_content_type(p_ca_frm, TEXT_STRTYPE_STRID);
	text_set_content_by_strid(p_ca_frm, IDS_CAS_C_CHARA_CODE);
