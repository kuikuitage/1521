...
u32 FlashParser::find_dm_head_start()
{
  u32 i = 0;
  u32 dm_head_start = 0;
...
u32 FlashParser::find_dm_head_start2()
{
  u32 i = 0;
  u32 dm_head_start = 0;
  u32 init_start = (u32)m_pFlashData;
...
{  
  s32 result = 0;
  unsigned int dm_head_start_main = find_dm_head_start();
  unsigned int dm_head_start_main2 = find_dm_head_start2();

  result = __get_dm_block_head_info(block_id, dest, dm_head_start_main2);

  if (result)
    return result;

  result = __get_dm_block_head_info(block_id, dest, dm_head_start_main);
...
int FlashParser::update_dm_block_head_info(int block_id)
{
  ...
  unsigned int dm_head_start_main = find_dm_head_start();
  result = __update_dm_block_head_info(block_id, 0xc);
  
  if (result)
    return result;
  
  result = __update_dm_block_head_info(block_id, dm_head_start_main);
