...
static void channel_request(void)
{
    ...
    OS_PRINTF("request table sec %d, pid %x\n", g_priv.cur_sec_num, g_priv.pid);
...
BOOL ap_table_parse_setup(os_msg_t *p_msg)
{
  if(p_msg != NULL)
  {
    ...
    {
    case NC_EVT_LOCKED:
      OS_PRINTF("locked in channel init\n");
    ...
    case DVB_DTM_FOUND:
      // process nit
      {
        ...
        OS_PRINTF("check channel version:new version =%1d, old version=%d\n", g_priv.version,channel_version);
  ...
  switch(g_priv.status)
  {
    ...
    case INIT_DONE_CHANNEL_PARSE: //done
      //release resource
      {
        ...
        u32 channel_version = 0;
        channel_version =  (u32)sys_status_get_channel_version();
        OS_PRINTF("process channel version:new version =%1d, old version=%d\n", g_priv.version,channel_version);
 ...
 void ap_table_parse_task_start(u32 freq, u32 sym, u16  modulation)
{
  ...
  nc_set_tp(nc_handle, g_priv.p_nc_svc, &search_info);
