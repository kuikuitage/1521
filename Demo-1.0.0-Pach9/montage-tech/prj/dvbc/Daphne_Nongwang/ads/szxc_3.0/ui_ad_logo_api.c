...
#if(CONFIG_ADS_ID == CONFIG_ADS_ID_SZXC30)
...
void pic_player_start(ap_pic_data_t *p_pic_data)
{
  ...
  OS_PRINTF("szxc3.0 ADV pic_player_start is_pic_stop[%d]\n",is_pic_stop);
...
RET_CODE ui_adv_set_pic(u32 size, u8 *pic_add)
{
  ...
  if (pic_index == 0)
  {
    ...
    OS_PRINTF("(4 * (g_rect.right - g_rect.left) * (g_rect.bottom - g_rect.top) : [%d], g_mem_size: [%d]\n",(4 * (g_rect.right - g_rect.left) * (g_rect.bottom - g_rect.top)) , g_mem_size);
  ...
  { 
    UI_PRINTF("[szxc3.0 ui_ads_pic_start] pic start !!!!!!   \n");
...
BOOL ui_adv_set_pic_by_type(ads_ad_type_t type)
{
  ads_module_cfg_t module_config = {0};
  u16 pg_id = 0;
#if((CONFIG_ADS_ID == CONFIG_ADS_ID_DESAI) || (CONFIG_ADS_ID == CONFIG_ADS_ID_SZXC30))
  u32 cmd = 0;
#endif
  dvbs_prog_node_t pg = {0};
  ...
  OS_PRINTF("szxc3.0 start show picture tickets2=%d\n", mtos_ticks_get());
#ifndef WIN32
#if(CONFIG_ADS_ID == CONFIG_ADS_ID_DESAI)
...
#elif(CONFIG_ADS_ID == CONFIG_ADS_ID_SZXC30)
