...
static ST_AD_DS_PICTURE_SHOW_INFO * pic_full_scr_info = NULL;
static ST_AD_DS_PICTURE_SHOW_INFO * pic_unauth_ad_info = NULL;
...
static RET_CODE nvram_ad_logo_write(ST_AD_DS_AV_SHOW_INFO * av_show_info)
{
  RET_CODE ret = FALSE;
  ...
  ST_AD_DS_LOG_SAVE_INFO w_logo_info;
  ...
  if((size!=0) && (size <= block_size - sizeof(ST_AD_DS_LOG_SAVE_INFO) - 4))
  {
    ...
    r_logo_info = (ST_AD_DS_LOG_SAVE_INFO*)p_block_buffer;
    if(r_logo_info->uiLogoSize == size)
    {
      //check crc
      //read orig data and crc info 
      p_read_crc_buffer = (u8*)(p_block_buffer + sizeof(ST_AD_DS_LOG_SAVE_INFO) + r_logo_info->uiLogoSize);
    ...
    //logo info write
    memcpy(p_block_buffer,&w_logo_info,sizeof(ST_AD_DS_LOG_SAVE_INFO));
    UI_PRINTF("### sizeof(ST_AD_DS_LOG_SAVE_INFO) = 0x%x\n",sizeof(ST_AD_DS_LOG_SAVE_INFO));
    /***rewrite data***/
    memcpy(p_block_buffer+sizeof(ST_AD_DS_LOG_SAVE_INFO), p_buf, size);
    ...
    memcpy(p_block_buffer+sizeof(ST_AD_DS_LOG_SAVE_INFO)+size, p_write_crc_buffer, 4);

    write_length = sizeof(ST_AD_DS_LOG_SAVE_INFO) + size + 4;
...
void ui_adv_get_osd_msg(u16 pg_id)
{
  ...
  ret = ads_io_ctrl(ADS_ID_ADT_DESAI,ADS_IOCMD_AD_OSD,&module_config);
...
static RET_CODE ui_pic_fullscr_play(void)
{
  ...
  if((pic_full_scr_info->uiDataLen == 0) ||
     ...
     (DSAD_PIC_GIF != pic_full_scr_info->enPicType))
  {
    return ret;
  }
  //fullscreen ad
...
{
  ST_AD_DS_OSD_SHOW_INFO * osd_show_info = NULL;
  u16 uni_str[260] = {0};
  u16 index = 0;
  
  osd_show_info = (ST_AD_DS_OSD_SHOW_INFO *)(osd_prom_show_info.stTextShow + uc_current_osd_index);
    ...
    for(index = uc_current_osd_index + 1; index <= osd_prom_show_info.ucOsdNum; index++)
    {
      ...
      else
      {
        if(osd_display_info[index].b_SaveFlag == TRUE)
         { 
           osd_show_info = (ST_AD_DS_OSD_SHOW_INFO *)(osd_prom_show_info.stTextShow + index);
           gb2312_to_unicode(osd_display_info[uc_current_osd_index].osd_display_buffer, 
                                                                osd_show_info->usContentSize, uni_str, MAX_ROLL_OSD_LENGTH);
...
{
  RET_CODE ret = ERR_FAILURE;
  ST_AD_DS_PICTURE_SHOW_INFO * pic_show_info = NULL;
  pic_show_info = (ST_AD_DS_PICTURE_SHOW_INFO*)p_data;
  ...
  switch(pic_show_info->enPosType)
  {
    case DSAD_STB_UNAUTHORIZED:
    {
      ...
      pic_unauth_ad_info = (ST_AD_DS_PICTURE_SHOW_INFO*)p_data;
    ...
    case DSAD_STB_FULLSCREEN:
      {
        ...
        pic_full_scr_info = (ST_AD_DS_PICTURE_SHOW_INFO*)p_data;
    ...
    case DSAD_STB_MENU:
      {
        if(DSAD_PIC_GIF == pic_show_info->enPicType)
        {
...
static RET_CODE ui_save_logo_data_to_nvm(ST_AD_DS_AV_SHOW_INFO * av_show_info)
{
  RET_CODE ret = ERR_FAILURE;
...
{
  RET_CODE ret = SUCCESS;
  ST_AD_DS_AV_SHOW_INFO * av_show_info = NULL;
  av_show_info = (ST_AD_DS_AV_SHOW_INFO*)p_data;
...
RET_CODE ui_adv_desai_display(ads_ad_type_t ad_type, u8 *p_data)
{
  ...
  switch(ad_type)
  {
    case ADS_AD_TYPE_OSD:
    {
      ...
      ret = ui_show_desai_osd_ad(p_data);
      break;
    }
