...
{	
	DEBUG(DVBAD, NOTICE, "exit get ad monitor\n");
...
static RET_CODE add_data_to_list(ads_data_node **p_list, ads_pic_attribute pic_attr)
{
	...
	{
		DEBUG(DVBAD, INFO, "null == p_list\n");
		return ERR_FAILURE;
	}
	DEBUG(DVBAD, INFO, "type:%d\n", p_node->ads_pic_attribute_data.type);
...
static RET_CODE dvbad_drv_erase(u32 AD_length)
{
  ...
  DEBUG(DVBAD, INFO, "Erase ad flash.size:%#x\n", elength);
  MT_ASSERT(elength != 0);
  if(NULL != p_dvbad_priv->nv_erase)
  {
    ...
    if(SUCCESS != ret)
    {
        DEBUG(DVBAD, ERR,"Erase flash data failed.\n");
        return ERR_FAILURE;
    }
  }
  else
  {
	DEBUG(DVBAD, ERR, "Erase ad flash failure\n");
	return ERR_FAILURE;
  }

  DEBUG(DVBAD, INFO, "Erase ad flash success\n");
  return SUCCESS;
}
...
static ads_data_node *dvbad_get_node_by_id(ads_data_node* p_data, u16 id)
{
	...
	while (id != p->ads_pic_attribute_data.file_number)
	{
		if (NULL== p->next)
		{
			DEBUG(DVBAD,ERR, "ERROR!(NULL== *p->dvbad_pic_attribute_node)\n");
			return NULL;
		}
		p = p->next;
	}
...
static RET_CODE dvbad_drv_readflash(u32 flashOffset, u8 *extBuffer, u32 size)
{
  ...
  DEBUG(DVBAD, INFO, "nvrOffset=%x,size=%x\n",flashOffset,size);
  ...
  if(NULL != p_dvbad_priv->nv_read && size_back != 0)
  {
    ...
    if(SUCCESS != ret)
    {
        DEBUG(DVBAD, INFO, "Read flash data failed.\n");
        return ERR_FAILURE;
     }
  }
  return  SUCCESS;
...
static RET_CODE dvbvad_drv_writeflash(u32 flashOffset, u8 * extBuffer, u32 size)
{
	...
	DEBUG(DVBAD, INFO, "Offset=%x,extBuffer=%#x,size=%#x\n",flashOffset,extBuffer,size);

	if(NULL==extBuffer)
	{
		DEBUG(DVBAD, ERR, "NULL data write to flash\n");
		return ERR_FAILURE;
	}
	if(NULL != p_dvbad_priv->nv_write && size_back != 0)
	{
		...
		if(SUCCESS != ret)
		{
			DEBUG(DVBAD, ERR,"Write flash data failed.\n");
			return ERR_FAILURE;
		}
	}
	return SUCCESS;
...
static RET_CODE	add_flash_data_to_list(ad_head_flash ad_head_data,u8 *p_buf, u32 *read_data_len,ads_data_node **p)
{
	...
	*read_data_len += sizeof(ads_pic_attribute);
	...
	if(NULL==p_node)
	{
		DEBUG(DVBAD, ERR, "null==p_node\n");
		return ERR_FAILURE;
	}
	DEBUG(DVBAD, INFO, "p_node:%d, p_ads_pic_data_addr:%d\n", p_node, p_node->p_ads_pic_data);
	...
	*read_data_len += p_node->p_pic_info.pic_original_size;
	...
	DEBUG(DVBAD, INFO, "*********************************\n");
	DEBUG(DVBAD, INFO, "one pic parser end.num:%d\n", ad_head_data.ad_pic_num);
	DEBUG(DVBAD, INFO, "file_number:%d\n", p_node->ads_pic_attribute_data.file_number);
	DEBUG(DVBAD, INFO, "text_size:%d\n", p_node->text_size);
	DEBUG(DVBAD, INFO, "id.num:%d\n", p_node->p_pic_info.pic_id);
	DEBUG(DVBAD, INFO, "pic_size:%d\n", p_node->p_pic_info.pic_original_size);
	DEBUG(DVBAD, INFO, "*********************************\n");
	return SUCCESS;
}
...
static RET_CODE read_ad_from_flash(dvbad_ads_pic_data *p_ads_pic_data)
{
	...
	if(SUCCESS!=ret)
	{
		DEBUG(DVBAD, ERR, "read flash failure\n");
		return DVBAD_UPDATE_FAILURE;
	}
	if(SUCCESS!=dvbad_check_sum((char*)&ad_head_data, sizeof(ad_head_flash)))
	{
		DEBUG(DVBAD, ERR, "read ad data from flash and check sum failure\n");
		return DVBAD_UPDATE_FAILURE;
	}
	...
	{
		delete_node(&p->next);
		DEBUG(DVBAD, INFO, "p:%d, addr:%d\n", p, &p_ads_pic_data->dvbad_pic_attribute_node);
	}
	if(NULL!=p->p_ads_pic_data)
	{
		DEBUG(DVBAD, INFO, "free p_ads_pic_data\n");
...
static RET_CODE dvbad_save_to_flash()
{
	...
	DEBUG(DVBAD, INFO, "Start save ad to flash\n");
	if(NULL==ad->p_ad_data)
	{
		DEBUG(DVBAD, ERR, "Have no data save to flash\n");
		return ERR_FAILURE;
	}
#ifndef WIN32
    ...
    if(flash_data == NULL)
    {
        DEBUG(DVBAD, ERR, "**memory malloc wrong\n");
        return ERR_FAILURE;
    }
    ...
    while(NULL != p)
    {
		...
		{
			DEBUG(DVBAD, ERR, "flash_data_size > AD_FLASH_SIZE! Have no enough flash\n");
			break;
		}
		flash_data[flash_data_size] = (u8)((p->text_size&0xFF00)>>8);
		flash_data[flash_data_size+1] = (u8)(p->text_size&0x00FF);
		DEBUG(DVBAD, INFO, "text_size:%#x,flash_data[0]:%d, flash_data[1]:%d, flash_data_size:%d\n", 
		...
		DEBUG(DVBAD, INFO, "id:%d, p_pic_info addr:%d\n",p->p_pic_info.pic_id, flash_data_size);
...
#ifndef WIN32
	DEBUG(DVBAD, INFO, "crc_32_write:%#x\n", crc_32_write);
    ret = dvbvad_drv_writeflash(0, flash_data, flash_data_size+4);
    if(ret != SUCCESS)
    {
        DEBUG(DVBAD, ERR, "ad write flash wrong\n");
		mtos_free(p_buf);
        mtos_free(flash_data);
        return ERR_FAILURE;
    }
	...
	mtos_free(p_buf);
    mtos_free(flash_data);
	DEBUG(DVBAD, INFO, "Save ad to flash succcess\n");
    return SUCCESS;
...
static ads_data_node *dvbad_get_data_node(ads_data_node *p_node, DVBAD_POS_TYPE ad_type)
{
	...
	DEBUG(DVBAD, INFO, "get data node now.ad_type:%d\n", ad_type);
	DEBUG(DVBAD, INFO, "p ->ads_pic_attribute_data.type:%d\n", p ->ads_pic_attribute_data.type);
	while(ad_type!=p ->ads_pic_attribute_data.type)
	{
		if(NULL==p->next)
		{
			DEBUG(DVBAD, ERR, "get data node failure\n");
			return NULL;
		}
		p  = p ->next;
	}
...
RET_CODE dvbad_get_logo_size(dvbad_log_save_info *p_logo_info)
{
	...
	DEBUG(DVBAD, INFO, "get logo size end.uiLogoSize:%d,ucLogType:%d\n", p_logo_info->uiLogoSize, p_logo_info->ucLogType);
	return TRUE;
}
...
RET_CODE dvbad_get_ad(dvbad_program_para para, dvbad_show_pic_info *p_pic, DVBAD_POS_TYPE ad_type)
{
	...
	dvbad_ads_unlock();
	DEBUG(DVBAD, INFO, "pic_id:%d, pic_type:%d,len:%d, type:%d\n",
...
#define ONE_PIC_INFO_LENGTH		(19)
	...
	for(i = 0; i<pic_num;i++)
	{
		...
		{
			DEBUG(DVBAD, ERR, "ads_save_pic_info return FALSE\n");
			return FALSE;
		}
	}		
	return TRUE;
...
static BOOL ads_get_head_data(u8 *p_data, u8 *p_xml_data, u32 *xml_len, u8 *p_info_data, u32 *info_len)
{
	...
	if(NULL==p_data)
	{
		DEBUG(DVBAD, ERR, "(NULL==p_data).error");
		return FALSE;
	}
	...
	{
		DEBUG(DVBAD, ERR, "wrong data_type!!\n");
		return FALSE;
	}
	OS_PRINTF("\n");
	DEBUG(DVBAD, INFO, "ads_get_head_data return true DATA_TYPE_PIC_INFO %d\n", data_type);
	return TRUE;
}
...
static RET_CODE ads_save_pic_data(ads_data_node *p_ads_pic_data_node, u8 *p_data, u32 pic_data_len, u16 pid)
{
	...
	DEBUG(DVBAD, INFO, "save pic data end.pic_data_len:%d, id:%d\n",pic_data_len, p->ads_pic_attribute_data.file_number);
	return SUCCESS;
}
...
static RET_CODE ads_get_next_pid(ads_data_node *p_node, u16 pid, u16 *next_pid)
{
	...
	if(NULL!=p->next)
	{
		*next_pid = p->next->ads_pic_attribute_data.file_number;
		DEBUG(DVBAD, INFO, "next pid is %d\n", *next_pid);
		return SUCCESS;
	}
...
static RET_CODE ads_get_pic_data(u8 *p_data, u8 *p_pic_data, u32 *pic_data_len)
{
	...
	*pic_data_len += data_len;
	...
	DEBUG(DVBAD, INFO, "get pic data end\n");
	return TRUE;
}
...
static filter_buf_node  *create_filter_buf_list(u8 *p_data, u32 data_len)
{
	...
	{
		DEBUG(DVBAD, ERR, "Error p_data:%d, data_len:%d!!!\n", p_data, data_len);
		return NULL;
	}
...
static RET_CODE add_data_to_filter_list(filter_buf_node** p_node, u8 *p_data, u32 data_len)
{
	if(NULL==*p_node)
	{
		DEBUG(DVBAD, INFO, "add new p_data\n");
		*p_node = create_filter_buf_list(p_data, data_len);
		MT_ASSERT(NULL!=*p_node);
		return SUCCESS;
	}
	DEBUG(DVBAD, INFO, "add p_data\n");
												...
												u8 old_data_type, u16 old_extension_id)
{
	...
	while(NULL!=p_node)
	{
		...
		{
			DEBUG(DVBAD, INFO, "data_type:%d,table_extension_id:%d,section_number:%d", data_type, table_extension_id, section_number);
			return p_node;
		}
		p_node = p_node->next;
	}
...
static RET_CODE parser_head_data(filter_buf_node *p_list)
{
	...
	while(1)
	{
		...
		if(NULL==p_node)
		{
			DEBUG(DVBAD, ERR, "None of next head node\n");
			return ERR_FAILURE;
		}
		if(FALSE==ads_get_head_data(p_node->p_data,ad->xml, &ad->xml_pos, ad->pic_info, &ad->pic_info_pos))
		{
			...
			ad->xml = NULL;
			ad->pic_info = NULL;
			DEBUG(DVBAD, ERR, "ads_get_head_data return FALSE!\n");
			break;
		}
						...
						(table_extension_id==ad->down_ctl.ads_pic_info.pic_information_table_id_end))
		{
			if(DATA_TYPE_XML == data_type)
			{
				...
				DEBUG(DVBAD, INFO, "sizeof(dvbad_ads_pic_data) %d\n", sizeof(dvbad_ads_pic_data));
				if(NULL==ad->p_ad_data)
				{
					mtos_free(ad->xml);
					mtos_free(ad->pic_info);
			...
			else if(DATA_TYPE_PIC_INFO == data_type)
			{
				...
				{
					DEBUG(DVBAD, ERR, "ads_pic_info_parser return FALSE\n");
					return DVBAD_FUN_ERR_PARA;
				}
				mtos_free(ad->xml);
				mtos_free(ad->pic_info);
				ad->xml = NULL;
				ad->pic_info = NULL;
				DEBUG(DVBAD, INFO, "parser head data success\n");
				return SUCCESS;
			}			
		}
	}
...
static filter_buf_node* get_data_node_by_id(filter_buf_node *p_list, u8 section_number, u16 pic_id)
{
	...
	while(NULL!=p_node)
	{
		...
		{
			DEBUG(DVBAD, INFO, "pic_id:%d,section_number:%d\n", pic_id, section_number);
			return p_node;
		}
		p_node = p_node->next;
	}
	DEBUG(DVBAD, ERR, "Have none pic_id node\n");
	return NULL;
}
...
static u32 get_one_pic_size(u16 pic_pid, ads_data_node *p_node)
{
	...
	DEBUG(DVBAD, INFO, "get pic size end:%d\n", p->p_pic_info.pic_original_size);
...
static RET_CODE save_filter_pic_data(filter_buf_node *p_list, ads_data_node *p_data_node, u16 pic_id)
{
	...
	if (NULL==ad->pic_data)
	{
		DEBUG(DVBAD, INFO, "malloc pic data\n");
	...
	do
	{
		...
		if(NULL==p_buf_node)
		{
			DEBUG(DVBAD, ERR, "no pic_id node\n");
			mtos_free(ad->pic_data);
			ad->pic_data = NULL;
			return ERR_FAILURE;
		}
...
static RET_CODE get_free_filter_id(u16 *filter_id)
{
	...
	DEBUG(DVBAD, INFO,"have no free filter\n");
	return ERR_BUSY;
}
...
static BOOL dvbad_set_filter_data_table(u16 filter_id, u8 table_id, u16 extension_id, u8 section_number)
{
	...
	DEBUG(DVBAD, INFO, "table_id:%d,extension_id:%d,section_number:%d\n",table_id,extension_id,section_number);
	...
	DEBUG(DVBAD, INFO,"dvbad_set_filter_data_table return true\n");
	return TRUE;
}
...
static ST_AD_DVBAD_FUNC_TYPE dvbad_set_filter_for_adhead(u16 filter_id, u16 table_id, u8 data_type, u8 section_number )
{
	...
	filter_info.pfDataNotifyFunction = NULL;
	ret = DVBAD_SetFilter(&filter_info);
	DEBUG(DVBAD, INFO,"end.ret:%d, section_number:%d, data_type:%d, table_id:%d\n", ret, section_number, data_type, table_id);
	return ret;
}
...
static BOOL dvbad_set_filter_ctrl_info(u16 filter_id)
{
	...
	DEBUG(DVBAD, INFO,"dvbad_set_filter_CTRL_INFO return true\n");
	return TRUE;
}
										...
										u16 table_id, u8 data_type)
{
	...
	DEBUG(DVBAD, INFO, "start get_filter_data, data_type %d\n", data_type);
	DEBUG(DVBAD, INFO, "index_xml_table_id_start %d\n", (ad->down_ctl.ads_xml_info.index_xml_table_id_start));
	DEBUG(DVBAD, INFO, "index_xml_table_id_end %d\n", (ad->down_ctl.ads_xml_info.index_xml_table_id_end));
	DEBUG(DVBAD, INFO, "pic_information_table_id_start %d\n", (ad->down_ctl.ads_pic_info.pic_information_table_id_start));
	DEBUG(DVBAD, INFO, "pic_information_table_id_end %d\n", (ad->down_ctl.ads_pic_info.pic_information_table_id_end));
	DEBUG(DVBAD, INFO, "AD_version:%d\n", ad->p_ad_data->ad_head_data.AD_version);
	DEBUG(DVBAD, INFO, "section_number:%d, table_id:%d, old_table_id:%d, set_table_id:%d\n", section_number, table_id, old_table_id, filter_set.set_table_id);
	DEBUG(DVBAD, INFO, "remaining_section:%d\n", remaining_section);
	DEBUG(DVBAD, INFO, "set_section_number:%d, set_data_type:%d, new_table_flag:%d, set_last_section_number:%d\n", 
	...
	if((0==section_number)&&(table_id==ad->down_ctl.ads_xml_info.index_xml_table_id_start))
	{
		...
		DEBUG(DVBAD, INFO, "reset remaining_section:%d\n", remaining_section);
	}
	if(old_table_id<table_id)
	{
		DEBUG(DVBAD, INFO, "old_table_id:%d, table_id:%d\n", old_table_id, table_id);
		remaining_section += last_section_number-section_number+1;
		old_table_id = table_id;
		filter_set.set_last_section_number = last_section_number;
		filter_set.new_table_flag = TRUE;
	...
	{
		DEBUG(DVBAD, INFO, "head filtration finsh\n");
		return DVBAD_FUN_FILTER_COMPELET;
	}
	if(FALSE==filter_set.new_table_flag)
	{
		DEBUG(DVBAD, INFO, "new_table_new_table_flag==FALSE\n");
		return DVBAD_FUN_OK;
	}
	for(i=0;i<DVB_ADS_FILTER_MAX_NUM;i++)
	{
		if(DVB_ADS_FREE==ad->filter[i].filter_status)
		{
			DEBUG(DVBAD, INFO, "free filter:%d\n", i);
		}
	}
	for(i = 0; i<DVB_ADS_FILTER_MAX_NUM; i++)
	{
		...
		else if(filter_set.set_section_number==filter_set.set_last_section_number)
		{
			...
			{
				DEBUG(DVBAD, INFO, "set filter end.Wait to get data\n");
				return DVBAD_FUN_OK;
			}
		}
		...
		if(DVBAD_FUN_OK!=ret)
		{
			DEBUG(DVBAD, ERR, "set_filter_for_adhead return ERR\n");
			return ret;
		}
		if(FALSE==filter_set.new_table_flag)
		{
			DEBUG(DVBAD, INFO, "New table_id\n");
			return DVBAD_FUN_OK;
		}
	}

	DEBUG(DVBAD, INFO, "end.None of filter is free\n");
	return DVBAD_FUN_OK;
}
...
static ST_AD_DVBAD_FUNC_TYPE reset_data_filter(ads_data_node *p_node, u8 *p_data, filter_buf_node *p_list)
{
	...
	DEBUG(DVBAD, INFO, "reset data filter. section_number:%d,last_section_number;%d,pic_pid:%d\n", section_number, last_section_number, pic_pid);
	if(0==section_number)
	{
	...
	if((last_section_number+1)==sum_section)
	{
		...
		{
			DEBUG(DVBAD, ERR, "save pic data failure\n");
			return DVBAD_DATA_FALSE;
		}
...
static RET_CODE parser_bat_data(u8 *p_data, u32 data_len)
{
	...
	{
		DEBUG(DVBAD, INFO, "update ad.ad_pic_num:%d, p_ad_data->version_number:%d;version_number:%d, ad_head_data.AD_version:%d;AD_version:%d, save_all_pic_flag:%d\n", 
	...
	else
	{
		...
		{
			DEBUG(DVBAD, INFO, "Is the latest version\n");
			exit_monitor(GET_AD_TRUE);
			return SUCCESS;
		}
		if(SUCCESS==read_ad_from_flash(ad->p_ad_data))
		{
			DEBUG(DVBAD, INFO, "get ad from flash end.");
		...
		{
			DEBUG(DVBAD, INFO, "read flash ad failure\n");
			ad->p_ad_data->ad_head_data.version_number = version_number;
			ad->p_ad_data->ad_head_data.AD_version = ad_version;
			if(SUCCESS!=get_free_filter_id(&filter_id))
				return ERR_FAILURE;
	...
	DEBUG(DVBAD, INFO, "version_number:%d, valid_flag:%d, ad_version:%d, rivate_id:%s, date_Length:%d\n", 
								version_number, ad->p_ad_data->ad_bat_data.valid_flag, ad_version, rivate_id, date_Length);
	DEBUG(DVBAD, INFO, "stream_id:%d,network_id:%d, service_id:%d, descriptor_length:%d\n", 
		...
		for(index = 0; index < DVB_ADS_FILTER_MAX_NUM; index++)
		{
			...
			{				
				...
				else if((DATA_PID == ad->filter[index].channel_pid)&&(AD_HEAD_TABLE_ID==p_data[0]))
				{
					...
					last_section_number = p_data[7];
					data_type = p_data[8];
					DEBUG(DVBAD, INFO, "parser,index:%d, p_data[0]:%d, table_extension_id %d, section_number %d, last_section_number %d, data_type %d\n", 
					...
					switch(table_extension_id)
					{
						...
						default:
							...
							{
								DEBUG(DVBAD, ERR, "add_data_to_filter_list failure\n");
								ad->filter[index].filter_status = DVB_ADS_FREE;
								dvbad_ads_filter_free(&ad->filter[index]);
								dvbad_ads_unlock();
								exit_monitor(GET_AD_FAILURE);
							...
							if(DVBAD_FUN_FILTER_COMPELET==reset_ret)
							{
								DEBUG(DVBAD, INFO, "get all head data.start parser head data\n");
								DEBUG(DVBAD, INFO, "AD_version:%d\n", ad->p_ad_data->ad_head_data.AD_version);
								if(SUCCESS!=parser_head_data(p_filter_data_list))
								{
									DEBUG(DVBAD, ERR, "parser head data failure\n");
									exit_monitor(GET_AD_FAILURE);
								}
								else
								{
							...
							else if(DVBAD_FUN_ERR_PARA==reset_ret)
							{
								...
								DEBUG(DVBAD, ERR, "get_filter_data return false\n");
								delete_head_node(&p_filter_data_list);
								exit_monitor(GET_AD_FAILURE);
							}
						break;
				...
				else if((DATA_PID == ad->filter[index].channel_pid)&&(AD_DATA_TABLE_ID==p_data[0]))
				{
					...
					{
						DEBUG(DVBAD, INFO, "filte data finish\n");
						delete_head_node(&p_filter_data_list);
						if(SUCCESS!=dvbad_save_to_flash())
							DEBUG(DVBAD, ERR, "save ad data to flash failure\n");
						if(SUCCESS==get_log_pid(&pid))
							dvb_set_ad_flag(GET_AD_TRUE);
						else 
							DEBUG(DVBAD, INFO, "Have no logo\n");
						exit_monitor(GET_AD_TRUE);
					}
					else if(DVBAD_DATA_FALSE==data_ret)
					{
						DEBUG(DVBAD, ERR,"reset_data_filter return err\n");
						delete_head_node(&p_filter_data_list);
						exit_monitor(GET_AD_FAILURE);
					}
    				dvbad_ads_lock();
...
BOOL dvbad_set_filter_bat()
{
	...
	if(ret != DVBAD_FUN_OK)
	{
		DEBUG(DVBAD, ERR, "[ERR! ret:%d\n",ret);
		MT_ASSERT(0);
	}
	DEBUG(DVBAD, INFO,"dvbad_set_filter_bat return true\n");
	return TRUE;
}
...
BOOL dvb_ad_nim_lock(u32 freq, u32 symbolrate, u16 qammode)
{
    ...
    DEBUG(DVBAD, INFO, "freq %d, symbolrate %d, qammode %d\n", freq, symbolrate, qammode);
    ...
    DEBUG(DVBAD, INFO, "nim_channel_connect lock return %d\n", ret);
...
static RET_CODE get_ad_head_from_fialsh()
{
	...
	if(SUCCESS!=ret)
	{
		DEBUG(DVBAD, ERR, "get_ad_head failure\n");
		return DVBAD_UPDATE_FAILURE;
	}
#endif	
	if(SUCCESS!=dvbad_check_sum((char*)&ad->p_ad_data->ad_head_data, sizeof(ad_head_flash)))
	{
		DEBUG(DVBAD, ERR, "get_ad_head and check sum failure\n");
		return DVBAD_UPDATE_FAILURE;
	}
	DEBUG(DVBAD, INFO, "get_ad_head end.ad_version:%d\n", ad->p_ad_data->ad_head_data.AD_version);
	return SUCCESS;
}
...
void dvbad_info_parser(dvbad_show_pic_info *p_show_info)
{
    u8 *p_data = NULL;
...
//    {
    ...
    p_data = (u8 *)p_show_info;
	p_dvbad_priv->display_ad(ad_type, p_data);
//     }
     /*
	...
	DEBUG(DVBAD, INFO, "dvbad_info_parser end\n");
}
