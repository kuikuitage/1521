...
#if(CONFIG_ADS_ID == CONFIG_ADS_ID_DVBAD)
...
void pic_player_init(void)
{
  ...
  #if((CONFIG_ADS_ID  == CONFIG_ADS_ID_DESAI)||(CONFIG_ADS_ID==CONFIG_ADS_ID_DVBAD))
...
void pic_player_start(ap_pic_data_t *p_pic_data)
{
  ...
  DEBUG(DVBAD, INFO,"is_pic_stop[%d],pic_count:%d\n",is_pic_stop, pic_count);
...
{
  UI_PRINTF("[AD_LOGO] g_show_pic = 0x%x, pic_index:%d\n",g_show_pic,pic_index);
  if(pic_index > 1)
    pic_index = 1;
...
RET_CODE ui_adv_set_pic(u32 size, u8 *pic_add)
{
  ...
  DEBUG(DVBAD,INFO,"size=0x%x, is_pic_stop:%d,is_pic_show_logo:%d\n",size,
  									is_pic_stop, is_pic_show_logo);
  #if 0
  OS_PRINTF("==========================\n");
  ...
  OS_PRINTF("g_picture_data[pic_index].size = 0x%x\n",g_picture_data[pic_index].file_size);
  ...
  { 
	...
	DEBUG(DVBAD, INFO, "file_name:%s\n", g_picture_data[0].file_name);
	DEBUG(DVBAD, INFO, "file_size:%d\n", g_picture_data[0].file_size);
	DEBUG(DVBAD, INFO, "file_or_mem:%d\n", g_picture_data[0].file_or_mem);
	DEBUG(DVBAD, INFO, "buf_get:%d\n", g_picture_data[0].buf_get);
	DEBUG(DVBAD, INFO, "p_mem:%x\n", g_picture_data[0].p_mem);
	DEBUG(DVBAD, INFO, "mem_size:%d\n", g_picture_data[0].mem_size);
	DEBUG(DVBAD, INFO, "rect.left:%d\n", g_picture_data[0].rect.left);
	DEBUG(DVBAD, INFO, "rect.top:%d\n", g_picture_data[0].rect.top);
	DEBUG(DVBAD, INFO, "rect.right:%d\n", g_picture_data[0].rect.right);
	DEBUG(DVBAD, INFO, "rect.bottom:%d\n", g_picture_data[0].rect.bottom);
	DEBUG(DVBAD, INFO, "p_rgn:%d\n", g_picture_data[0].p_rgn);
	DEBUG(DVBAD, INFO, "dec_mode:%d\n", g_picture_data[0].dec_mode);
...
BOOL ui_adv_set_pic_by_type(ads_ad_type_t type)
{
  ...
  u16 pg_id = 0;
#if((CONFIG_ADS_ID == CONFIG_ADS_ID_DESAI) || \
				(CONFIG_ADS_ID == CONFIG_ADS_ID_SZXC)||\
				(CONFIG_ADS_ID == CONFIG_ADS_ID_DVBAD))
  u32 cmd = 0;
#endif
  dvbs_prog_node_t pg = {0};
  ...
  dec_mode = DEC_FRAME_MODE;
	...
	switch(type)
	{
		...
		case ADS_AD_TYPE_VOLBAR:
			...
			g_rect.right = g_rect.left+280;//280
			g_rect.top = 378;//335
			g_rect.bottom = g_rect.top+180;//140
			break;
...
#elif(CONFIG_ADS_ID == CONFIG_ADS_ID_DVBAD)
	ads_dvbad_set_type(type, &cmd, &module_config);
	ret = ads_io_ctrl(ADS_ID_ADT_DVBAD, cmd, &module_config);
	ret = (ret == SUCCESS)?SUCCESS : ERR_FAILURE;
#else
