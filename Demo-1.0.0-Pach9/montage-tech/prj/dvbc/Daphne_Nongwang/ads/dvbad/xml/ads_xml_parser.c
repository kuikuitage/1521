...
static RET_CODE ads_xml_pic_attribute_parse(ads_data_node *p_dvbad_pic_attribute_list,const xmlNodePtr cur_pic, DVBAD_POS_TYPE type)
{
	...
	if(data_NULL_flag)
	{
		...
		{
			DEBUG(DVBAD,ERR,"Error!NULL==dvbad_pic_attribute_list\n");
			return -1;
		}
		else
		{
...
#define CHANNEL_TYPE	2
	...
	{
		DEBUG(DVBAD, ERR, "ads_pic_attribute_data == NULL\n");
		return ERR_FAILURE;
	}
	while(NULL!=cur){
						...
						if(NULL!=cur_pic_attribute)
						{
							...
							if(NULL!=cur_pic_attribute)
							{
								if(FALSE==ads_xml_text_get(p_last_data_list, cur_pic_attribute, buf_len))
									DEBUG(DVBAD, ERR, "get xml text failure\n");
							}
						}
...
//	int i;
	...
	DEBUG(DVBAD, INFO, "xml parse start\n");
	if(NULL==p_buf)
	{
		DEBUG(DVBAD,ERR,"Error!NULL==p_buf\n");
		return ERR_FAILURE;
	}
	doc = xmlParseMemory((char*)p_buf, buf_len);
	DEBUG(DVBAD, INFO, "get doc end\n");
	if(NULL==doc)
	{
		DEBUG(DVBAD,ERR,"Error! xmlParseMemory doc failure\n");
		return ERR_FAILURE;
	}
	root = XML_GET_ROOT(doc);
	if(NULL==root)
	{
		DEBUG(DVBAD,ERR,"Error! NULL==cur get root failure\n");
		xmlFreeDoc(doc);
	}
//	for(i = 0; i<buf_len; i++)
//		OS_PRINTF("%c", p_buf[i]);
//	OS_PRINTF("\n");
	...
	DEBUG(DVBAD, INFO, "xml parse end.ret:%d\n", ret);
	return ret;
}
