...
extern void dvbad_info_parser(dvbad_show_pic_info *p_show_info);
...
/*
static ST_AD_DS_POS_TYPE dvbad_ad_type_get(u32 ads_type)
{
	return DSAD_STB_POS_UNKNOWN;
}*/
...
static RET_CODE ads_adt_dvbad_open()
{
	...
	DEBUG(DVBAD,INFO,"ads_adt_dvbad_open start\n");
	...
	DEBUG(DVBAD, ERR, "ad monitor time out\n");
	return ERR_FAILURE;
}
...
static DVBAD_POS_TYPE dvbad_type_get(u32 ads_type)
{
	...
	DEBUG(DVBAD, INFO, "get ad_type end.ad_type:%d\n", ad_type);
    return ad_type;
}
...
//	dvbad_osd_program_show_info osd_show;
	...
	DEBUG(DVBAD, INFO, "cmd:%d,ad_type:%d,network_id:%d,ts_id:%d,service_id:%d\n",
						cmd, p_cfg->ad_type, p_cfg->service_info.network_id,
						p_cfg->service_info.ts_id, p_cfg->service_info.service_id);
...
RET_CODE ads_adt_dvbad_attach(ads_module_cfg_t * p_cfg, u32 *p_adm_id)
{
	...
	g_ads_priv.adm_op[ADS_ID_ADT_DVBAD].func.close
		= ads_adt_dvbad_close;
	g_ads_priv.adm_op[ADS_ID_ADT_DVBAD].func.display
		= NULL;
	g_ads_priv.adm_op[ADS_ID_ADT_DVBAD].func.hide
		= NULL;
  ...
  p_dvbad_priv->hide_ad = p_cfg->hide_ad;
   ...
   *p_adm_id = ADS_ID_ADT_DVBAD;
	DEBUG(DVBAD, INFO, "p_cfg channel_frequency %d, channel_symbolrate %d, channel_qam%d",
						p_cfg->channel_frequency,  p_cfg->channel_symbolrate, p_cfg->channel_qam);
   return SUCCESS;
}
