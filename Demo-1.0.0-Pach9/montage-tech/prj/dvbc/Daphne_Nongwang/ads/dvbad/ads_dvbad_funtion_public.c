...
static RET_CODE nvram_write(u32 offset, u8 *p_buf, u32 size)
{
  ...
  DEBUG(DVBAD, INFO,"ad write addr[0x%x], size[0x%x]\n", offset, size);
  if(size!=0)
  {
    ret=dm_direct_write(p_dm_handle,DEC_RW_ADS_BLOCK_ID,offset,size,p_buf);
	if(DM_FAIL == ret)
	{
		DEBUG(DVBAD, ERR, "write ad to flash failure\n");
		return ERR_FAILURE;
	}
  }
...
static RET_CODE ads_dvbad_module_init()
{
#if 1
  RET_CODE ret = ERR_FAILURE;
  ...
  module_config.display_ad = ui_adv_dvbad_display;
  module_config.hide_ad = ui_adv_dvbad_hide;
  ...
  ret=ads_init(ADS_ID_ADT_DVBAD,&module_config);
  if(ret != SUCCESS)
  {
    return ret;
  }
  ...
  mtos_printk("ad Version:%s\n",version);
  return SUCCESS;
#endif
}
RET_CODE nvram_ad_logo_read(u32 offset, u8 *p_buf, u32 p_size);
static void ui_show_dvbad_adver_logo(void)
{
    u8 *p_logo_data = NULL;
    dvbad_log_save_info logo_info;

	DEBUG(DVBAD, INFO, "show ad logo start\n");
	memset((void*)&logo_info, 0, sizeof(dvbad_log_save_info));
	if(FALSE==dvbad_get_logo_size(&logo_info))
		return;
	p_logo_data = mtos_malloc(logo_info.uiLogoSize);
	memset((void*)p_logo_data, 0, logo_info.uiLogoSize);
	if(NULL==p_logo_data)
	{
		DEBUG(DVBAD, ERR, "malloc failure\n");
		return;
	}
	if(FALSE==dvbad_get_logo(p_logo_data, logo_info.uiLogoSize))
		return;
	DEBUG(DVBAD, INFO, "get logo data end.start show!\n");
	ui_show_logo_by_data(logo_info.uiLogoSize, p_logo_data);
	mtos_free(p_logo_data);
  }
...
#else
    if(ERR_FAILURE==dvbad_get_logo_flag() ) /**upg first boot***/
#endif  
  {
