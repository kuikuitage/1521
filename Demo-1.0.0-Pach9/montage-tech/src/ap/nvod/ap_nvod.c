...
static void dvbc_lock_tuner(nvod_priv_t *p_priv, u32 freq, u32 sym, u32 mod)
{
  ...
  nc_set_tp(nc_handle, p_nvod_data->p_nim_svc, &p_nvod_data->search_info);
}
...
static void nvod_start(nvod_priv_t *p_priv, os_msg_t *p_msg)
{
  ...
  MT_ASSERT(p_priv->p_nvod_db != NULL);
  ...
  MT_ASSERT(p_priv->p_priv_data->tp_cnt != 0);
  MT_ASSERT(p_priv->p_priv_data->tp_cnt <= MAX_TP_CNT);
