...
#define SUB_PIXEL_BUF_SIZE   (720*576+3*(KBYTES))
...
void subt_txt_info_reset_1(handle_t handle);//temp solution
...
static void do_play_chan_open(pb_handle_t *p_handle, play_param_t *p_param)
{
  ...
  OS_PRINTF("open play chan:  vpid =%d , apid = %d\n", 
                    (u16)p_pg_info->v_pid, (u16)p_pg_info->a_pid);
...
static void do_dynamic_pid(pb_handle_t *p_handle, pmt_t *p_pmt)
{
  priv_data_t *p_data = &p_handle->data;
  ...
  */
  ...
  if(p_pg_info->v_pid != new_video_pid)
  {
    ...
    pid_changed = TRUE;
    OS_PRINTF("dynamic pid: change v_pid to %d\n", new_video_pid);
  }
  ...
  if(pid_changed)
  {
    send_evt_to_ui(PB_EVT_DYNAMIC_PID, 0, 0);
  }
}
...
static void lock_dvbs_tuner_1(pb_handle_t *p_handle, play_param_t *p_param)
{
  priv_data_t *p_data = &p_handle->data;
  class_handle_t nc_handle = class_get_handle_by_id(NC_CLASS_ID);

  nc_set_diseqc(nc_handle, &p_param->inner.dvbs_lock_info.disepc_rcv);
  ...
  nc_set_tp(nc_handle, p_data->p_nim_svc, &p_data->search_info);
}

static void lock_dvbt_tuner_1(pb_handle_t *p_handle, play_param_t *p_param)
{
  priv_data_t *p_data = &p_handle->data;
  class_handle_t nc_handle = class_get_handle_by_id(NC_CLASS_ID);
  ...
  p_data->search_info.channel_info.frequency
    = p_param->inner.dvbs_lock_info.tp_rcv.freq;
  p_data->search_info.channel_info.param.dvbc.modulation
    = p_param->inner.dvbs_lock_info.tp_rcv.nim_modulate;
  p_data->search_info.channel_info.param.dvbc.symbol_rate
    = p_param->inner.dvbs_lock_info.tp_rcv.sym;
 
  nc_set_tp(nc_handle, p_data->p_nim_svc, &p_data->search_info);
}

static void lock_dvbc_tuner_1(pb_handle_t *p_handle, play_param_t *p_param)
{
  priv_data_t *p_data = &p_handle->data;
  class_handle_t nc_handle = class_get_handle_by_id(NC_CLASS_ID);
  ...
  p_data->search_info.channel_info.frequency = p_param->inner.dvbs_lock_info.tp_rcv.freq;
  p_data->search_info.channel_info.param.dvbc.symbol_rate =
    p_param->inner.dvbs_lock_info.tp_rcv.sym;
  p_data->search_info.channel_info.param.dvbc.modulation =
    p_param->inner.dvbs_lock_info.tp_rcv.nim_modulate;  
...
static void lock_tuner_1(pb_handle_t *p_handle, play_param_t *p_param)
{
  OS_PRINTF("playback lock tuner\n");
  if(SYS_DVBS == p_param->lock_mode)
  {
    lock_dvbs_tuner_1(p_handle, p_param);
  }
  else if (SYS_DVBT == p_param->lock_mode)
  {
    lock_dvbt_tuner_1(p_handle, p_param);
  }
  else if(SYS_DVBC == p_param->lock_mode)
  {
    
    lock_dvbc_tuner_1(p_handle, p_param);
  }
  else
  {
    // Not support yet
...
static void do_play(pb_handle_t *p_handle, play_param_t *p_param)
{
  ...
  OS_PRINTF("Mark ap playback 1 do play:  vpid =%d , apid = %d\n", 
                    (u16)p_pg_info->v_pid, (u16)p_pg_info->a_pid);
...
static void do_stop(pb_handle_t *p_handle, stop_mode_t mode)
{
  ...
  ///TODO:should check with mark
#ifdef WARRIORS_STORM//minnan added
if(STOP_PLAY_NONE != mode)
{
  mode = STOP_PLAY_BLACK;
}
#endif
...
static BOOL check_same_tp(pb_handle_t *p_handle, play_param_t *p_param)
{
  nc_channel_info_t search_info = {0};
  nc_channel_info_t lock_info = {0};
  class_handle_t nc_handle = class_get_handle_by_id(NC_CLASS_ID);
  nc_get_tp(nc_handle, &lock_info);
  ...
  else if(p_param->lock_mode == SYS_DVBT)
  {
    return((p_param->inner.dvbs_lock_info.tp_rcv.freq == lock_info.channel_info.frequency));
  }
...
static void pb_on_nit_found(pb_handle_t *p_handle, nit_t *p_nit)
{
  priv_data_t *p_data = &p_handle->data;  
...
static void nit_request(pb_handle_t *p_handle)
{
  priv_data_t *p_data = &p_handle->data;
  dvb_request_t dvb_req = {0};
#if 0
...
static void pat_request(pb_handle_t *p_handle)
{
  priv_data_t *p_data = &p_handle->data;
  dvb_request_t dvb_req = {0};
...
static void sdt_request(pb_handle_t *p_handle)
{
  priv_data_t *p_data = &p_handle->data;
  dvb_request_t dvb_req = {0};
...
static void pmt_request(pb_handle_t *p_handle, u16 pid, u16 s_id)
{
  priv_data_t *p_data = &p_handle->data;
  dvb_request_t dvb_req = {0};
...
static void init(handle_t handle)
{
  ...
  p_handle->data.p_sub_ttx_data = init_sub_ttx_1();
  start_subt_dec_1(p_handle->data.p_sub_ttx_data);

  p_handle->data.p_audio_dev = dev_find_identifier(NULL, DEV_IDT_TYPE,
                                             SYS_DEV_TYPE_AUDIO);
  MT_ASSERT(p_handle->data.p_audio_dev != NULL);
  ...
  MT_ASSERT(p_handle->data.p_video_dev != NULL);
  p_handle->data.p_dmx_dev = dev_find_identifier(NULL, DEV_IDT_TYPE,
                                           SYS_DEV_TYPE_PTI);
  MT_ASSERT(p_handle->data.p_dmx_dev != NULL);
}
