...
static BOOL is_need_free_bat(scan_priv_t *p_scan_priv, bat_t *p_bat)
{
  ...
  for(i = 0; i < MAX_BOU_ID_NUM; i++)
  {
    ...
    {
      if(p_scan_priv->p_priv_data->p_rt_data->bat_sec_info[i].sec_num_first == p_bat->sec_number)
      {
        p_scan_priv->p_priv_data->p_rt_data->bat_sec_info[i].sec_num_repeat++;
      }

      if(p_scan_priv->p_priv_data->p_rt_data->bat_sec_info[i].sec_num_repeat == 3)
      {
        ret = TRUE;
      }
  ...
  for(i = 0; i < MAX_BOU_ID_NUM; i++)
  {
    ...
    else
    {
      if(p_scan_priv->p_priv_data->p_rt_data->bat_sec_info[i].bouquet_id == 0)
      {
        ...
        p_scan_priv->p_priv_data->p_rt_data->bat_sec_info[i].sec_num_first = p_bat->sec_number;
...
static scan_pg_info_t *scan_find_pg(scan_priv_t *p_scan_priv, u32 s_id)
{
  ...
  if(p_scan_data->p_rt_data->pg_offset[s_id] == 0xFF)
  {
    ...
    MT_ASSERT(p_scan_data->p_rt_data->cur_pg_offset <= SCAN_MAX_PG_PER_TP);
  }
  else
  {
...
static void scan_start(scan_priv_t *p_priv, os_msg_t *p_msg)
{
  ...
  MT_ASSERT(p_priv->p_priv_data->p_rt_data != NULL);
...
static void scan_on_pat_time_out(scan_priv_t *p_scan_priv, os_msg_t *p_msg)
{
  scan_data_t *p_scan_data = p_scan_priv->p_priv_data;
  scan_policy_t *p_policy = p_scan_priv->p_policy;
  dvb_request_t param = {0};
...
static void scan_on_nit_found(scan_priv_t *p_scan_priv, os_msg_t *p_msg)
{
  scan_data_t *p_scan_data = p_scan_priv->p_priv_data;
...
//    p_pg->video_pid,p_pg->audio[0].p_id,p_pg->pcr_pid,p_pg->s_id,p_pg->is_scrambled);
  ...
  switch(scramble_org)
  {
    ...
    //    OS_PRINTF("is found :%d\n",p_state->is_found);
        if(p_state->is_found)
        {
    //OS_PRINTF("set pg:%s scramble:%d\n",p_pg->name,p_state->is_scramble);
...
static void scan_on_sdt_found(scan_priv_t *p_scan_priv, os_msg_t *p_msg)
{
  ...
  for(i = 0; i < p_sdt->svc_count; i++)
  {
    ...
    p_pg->aud_out = p_sdt->svc_des[i].audio_out;
    ...
    memcpy(p_pg->name, p_sdt->svc_des[i].name, MAX_SVC_NAME_LEN);
...
app_t *construct_ap_scan(void)
{
  ...
  MT_ASSERT(p_scan_priv->p_priv_data != NULL);
