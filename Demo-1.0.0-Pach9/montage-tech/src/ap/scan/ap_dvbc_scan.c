...
static BOOL is_tp_exist(dvbc_scan_priv_t *p_priv, scan_tp_info_i_t *p_tp_info)
{
  ...
  switch(p_msg->content)
  {
    ...
    case DVB_NIT_FOUND:
      ret = on_nit(p_priv, p_msg);
      {
        ...
        evt.data1 = (u32)p_priv->total_tp_found_list;
  ...
  nc_channel_info_t *p_channel)
{
  dvbc_scan_priv_t *p_priv = (dvbc_scan_priv_t *)p_data;
/*
  if(p_priv->cur_idx_found_list != 0)
  { 
...
static u8 dvbc_get_bouquet_id_list(void *p_policy, u16 *p_list)
{
  ...
  scan_preset_data_t *p_data)
{
  ...
  //p_priv->tp_scan_policy.performance = dvbc_scan_performance;
