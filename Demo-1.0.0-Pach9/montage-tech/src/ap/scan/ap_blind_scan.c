...
static RET_CODE on_start(bl_scan_priv_t *p_priv, os_msg_t *p_msg)
{
  ...
  nc_set_blind_scan_mode(nc_handle, TRUE);
...
static BOOL pre_scan(bl_scan_priv_t *p_priv, u16 *p_find_cnt)
{
  ...
  if(p_priv->cur_fre_khz >= p_cur_cfg->end_fre_khz) //finish current
  {
    ...
    if(p_priv->cur_bs_times < p_priv->total_bs_times)
    {
      ...
      if(p_cur_cfg->unicable_param.use_uc)
      {
        ...
        nc_set_22k(nc_handle, 0);
        nc_set_polarization(nc_handle,NIM_PORLAR_VERTICAL);
      }
      else
      {
        nc_set_22k(nc_handle, p_cur_cfg->onoff22k);
        nc_set_polarization(nc_handle, p_cur_cfg->polarization);
      }
    }
    else
    {
  ...
  if(nc_is_lnb_check_enable(nc_handle))
  {
    dev_io_ctrl(p_priv->p_dev, NIM_REMOVE_PROTECT, 0);  
    //delay 30ms.
    mtos_task_delay_ms(30);
    //reset
    dev_io_ctrl(p_priv->p_dev, NIM_ENABLE_CHECK_PROTECT, 0);
  }
  ...
  nim_channel_scan(p_priv->p_dev, &scan_info);
  ...
  while(TRUE)
  {
    ...
    if(pre_scan(p_priv, &count))
    {
      if(count > 0)
      {
        ...
        MT_ASSERT(p_priv->channel_count <= MAX_TP_NUM_PER_SAT);
      }
      else
      {
...
/*static void blind_scan_performance(void *p_data,perf_check_point_t check,u32 context1,u32 context2)
{
  bl_scan_priv_t *p_priv = (bl_scan_priv_t *)p_data;
  ...
  scan_preset_data_t *p_data)
{
  ...
  //get device
  p_priv->p_dev = dev_find_identifier(NULL, DEV_IDT_TYPE,
    SYS_DEV_TYPE_NIM);
  ...
  p_priv->bl_scan_policy.get_scan_scramble_origin = NULL;
  ...
  MT_ASSERT(p_priv->total_preset_sat <= MAX_SAT_NUM_SUPPORTED);
