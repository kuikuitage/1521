...
static BOOL is_tp_exist(dtmb_scan_priv_t *p_priv, scan_tp_info_i_t *p_tp_info)
{
  ...
  nc_channel_info_t *p_channel)
{
  ...
  while(1)
  {
    if(p_priv->cur_idx_found_list < p_priv->total_tp_found_list)
    {
      ...
      p_channel->channel_info.param.dvbs.symbol_rate = p_tp_para->sym;
      p_channel->channel_info.param.dvbs.fec_inner = NIM_CR_AUTO;
