    ...
    switch(p_msg->content)
    {
      ...
      case DVB_DSMCC_DDM_FOUND:
        ...
        if(p_data->sm == OTA_SM_WAIT_DDM)
        {
          ...
          offset = ota_get_start_addr(p_data, mod_id, p_ddb->sec_num,
                        p_ddb->content_len, TRUE);
          p_start = p_data->ota_cfg.p_save_buf + offset;
          p_dst = p_start + p_ddb->content_len;
        
          memmove(p_dst, p_start, (p_data->ota_cfg.p_save_buf + p_data->ota_cfg.save_size - p_dst));
          
          memcpy(p_start, p_ddb->content, p_ddb->content_len);

          p_data->p_mod_info[mod_id].section_info[p_ddb->sec_num].section_len =
            p_ddb->content_len;              
          p_data->buf_cnt += p_ddb->content_len;            
          
          p_data->p_mod_info[mod_id].section_info[p_ddb->sec_num].is_received = TRUE;
          p_data->p_mod_info[mod_id].section_info[p_ddb->sec_num].start_addr = offset;
          
          p_data->received_len += p_ddb->content_len;
          ...
          case DVB_TABLE_ID_DSMCC_MSG:
            ...
            else if((p_msg->context & 0xFFFF) == DSMCC_MSG_DSI)
            {
              ota_request_dsi_single(
                p_data->p_dvb_svc, p_data->otai.download_data_pid);
            }
            break;
  ...
  else
  {
    switch(p_data->sm)
    {
      ...
      case OTA_SM_LOCKING:
        ...
        {
          case SYS_DVBS:
            nc_set_diseqc(nc_handle, &(p_data->otai.locks.disepc_rcv));

            nc_set_polarization(nc_handle, p_data->otai.locks.tp_rcv.polarity);

            nc_set_22k(nc_handle,p_data->otai.locks.tp_rcv.is_on22k);
          ...
          if(p_data->is_zipped)
          {
            ...
            if(p_data->dii_info.dii_type_info[p_data->type_idx].mod_type == MODULE_TYPE_RD) 
              { //Read Only area
                ...
                MT_ASSERT(p_ssdata_start != NULL);
                
                p_ssdata_start += 20;
                new_ver = p_data->otai.new_software_version;
                memcpy(p_ssdata_start, &new_ver, 4);
            } 
