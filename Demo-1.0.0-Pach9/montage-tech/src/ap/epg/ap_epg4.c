  ...
  {  
    switch(p_msg->content)
    {
      case DVB_ONE_SEC_FOUND:
        ...
        ret = epg_data_add(p_epg_db_handle, p_eit, &pf_new_ver);
      ...
      case EPG_CMD_START_SCAN: 
        ...
        log_perf(LOG_AP_EPG, PERF_ONE_PF_REQUEST, 0 , 0);
        log_perf(LOG_AP_EPG, PERF_ONE_TABLE_REQUEST, 0 , 0);
        break;
...
//state machine
  ...
  //      epg_data_get_db_report(p_epg_db_handle);
      ...
      case EPG_DB_NEW_SEC_FOUND:
        ...
        break;
      case EPG_DB_PF_READY:
        {
        if(0x4E == ((eit_t *)p_msg->para1)->table_id)
        {
          log_perf(LOG_AP_EPG, PERF_ONE_PF_FOUND, 0 , 0);
        }
        break;}
      case EPG_DB_ONE_TAB_READY:
        {
          if(0x50 == ((eit_t *)p_msg->para1)->table_id)
          log_perf(LOG_AP_EPG, PERF_ONE_TABLE_FOUND, 50 , 0);
        else if(0x51 == ((eit_t *)p_msg->para1)->table_id)
          log_perf(LOG_AP_EPG, PERF_ONE_TABLE_FOUND, 51 , 0);
        break;
        }
      default:
        //MT_ASSERT(0);   //need modify
      break;
    }  
}
