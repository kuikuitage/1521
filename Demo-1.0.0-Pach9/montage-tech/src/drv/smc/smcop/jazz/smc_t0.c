...
{    
  ...
  *pnum_write_ok = 0;
  ...
  if(pcommand[0] == 0x74 && pcommand[1] == 0xf5 && pcommand[4] == 0x02 && command_len == 5)
    desai_flag = 1;
    
    while ((command_len > 0) && (err == ERR_SMC_NO_ERR) && (NextWriteSize > 0))
    {  
        sc_tx(pcommand, NextWriteSize);
...
//         NextReadSize = num_to_read;
  ...
  SCT0_P("num_to_read is %d\n", num_to_read);
    ...
    {
          err = sc_rx(&presponse[TotalRead], num_to_read, pnum_read_ok);
    if (err != ERR_SMC_NO_ERR)
    {
      SCT0_E("\r\n");
    }
            ...
            u8 *presponse, u32 num_to_read, u32 *pnum_read_ok)
{
    ...
    /* Transmit command */
    sc_tx(pcommand, command_len);

  TotalRead = 0;
  NextReadSize = 8;    //Head Length
  
  err = sc_rx(&presponse[TotalRead],
                NextReadSize,
                pnum_read_ok);
