...
static ERRORCODE_T IO_T1ReadBlock(tscdevice *pSCDev, tt1block *pBlock)
{
  ...
  err = sc_rx(&pBlock->Header.NAD, 1, NULL);
  if (err != ERR_SMC_NO_ERR)
  {
    SCT1_E("\r\nRead NAD ERROR");
  ...
  /* Read PCB */
  err = sc_rx(&pBlock->Header.PCB, 1, NULL);
  if (err != ERR_SMC_NO_ERR)
  {
    SCT1_E("\r\nRead PCB ERROR");
  ...
  /* Read LEN */
  err = sc_rx(&pBlock->Header.LEN, 1, NULL);
  if (err != ERR_SMC_NO_ERR)
  {
    SCT1_E("\r\nRead LEN ERROR");
  ...
  /* Read any info field */
  err = sc_rx(pBlock->pBuffer, pBlock->Header.LEN, NULL);
  if (err != ERR_SMC_NO_ERR)
  {
  ...
  if (pSCDev->RC == 1)
  {
    err = sc_rx((u8 *)&pBlock->EDC.CRC, 2, NULL);
  }
  else
  {
    err = sc_rx(&pBlock->EDC.LRC, 1, NULL);
  }
  if (err != ERR_SMC_NO_ERR)
  {
