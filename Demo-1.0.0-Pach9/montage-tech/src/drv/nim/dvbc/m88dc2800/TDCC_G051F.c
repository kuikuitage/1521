...
u32 TDCC_G051F_GetSignalStrength ( MT_FE_DC2800_Device_Handle handle )
{
	...
	if ( ( dc2800_dmd_reg_read (handle->demod_dev_addr,0x43) &0x08 ) == 0x08 )
	{
		if (((dc2800_dmd_reg_read (handle->demod_dev_addr,0xE3) & 0xC0) == 0x00)
			&& ((dc2800_dmd_reg_read (handle->demod_dev_addr,0xE4) & 0xC0) == 0x00))
		{
			SignalStrength = 255 - (( dc2800_dmd_reg_read (handle->demod_dev_addr,0x55 ) >> 1 ) 
              + ( dc2800_dmd_reg_read(handle->demod_dev_addr,0x56 ) >> 1 ) );
		}
		else
		{
			SignalStrength = 255 - ( ( dc2800_dmd_reg_read (handle->demod_dev_addr,0x3B ) >> 1 ) 
            + ( dc2800_dmd_reg_read (handle->demod_dev_addr,0x3C ) >> 1 ) );
		}
	}
	else
	{
...
s32 TDCC_G051F_Set(MT_FE_DC2800_Device_Handle handle ,u32 freq_KHz, nim_modulation_t nqam)
{
	...
	dc2800_tn_seq_write(handle->tuner_settings.tuner_dev_addr,tdata, 5);
