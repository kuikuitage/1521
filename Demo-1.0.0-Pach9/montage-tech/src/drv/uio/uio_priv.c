...
BOOL uio_check_rpt_key(uio_type_t uio, u8 code, u8 ir_index)
{
...
extern void mcu_panel_attach(lld_uio_t *p_lld);
extern void gpio_panel_attach(lld_uio_t *p_lld);
extern void irda_attach(lld_uio_t *p_lld);
extern void gpio_panel_jazz_attach(lld_uio_t * p_lld);

RET_CODE uio_attach(char *p_name)
{
  ...
  if(0 == memcmp(p_name, "mcu", 3))
  {
    mcu_panel_attach(p_lld);
  }
  return SUCCESS;
}

RET_CODE uio_jazz_attach(char *p_name)
{
  uio_device_t *p_dev = NULL;
  device_base_t *p_base = NULL;
  lld_uio_t *p_lld = NULL;
