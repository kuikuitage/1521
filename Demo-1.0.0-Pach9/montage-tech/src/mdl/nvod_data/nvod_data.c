...
static u8 eit_info_svc_type(void *p_data, eit_t *p_eit_info, u32 *p_shift_svc_idx, u32 *p_ref_svc_idx)
{
  ...
  for(loopi = 0; loopi < p_nvod->nvod_svc_cnt; loopi++)
  {
    ...
    p_ref_svc = (nvod_reference_svc_t *)&p_nvod->svc[loopi];
...
static u32 add_shift_svc_evt(void *p_data, eit_t *p_eit_info, u32 shift_svc_idx, u32 ref_svc_idx)
{
  nvod_db_t *p_nvod = (nvod_db_t *)p_data;
  ...
  MT_ASSERT(ref_svc_idx < p_nvod->nvod_svc_cnt);

  p_svc = (nvod_reference_svc_t *)(&p_nvod->svc[ref_svc_idx]);
  ...
  while (evt_idx < p_eit_info->tot_evt_num)
  {
    ...
    {
      OS_PRINTF("add_shift_svc_evt ##debug:evt FULL %d !\n", evt_idx);
      break;
    }
    ...
    if (p_eit_info->table_id >= DVB_TABLE_ID_EIT_SCH_ACTUAL)
    {
      ...
      {
        evt_idx ++;
        printf_time(&p_evt_desc->st_tm,"add_shift_svc_evt ##debug:overdue!");
        continue;
      }
    }
    ...
    {
      evt_idx ++;
      printf_time(&p_evt_desc->st_tm,"add_shift_svc_evt ##debug:no des!");
      continue;
    }
    ...
    p_new->time_shifted_svc_idx = shift_svc_idx;

    p_svc_evt = p_nvod->p_ref_evt_head;
    if(p_svc_evt == NULL)
    {
      printf_time(&p_evt_desc->st_tm,"##debug: no head !!!!");
    }
    while (p_svc_evt)
    {
      if (p_svc_evt->event_id == p_evt_desc->time_shifted_evt_des.evt_id
        && p_svc_evt->ref_svc_idx == ref_svc_idx)
      {
        break;
      }
    ...
    {
      printf_time(&p_evt_desc->st_tm,"##debug: no next !!!!");
    ...
    if (!add_new)
    {
      //printf_time(&p_evt_desc->st_tm,"##debug: can't add !!!!");
    ...
    else
    {
      ...
      printf_time(&p_evt_desc->st_tm,"##debug: add suc !!!!");
      //OS_PRINTF("event id 0x%x, svc id %d\n", p_new->event_id, p_svc_evt->ref_svc_idx);
      printf_time(&p_new->start_time, "s");
      printf_time(&p_new->drt_time, "e");
...
static u32 add_ref_svc_evt(void *p_data, eit_t *p_eit_info, u32 ref_svc_idx)
{
  nvod_db_t *p_nvod = (nvod_db_t *)p_data;
  nvod_reference_svc_t *p_svc = NULL;
  sht_evt_desc_t *p_evt_desc = NULL;
  ...
  while (evt_idx < p_eit_info->tot_evt_num)
  {
    ...
    if (check_ref_svc_evt_exist(p_data, p_evt_desc->evt_id, ref_svc_idx))
    {
      evt_idx ++;
      continue;
    }
    ...
    p_new_evt->ref_svc_idx = ref_svc_idx;
    ...
    else
    {
      ...
      OS_PRINTF("@@## short evt : %d\n", p_new_evt->sht_txt_len);
    }

    if(p_evt_desc->name_len > 0)
    {
      dvb_to_unicode(p_evt_desc->evt_nm, p_evt_desc->name_len,
    ...
    dvb_to_unicode(p_evt_desc->lang_code, LANGUAGE_CODE_LEN , 
...
static void nvod_mem_alloc(void *p_data)
{
  ...
  alloc_size = 2*(MAX_SHIFT_EVT * sizeof(time_shifted_svc_evt_t) + non_fix_size);
  p_buffer = mtos_malloc(alloc_size);

  OS_PRINTF("~~~~~~~~~~~~~~~~~~~~~~~~~~nvod_mem_alloc[0x%x ~ 0x%x]\n", (u32)p_buffer,
    (u32)p_buffer + alloc_size);
...
{
  nvod_db_t *p_nvod = (nvod_db_t *)p_data;
  u32 shift_svc_idx = 0, ref_svc_idx = 0;
  ...
  switch (eit_info_svc_type(p_data, p_eit_info, &shift_svc_idx, &ref_svc_idx))
  {
  case DVB_NVOD_REF_SVC:
    if (p_eit_info->table_id != DVB_TABLE_ID_EIT_ACTUAL
      && p_eit_info->table_id != DVB_TABLE_ID_EIT_OTHER)
    ...
    if (add_ref_svc_evt(p_data, p_eit_info, ref_svc_idx))
    {
  ...
  case DVB_NVOD_TIME_SHIFTED_SVC:
    if (p_nvod->svc[ref_svc_idx].reference_evt_cnt == 0)
    {
      return NVOD_DB_NORM;
    }
    
    if (add_shift_svc_evt(p_data, p_eit_info, shift_svc_idx, ref_svc_idx))
    {
  ...
  default:
    OS_PRINTF("\n##debug: nvod_data_add_evt [%x][%d, %d, %d]\n",p_eit_info->table_id,
      p_eit_info->org_nw_id, p_eit_info->stream_id, p_eit_info->svc_id);
    break;
  }
