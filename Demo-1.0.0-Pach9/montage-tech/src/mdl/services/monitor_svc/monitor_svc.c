...
/*!
  ...
  */
#define DATA_BUFFER_SIZE          (4*(KBYTES) + 188)
...
static BOOL parse_pat(class_handle_t handle, u8 *p_buf)
{
  u8 *p_ptr = NULL;
  u16 section_len = 0;
  u16 prog_no = 0;
...
static void monitor_check_service_id(handle_t handle)
{
  ...
  else
  {
    ...
    if (dmx_handle != 0xFFFF)
    {
      ...
      else if ((cur_ticks - p_priv->table_info[M_PAT_TABLE].start_ticks) * 10 > 
              p_priv->table_info[M_PAT_TABLE].timeout)
      {
        m_free_filter(handle, M_PAT_TABLE);
        OS_PRINTF("monitor timeout %d\n", M_PAT_TABLE);
        p_priv->table_info[M_PAT_TABLE].start_ticks = cur_ticks;
