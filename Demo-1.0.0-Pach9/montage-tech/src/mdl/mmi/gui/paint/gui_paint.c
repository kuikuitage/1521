                              ...
                              bitmap_t *p_bmp)
{
  ...
  p_bmp->bpp = gdi_get_bpp((u8)p_hdr_bmp->pixel_type);
  p_bmp->p_bits = p_bits;
...
//  rsc_lock_data(RSC_TYPE_PALETTE);
  ...
  gdi_set_palette(0, color_num, (color_t *)p_bits);
...
u16 gui_draw_char(hdc_t hdc,
                  ...
                  u16 top)
{
  u8 *p_font_data = NULL, *p_pal_data = NULL;
  rsc_font_t hdr_font;
  rsc_palette_t hdr_pal;
  rect_t r_char;
  bitmap_t char_info;
  u8 interval = 0, format = 0, bpp = 0;
  BOOL ret_boo = FALSE;
  ...
  if(!rsc_get_font_by_char(p_info->rsc_handle, p_fstyle, char_code, &hdr_font, &p_font_data))
  {
    return left;
  }

  format = gdi_get_screen_pixel_type();
  bpp = gdi_get_bpp(format);
  
  interval = rsc_get_font_interval(p_info->rsc_handle, hdr_font.head.id);
  ret_boo = rsc_get_char(p_info->rsc_handle,
    &hdr_font, p_font_data, p_fstyle, char_code, &info, bpp);
  if(!ret_boo)
  {
    PAINT_PRINTF("gui_draw_char(%d)\n", __LINE__);
    return left;
  }
...
// TODO: only process 8bit mode, it should be compitable with bitmap itself
  ...
  switch(hdr_font.type)
  {
    case RSC_FONT_TYPE_ARRAY:
      ...
      if(bpp >= 16)
      {
        char_info.pitch = ((char_info.width * (bpp / 8)) + 7) / 8 * 8;
      }
      else
      {
        char_info.pitch = (char_info.width * char_info.bpp + 7) >> 3;
      }
      break;
    ...
    case RSC_FONT_TYPE_VECTOR:
      ...
      if(info.is_alpha_spt)
      {
        ...
        char_info.pitch = (((char_info.width * char_info.bpp) >> 3) + 7) / 8 * 8;
      ...
      else
      {
        ...
        char_info.p_strok_alpha = NULL;
        char_info.p_strok_char = NULL;
        char_info.pitch = (((info.alpha_pitch * char_info.bpp) >>3) + 7) / 8 * 8;
      }
      break;
  ...
  *p_height = 0;
  ...
  while(1)
  {
    ...
    *p_height = MAX(*p_height, char_height);
    ...
    i++;
    if((char_code >= 0x4e00) && (char_code <= 0x9fbf))/////Chinese   code
    {
      break;
    }
  }
                              ...
                              u32 draw_style)
{
  ...
  while(i < str_len)
  {
    ...
    {      
      ...
      {
        if(is_start_with_word || (draw_style == STRDRAW_BREAK_WORD))
        {
          if(remn_width >= char_width)
          {
            p_info->p_lines[lines]++;
                    ...
                    u32 draw_style)
{
  ...
  if((is_arabic_uni_code(p_str[0]) == TRUE) && (style & STL_LEFT))
                           ...
                           u32 draw_style)
{
  ...
  while(i < str_len)
  {
    ...
    else
    {
      ...
      {
        chars = gui_get_word(p_fstyle, p_str + i, &word_width, &word_height);
...
hdc_t gui_begin_paint(control_t *p_ctrl, hdc_t parent_dc)
{
  ...
  gdi_get_cliprgn_boundrect(&p_ctrl->invrgn_info.crgn, &vrc);
  ctrl_client2screen(p_ctrl, &vrc);
//  offset_rect(&vrc, src.left, src.top);

  hdc = gdi_get_dc(&src, &vrc, cont_get_gcrgn_info(p_parent), parent_dc);
...
#endif
/*
   if(parent_dc == HDC_INVALID)   // need to clean dc with color key
   {                               //only clean the area of effected region
    gdi_clean_dc(hdc, FALSE);
   }
  */
  return hdc;
}
...
void gui_paint_init(paint_param_t *p_param)
{
  ...
  //p_info->p_line_str_code =
  //  (u16 *)mmi_alloc_buf((p_param->max_str_len + 1) * sizeof(u16));
  //MT_ASSERT(p_info->p_line_str_code != NULL);

  //p_info->p_line_str_pos = (u16 *)mmi_alloc_buf(p_param->max_str_lines * sizeof(u16));
  //MT_ASSERT(p_info->p_line_str_pos != NULL);
...
void gui_paint_release(void)
{
  ...
  //if(p_info->p_line_str_code != NULL)
  //{
  //  mmi_free_buf(p_info->p_line_str_code);
  //  p_info->p_line_str_code = NULL;
  //}
  ...
  //if(p_info->p_line_str_pos != NULL)
  //{
  //  mmi_free_buf(p_info->p_line_str_pos);
  //  p_info->p_line_str_pos = NULL;
  //}
