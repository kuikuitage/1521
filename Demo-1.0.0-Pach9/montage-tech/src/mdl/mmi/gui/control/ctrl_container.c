...
RET_CODE cont_register_class(u16 max_cnt)
{
  ...
  register_info.data_size = sizeof(ctrl_cont_data_t);
  ...
  if(ctrl_register_ctrl_class((u8 *)CTRL_CONT, &register_info) != SUCCESS)
  {
...
void cont_move_focus(control_t *p_ctrl, u16 msg)
{
  ...
  if(p_active != NULL)
  {
    ...
    while((next_id) && (next_id != p_active->id))
    {
      ...
      if(OBJ_ATTR_ACTIVE == p_next_ctrl->attr)
      {
        break;
      }
      else
...
{
  ctrl_cont_data_t *p_data = NULL;

  MT_ASSERT(p_ctrl != NULL);

  p_data = (ctrl_cont_data_t *)p_ctrl->priv_data;

  return p_data->p_clip_info;
}
