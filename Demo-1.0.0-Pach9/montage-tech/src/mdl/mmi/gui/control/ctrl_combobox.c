...
{
  ctrl_cbox_data_t *p_data = (ctrl_cbox_data_t *)p_ctrl->priv_data;
  ...
  return (cbox_static_data_t *)&p_data->union_data.static_data;
}
...
static cbox_num_data_t *_cbox_get_num_data(control_t *p_ctrl)
{
  ...
  return (cbox_num_data_t *)&p_data->union_data.num_data;
}
...
static cbox_dync_data_t *_cbox_get_dync_data(control_t *p_ctrl)
{
  ...
  return (cbox_dync_data_t *)&p_data->union_data.dync_data;
}
...
static cbox_comm_data_t *_cbox_get_common_data(control_t *p_ctrl)
{
  ...
  {
    return (cbox_comm_data_t *)&p_data->union_data.static_data;
  }
...
static void _cbox_update_content(control_t *p_ctrl)
{
  ...
  switch(p_ctrl->priv_attr & CBOX_WORKMODE_MASK)
  {
    case CBOX_WORKMODE_STATIC:
    case CBOX_WORKMODE_DYNAMIC:
      p_data->str_char = cbox_get_content(p_ctrl, p_comm_data->focus);
      break;

    case CBOX_WORKMODE_NUMBER:
      p_data->str_char = p_data->union_data.num_data.curn;
      break;
...
static u8 _cbox_get_fstyle(u8 attr, ctrl_cbox_data_t *p_data)
{
  u8 font = 0;
...
static void _cbox_draw_content(control_t *p_ctrl, hdc_t hdc)
{
  ...
  font = _cbox_get_fstyle(attr, p_data);

  ctrl_get_client_rect(p_ctrl, &temp_rect);
  switch(p_ctrl->priv_attr & CBOX_WORKMODE_MASK)
  {
    case CBOX_WORKMODE_STATIC:
      switch(p_ctrl->priv_attr & CBOX_ITEM_STRTYPE_MASK)
      {
        case CBOX_ITEM_STRTYPE_STRID:
          gui_draw_strid(hdc, &temp_rect, p_ctrl->priv_attr,
                         ...
                         0, (u16)p_data->str_char, font, STRDRAW_NORMAL);
          break;
    ...
    case CBOX_WORKMODE_DYNAMIC:
      gui_draw_unistr(hdc, &temp_rect, p_ctrl->priv_attr,
                      p_data->str_left, p_data->str_top, 0,
                      (u16 *)(p_data->str_char), font, STRDRAW_NORMAL);
      break;
    default:
      MT_ASSERT(0);
  }
...
static control_t *_cbox_get_droplist(control_t *p_cbox)
{
  ctrl_cbox_data_t *p_data = NULL;

  MT_ASSERT(p_cbox != NULL);

  p_data = (ctrl_cbox_data_t *)p_cbox->priv_data;
...
s32 cbox_register_class(u16 max_cnt)
{
  ...
  register_info.data_size = sizeof(ctrl_cbox_data_t);
  ...
  if(ctrl_register_ctrl_class((u8 *)CTRL_CBOX, &register_info) != SUCCESS)
  {
                                ...
                                u8 interval)
{
  ...
  if(p_list != NULL)
  {
    list_set_mid_rect(p_list, left, top, width, height, interval);
  }
}
...
void cbox_attach_droplist(control_t *p_ctrl, control_t *p_droplist)
{
  ...
  p_data->p_droplist = p_droplist;
  if(p_data->p_droplist != NULL)
  {
    page = list_get_page(p_data->p_droplist);
    list_set_count(p_data->p_droplist, _cbox_calc_droplist_count(p_ctrl), page);
    ctrl_set_attr(p_data->p_droplist, OBJ_ATTR_HIDDEN);
  }
}
                                     ...
                                     s16 bottom)
{
  ...
  if(p_sbar != NULL)
  {
    sbar_set_mid_rect(p_sbar, left, top, right, bottom);
  }
}
...
BOOL cbox_droplist_is_opened(control_t *p_ctrl)
{
  ...
  if (p_data->p_droplist != NULL)
  {
    return p_data->is_droplist_opened;
  }
...
BOOL cbox_close_droplist(control_t *p_ctrl, BOOL is_update)
{
  ...
  ctrl_erase_ctrl(p_list);

  if(p_sbar != NULL)
  {
    ctrl_set_attr(p_sbar, OBJ_ATTR_HIDDEN);
    ctrl_erase_ctrl(p_sbar);
  }
  ...
  ctrl_add_rect_to_invrgn(p_ctrl, NULL);
  p_data->is_droplist_opened = FALSE;
...
static RET_CODE on_cbox_select(control_t *p_ctrl, u16 msg, u32 para1, u32 para2)
{
  ...
  if(!(p_ctrl->priv_attr & CBOX_STATUS_LIST))
  {
    ...
    {
      ctrl_process_msg(p_data->p_droplist, MSG_PAINT, FALSE, 0);
    }
  }
  else
  {
