...
static u16 _sbar_get_rstyle_idx(control_t *p_ctrl)
{
  ctrl_sbar_data_t *p_data = NULL;
  u16 rstyle = 0;

  MT_ASSERT(p_ctrl != NULL);

  p_data = (ctrl_sbar_data_t *)p_ctrl->priv_data;

  if(p_data->pressed != 0)
  {
    rstyle = p_data->h_mid_rstyle;
  }
  else if(p_ctrl->attr == OBJ_ATTR_INACTIVE)
  {
    rstyle = p_data->g_mid_rstyle;
  }
  else
  {
    rstyle = p_data->n_mid_rstyle;
  }
  return rstyle;
}
...
static u16 _sbar_calc_grid(control_t *p_ctrl)
{
  ...
  u16 rstyle_id = 0, bmp_w = 0, bmp_h = 0;
  ...
  if(p_ctrl->priv_attr & SBAR_AUTOSIZE_MASK)
  {
    mid = p_ctrl->priv_attr & SBAR_DIRECTION_MASK ?
          RECTW(p_data->mid_rect) : RECTH(p_data->mid_rect);
    grid = p_data->page * mid / p_data->total;
...
static BOOL _sbar_get_thumb_rect(control_t *p_ctrl, rect_t *p_thumb_rect)
{
  ...
  if((p_data->page >= p_data->total) || (p_data->page == 0))
  {
    memcpy(p_thumb_rect, &p_data->mid_rect, sizeof(rect_t));
    return TRUE;
  }
  else   // normal case
  {
    ...
    last_page = (BOOL)(!(p_data->cur_pos + p_data->page < p_data->total));
    ...
    if(p_ctrl->priv_attr & SBAR_DIRECTION_MASK) //get thumb rect
    {
      ...
      if(last_page)
      {
        p_thumb_rect->left = p_data->mid_rect.right - gird;
      }
      else
      {
        p_thumb_rect->left = p_data->mid_rect.left + p_data->cur_pos *
                             (p_data->mid_rect.right -
                              p_data->mid_rect.left -
                              gird) / (p_data->total - p_data->page);
      }
      p_thumb_rect->right = p_thumb_rect->left + gird;
      p_thumb_rect->top = p_data->mid_rect.top;
      p_thumb_rect->bottom = p_data->mid_rect.bottom;
    }
    else
    {
      if(last_page)
      {
        p_thumb_rect->top = p_data->mid_rect.bottom - gird;
      }
      else
      {
        p_thumb_rect->top = p_data->mid_rect.top + p_data->cur_pos *
                            (p_data->mid_rect.bottom - p_data->mid_rect.top -
                             gird) / (p_data->total - p_data->page);
      }
      p_thumb_rect->bottom = p_thumb_rect->top + gird;
      p_thumb_rect->left = p_data->mid_rect.left;
      p_thumb_rect->right = p_data->mid_rect.right;
    }
...
RET_CODE sbar_register_class(u16 max_cnt)
{
  ...
  register_info.data_size = sizeof(ctrl_sbar_data_t);
  ...
  if(ctrl_register_ctrl_class((u8 *)CTRL_SBAR, &register_info) != SUCCESS)
  {
...
void sbar_set_mid_rect(control_t *p_ctrl,
                       s16 left,
                       s16 top,
                       s16 right,
                       s16 bottom)
...
BOOL sbar_increase(control_t *p_ctrl)
{
  ...
  p_data = (ctrl_sbar_data_t *)p_ctrl->priv_data;
  if(p_data->cur_pos < p_data->total - 1)
  {
    p_data->cur_pos++;
