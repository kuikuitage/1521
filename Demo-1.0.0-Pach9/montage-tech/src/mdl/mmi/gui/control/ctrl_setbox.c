...
{
  ctrl_sbox_data_t *p_data = (ctrl_sbox_data_t *)p_ctrl->priv_data;
...
static void _sbox_get_style_by_attr(u8 attr, control_t *p_ctrl,
  u16 index, u8 *p_rstyle_idx, u8 *p_fstyle_idx)
{
  ctrl_sbox_data_t *p_data = NULL;
  ...
  switch(attr)
  {
    case OBJ_ATTR_HL:
      ...
      {
        *p_rstyle_idx = p_data->h_mid_rstyle;
        *p_fstyle_idx = p_data->f_fstyle;
      }
      else
      {
        *p_rstyle_idx = p_data->n_mid_rstyle;
        *p_fstyle_idx = p_data->n_fstyle;
      }
      break;
    case OBJ_ATTR_INACTIVE:
      // gray
      *p_rstyle_idx = p_data->g_mid_rstyle;
...
static void _sbox_free_ctrl(control_t *p_ctrl)
{
  ...
  p_data = (ctrl_sbox_data_t *)p_ctrl->priv_data;
  if(p_data->p_left != NULL)
  {
    mmi_free_buf(p_data->p_left);
    p_data->p_left = 0;
  }
  ...
  if(p_data->p_width != NULL)
  {
    mmi_free_buf(p_data->p_width);
    p_data->p_width = 0;
  }
  if(p_data->p_item_context != NULL)
  {
    mmi_free_buf(p_data->p_item_context);
    p_data->p_item_context = 0;
  }
}
...
RET_CODE sbox_register_class(u16 max_cnt)
{
  ...
  register_info.data_size = sizeof(ctrl_sbox_data_t);
  ...
  if(ctrl_register_ctrl_class((u8 *)CTRL_SBOX, &register_info) != SUCCESS)
  {
...
void sbox_set_mid_rstyle(control_t *p_ctrl,
  u8 n_mid_rstyle, u8 h_mid_rstyle, u8 g_mid_rstyle)
{
  ctrl_sbox_data_t *p_data = NULL;
  ...
  p_data->n_mid_rstyle = n_mid_rstyle;
  p_data->h_mid_rstyle = h_mid_rstyle;
  p_data->g_mid_rstyle = g_mid_rstyle;
}
...
BOOL sbox_set_count(control_t *p_ctrl, u8 total)
{
  ...
  //left
  p_data->p_left = mmi_realloc_buf(p_data->p_left, sizeof(u16) * total);
  MT_ASSERT(p_data->p_left != NULL);

  if(p_data->p_left == NULL)
  {
    return FALSE;
  }
  memset(p_data->p_left, 0, sizeof(u16) * total);

  //width
  p_data->p_width = mmi_realloc_buf(p_data->p_width, sizeof(u16) * total);
  MT_ASSERT(p_data->p_width != NULL);

  if(p_data->p_width == NULL)
  {
    return FALSE;
  }
  memset(p_data->p_width , 0, sizeof(u16) * total);

  //context
  p_data->p_item_context = mmi_realloc_buf(p_data->p_item_context, sizeof(u32) * total);
  MT_ASSERT(p_data->p_item_context != NULL);

  if(p_data->p_item_context == NULL)
  {
    return FALSE;
  }
  memset(p_data->p_item_context , 0, sizeof(u32) * total);
...
void sbox_set_string_align_type(control_t *p_ctrl, u32 style)
{
  MT_ASSERT(p_ctrl != NULL);
...
u32 sbox_get_item_context(control_t *p_ctrl, u8 item_idx)
{
  ...
  if(p_data->p_item_context != NULL)
  {
    content = *(p_data->p_item_context + item_idx);
  }
...
void sbox_set_item_context(control_t *p_ctrl, u8 item_idx, u32 context)
{
  ...
  p_data = (ctrl_sbox_data_t *)p_ctrl->priv_data;
  *(p_data->p_item_context + item_idx) = context;
}

void sbox_get_string_offset(control_t *p_ctrl, u8 *p_left, u8 *p_top)
{
  ctrl_sbox_data_t *p_data = NULL;
  ...
  u8 item_idx, u16 *p_left, u16 *p_width)
{
  ...
  *p_left = *(p_data->p_left + item_idx);
  *p_width = *(p_data->p_width + item_idx);
}
  ...
  u8 item_idx, u16 left, u16 width)
{
  ...
  p_data = (ctrl_sbox_data_t *)p_ctrl->priv_data;
  *(p_data->p_left + item_idx) = left;
  *(p_data->p_width + item_idx) = width;
}

void sbox_set_fstyle(control_t *p_ctrl,
  u8 f_fstyle, u8 n_fstyle, u8 g_fstyle)
{
  ctrl_sbox_data_t *p_data = NULL;
  ...
  u16 msg, u32 para1, u32 para2)
{
  ...
  MT_ASSERT(p_data->focus <= p_data->total);
  ...
  if(p_data->focus >= p_data->total)
  {
    p_data->focus = 0;
  }

  new_focus = p_data->focus;
