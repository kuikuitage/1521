...
static void _op_clip_and_recalc_invrgns(znode_t *p_first, znode_t *p_node)
{
  ...
  while(p_affected != NULL)
  {
    ...
    if(ctrl_get_attr(p_root) != OBJ_ATTR_HIDDEN)
    {
      /* initalize, again */
...
static BOOL _remove_mainwin_from_zorder(mainwin_t *p_win)
{
  ...
  if(ctrl_get_attr(p_win->p_root) == OBJ_ATTR_HIDDEN)
  {
    //do nothing
  }
  else
  {
    ctrl_set_attr(p_win->p_root, OBJ_ATTR_HIDDEN);
...
static BOOL _get_parent_mainwin(u8 root_id,
                                u8 parent_root_id,
                                mainwin_t **p_parent)
{
  fw_main_t *p_info = NULL;
...
control_t *fw_find_root_by_id(u8 root_id)
{
  znode_t *p_node = NULL;
  fw_main_t *p_info = NULL;
...
control_t *fw_create_mainwin(u8 root_id,
                             u16 x,
                             u16 y,
                             u16 w,
                             u16 h,
                             ...
                             u8 style)
{
  ...
  // create root
  if((p_root =
        ctrl_create_ctrl((u8 *)CTRL_CONT, root_id, x, y, w, h, NULL, 0)) == NULL)
  {
...
RET_CODE fw_move_mainwin(u8 root_id, u16 x, u16 y)
{
  fw_main_t *p_info = NULL;
  mainwin_t *p_win = NULL;
  znode_t *p_node = NULL;
  rect_t frame = {0};
  u8 old_attr = OBJ_ATTR_ACTIVE;
  ...
  p_node = zlist_find_node_by_mainwin(p_win->style & STL_EX_TOPMOST ?
  ...
  old_attr = ctrl_get_attr(p_win->p_root);

  ctrl_set_attr(p_win->p_root, OBJ_ATTR_HIDDEN);

  _recalc_gcrgns_on_remove(p_node);

  ctrl_set_attr(p_win->p_root, old_attr);
...
control_t *fw_attach_root(control_t *p_root, u8 parent_root_id)
{
  u8 root_id = 0;
  mainwin_t *p_parent = NULL;
...
#if 1 /* close mainwin from top to bottom */
...
#else  /* close mainwin from bottom to top */
  p_node = zlist_get_the_last(&p_info->infor.zl_normal);
  while(p_node != NULL)
  {
    if((p_win = p_node->p_host) != NULL)
    {
      p_root = p_win->p_root;
      /* because it will change the chain of zorder, so save someting at first
        */
      p_node = p_node->p_prev;

      if(p_root != fw_get_root())
      {
        if(ctrl_process_msg(p_root, MSG_SAVE, 0, 0) == ERR_FAILURE)
        {
          /* something is failed */
          FW_PRINTF("ROOT[%d] break fw_destroy_all_mainwin()......\n",
                    ctrl_get_ctrl_id(p_root));
...
mainwin_t *fw_find_mainwin_by_id(u8 root_id)
{
  znode_t *p_node = NULL;
  fw_main_t *p_info = NULL;
...
BOOL fw_destroy_mainwin_by_id(u8 root_id)
{
