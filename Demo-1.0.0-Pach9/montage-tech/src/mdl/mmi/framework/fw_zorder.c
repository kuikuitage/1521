...
static void _op_clip_list_by_node(znode_t *p_first, znode_t *p_node)
{
  ...
  {
    if(ctrl_get_attr(p_affected->p_host->p_root) != OBJ_ATTR_HIDDEN)
    {
...
void znode_set_style(znode_t *p_node, u8 new_style)
{
  mainwin_t *p_win = p_node->p_host;
  u32 old_style = ctrl_get_attr(p_win->p_root);
  ...
  while(p_node != NULL)
  {
    if(p_node->p_host != NULL)
    {
      ...
      if((ctrl_get_attr(p_root) != OBJ_ATTR_INACTIVE)
        && (ctrl_get_attr(p_root) != OBJ_ATTR_HIDDEN))
      {
        return p_node;
      }
    }
...
znode_t *zlist_find_node_by_id(zlist_t *p_list, u8 root_id)
{
  znode_t *p_node = NULL;
  mainwin_t *p_win = NULL;
  control_t *p_root = NULL;
