...
#if 0
static void dm_set_fix_id_v3(void *p_data, u8 block_id, u16 fix_num)
{
  s32               block_idx = 0;
  dm_global_info_t *p_dm_info = NULL;
  dm_var_t         *p_dm_var  = NULL;
  ...
  find_block_index(p_dm_var,block_id);

  if((block_idx == ERR_FAILURE)
    || (BLOCK_TYPE_RO == p_dm_info->p_blk_mgr[block_idx].type)
    || (p_dm_info->p_blk_mgr[block_idx].node_num <= fix_num))
  {
    return;
  }

  //use the max
  if(fix_num > p_dm_info->p_blk_mgr[block_idx].fix_num)
  {
     p_dm_info->p_blk_mgr[block_idx].fix_num = fix_num;
  }
}
#endif
...
static dm_ret_t dm_flush_cache_v3(void *p_data, u8 block_id, u16 node_id)
{
  ...
  else
  {
    if(CACHE_ASYNC == p_node->cache_state)
    {
      ...
      data_read(p_dm_var, node_addr,
          p_node_buf, p_dm_info->max_node_length, p_block->type);

      cmp_rslt = memcmp(p_node_buf + NODE_HEADER_SIZE,
...
//  p_dm_proc->set_fix_id       = dm_set_fix_id_v3;
  ...
  if(p_para->open_monitor)
  {
    ...
    mtos_task_create((u8 *)"dm_v3", task_process, p_dm_proc->p_priv_data,
