...
void avc_switch_video_mode_1(void *p_data, avc_video_mode_1_t new_std)
{
  ...
  if(b_switch == TRUE)
  {
    ...
    disp_set_tv_sys(p_avc_data->p_disp_dev, disp_chan, video_mode);
...
BOOL avc_video_pause_1(void *p_data)
{
  RET_CODE ret = SUCCESS;
  
  avc_var_t *p_avc_data = (avc_var_t *)p_data;
  MT_ASSERT(p_avc_data != NULL);
  ret = vdec_pause(p_avc_data->p_video_dev);
