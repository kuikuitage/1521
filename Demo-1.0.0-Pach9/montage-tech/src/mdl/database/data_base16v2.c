...
u8 db_create_table_16v2(void *p_data, u8 block_id, u16 elements_num)
{
  ...
  p_tab_info = p_db_var->table_info;
  MT_ASSERT(p_tab_info != NULL);
  MT_ASSERT(p_dm_handle != NULL);

  node_id_used = p_db_var->node_id_used;
  clean_block = p_db_var->clean_block;

  //OS_PRINTF("********db_create_table_16v2\n");
  for(table_id = 0; table_id < MAX_TABLE_NUM; table_id++)
  {
    if(p_tab_info[table_id].block_id == INVALID_ID)
    {
      p_tab_info[table_id].block_id = block_id;
      p_tab_info[table_id].max_element_number = elements_num;

      p_view_info[table_id].ni_start = node_id_used;
      if(p_view_info[table_id].p_mem_backup == NULL)
      {
        p_view_info[table_id].p_mem_backup = mtos_malloc(elements_num * sizeof(item_type_t));
        MT_ASSERT(p_view_info[table_id].p_mem_backup != NULL);
        p_view_info[table_id].p_sync_st = mtos_malloc(sizeof(BOOL));
        MT_ASSERT(p_view_info[table_id].p_sync_st != NULL);
        p_view_info[table_id].item_count = 0;
      }

      //OS_PRINTF("********db_create_table_16v2 tab=%d ni=%d\n",
        //table_id,p_view_info[table_id].ni_start);
      node_id_used += (elements_num + VIEW_BUNDLE_SIZE - 1) / VIEW_BUNDLE_SIZE;
      MT_ASSERT(node_id_used < VIEW_NI_NUM);

      if(block_id != clean_block)
      {
        //Set
        //OS_PRINTF("********dm_reset_iw_block\n");
        dm_reset_iw_block(p_dm_handle, block_id, FALSE);
        clean_block = block_id;
      }
      p_db_var->node_id_used = node_id_used;
...
u8 db_create_view_16v2(void *p_data, u8 block_id, u8 table_id)
{
  ...
  MT_ASSERT(p_tab_info != NULL);
  MT_ASSERT(p_dm_handle != NULL);
  MT_ASSERT((table_id < MAX_TABLE_NUM)
           && (p_view_info[table_id].block_id == INVALID_ID));

  p_view_info[table_id].block_id = block_id;

  //set fix id
  //OS_PRINTF("********db_create_view_16v2 \n");
  if(block_id == p_tab_info[table_id].block_id)
  {
    u16 cur_view_bundle =
      (p_tab_info[table_id].max_element_number +
       VIEW_BUNDLE_SIZE - 1) / VIEW_BUNDLE_SIZE;
    dm_set_fix_id(p_dm_handle, block_id,
                  p_view_info[table_id].ni_start + cur_view_bundle);
  ...
  //OS_PRINTF("********db_create_view_16v2 macro = %d tab=%d ni=%d\n",
      //VIEW_NI_NUM,table_id,p_view_info[table_id].ni_start);
  //load view node , from discrete block node's to list
  for(i = p_view_info[table_id].ni_start; i < VIEW_NI_NUM; i++)
  {
    //load finish
                              ...
                              u16 len)
{
  ...
  if(db_get_element_16v2(p_data, table_id, element_id, &temp, 0, 1) == TRUE)
  {
                     ...
                     &element_id, p_buffer, len) == DM_SUC)
    {
      ...
      //dm_flush_cache(g_table_info[table_id].block_id, element_id);
      return DB_SUC;
    }
    OS_PRINTF("FAILED TO write\n");
    return DB_FAIL;
  }
  else
  {
    OS_PRINTF("Data not found\n");
    return DB_NOT_FOUND;
  }
}
                                ...
                                u8 *p_vv_id)
{
  ...
  *p_vv_id = cur_vv_id;
  ...
  memset(p_vv_info[cur_vv_id].p_mem, DB_BLK_DAFAULT_STAT,
         sizeof(item_type_t) * max_element_number);

  //ticks = mtos_ticks_get();
  if(p_view_info[table_id].p_mem_backup != NULL && 
    *(p_view_info[table_id].p_sync_st) == TRUE)
  {
    for(i = 0; i < p_view_info[table_id].packet_total; i++)
    {
      ...
      {
        memcpy(packet.item,p_view_info[table_id].p_mem_backup + 
          sizeof(item_type_t) * i * VIEW_BUNDLE_SIZE,
  ...
  else
  {
    ...
    for(i = 0; i < p_view_info[table_id].packet_total; i++)
    {
      ...
      for(j = 0; j < VIEW_BUNDLE_SIZE; j++)
      {
        ...
        cur_item = packet.item[j];
        if(p_view_info[table_id].p_mem_backup != NULL)
          *((item_type_t *)p_view_info[table_id].p_mem_backup + 
          i * VIEW_BUNDLE_SIZE + j) = packet.item[j];
        //Fetch information in detail
        if(IS_ACTIVE(cur_item) != 0)
        {
          if(is_filter_item(p_data, cur_vv_id, cur_item) == TRUE)
  ...
  //OS_PRINTF("## create_virtual_view delta time = %d *10ms\n",mtos_ticks_get() - ticks);

  //db_dbg(("DATA_BASE :create table_id %d, num %d\n", table_id, find_cnt));
...
void db_destroy_virtual_view_16v2(void *p_data, u8 vv_id)
{
  ...
  if(vv_id > 0) //vv_id == 0 means default view can't destroy
  {
...
u16 db_get_real_view_count_16v2(void *p_data, u8 vv_id)
{
  ...
  return p_vv_infor[vv_id].vv_total + p_vv_infor[vv_id].pass_num;
}
                                   ...
                                   view_packet_t *p_packet_r)
{
  ...
  //Index of node to be cleared
  u32 node_inx_r = r_node_inx;
  u8 w_pos = r_pos;
  ...
  if(node_pos_r != 0)
  {
    ...
    memset(&(packet_w.item[w_pos]), DB_BLK_DAFAULT_STAT,
           sizeof(item_type_t) * (VIEW_BUNDLE_SIZE - w_pos));
  }
    ...
    for(; node_pos_r < VIEW_BUNDLE_SIZE; node_pos_r++)
    {
      ...
      if(is_filter_item(p_data, vv_id, p_packet_r->item[node_pos_r]) == FALSE)
      {
        if(w_pos == 0) //is a new write packet
        {
          ...
          memset(packet_w.item, DB_BLK_DAFAULT_STAT, sizeof(packet_w.item));
        }
                               ...
                               u16 add_num)
{
  ...
  for(; add_node_idx < VIEW_NI_NUM; add_node_idx++)
  {
  ...
  item_type_t mod_item,BOOL is_add)
{
  ...
  view_info_t *p_view_info = p_db_var->view_info;
  u32 i = 0;
  item_type_t *p_read = (item_type_t *)p_view_info[up_table_id].p_mem_backup;
  item_type_t cur_item = DB_BLK_DAFAULT_STAT;
  u16 ret = 0xffff;
  ...
  if(!is_add)
  {
    for(i = 0; i < p_view_info[up_table_id].item_count ; i++)
    {
      {
        cur_item = p_read[i];
        if(cur_item & 1 << DB_SEL_POS_IN_VIEW)
          cur_item &= ~(1 << DB_SEL_POS_IN_VIEW);
        if(cur_item & 1 << DB_DEL_POS_IN_VIEW)
          cur_item &= ~(1 << DB_DEL_POS_IN_VIEW);

        if(cur_item == mod_item)
        {
...
void db_dump_total_item_16v2(void *p_data,u8 up_table_id,u16 min_index)
{
  ...
  u32 pakcek_min = min_index % VIEW_BUNDLE_SIZE == 0 ?
    (min_index / VIEW_BUNDLE_SIZE):
    (min_index / VIEW_BUNDLE_SIZE + 1);

  u32 item_left = p_view_info[up_table_id].item_count;
  u16 add_node_idx = 0;
  u32 i = 0;
  u16 pkt_total = 0;
  ...
  else
  {
    for(i = 0; i < pakcek_total; i++)
    {
      ...
      if(i != pakcek_total - 1)
      {
        memcpy(packet.item,p_view_info[up_table_id].p_mem_backup + 
          sizeof(item_type_t) * i * VIEW_BUNDLE_SIZE,
          sizeof(item_type_t) * VIEW_BUNDLE_SIZE);
      }
      else if(i == 0)
...
void db_sync_new_item_to_dmup_buf(void *p_data, u8 vv_id)
{
  db_var_t *p_db_var = (db_var_t *)p_data;
  db_vv_info_t *p_vv_info = p_db_var->vv_info;
  item_type_t *p_read = p_vv_info[vv_id].p_mem;
...
BOOL db_update_view_16v2(void *p_data, u8 vv_id)
{
  ...
  min_index = p_vv_info[vv_id].create_num;  //from the first add item

  vv_item_total = cleanup_virtual_view(p_data, vv_id);
  MT_ASSERT(vv_item_total == p_vv_info[vv_id].active_num);

  //db_dbg(("DATA_BASE :up table_id %d, vv_item_total %d, create %d\n",
  //  g_vv_info[vv_id].table_id, vv_item_total, g_vv_info[vv_id].create_num));
  ...
  if(p_view_info[up_table_id].p_mem_backup != NULL && 
    *(p_view_info[up_table_id].p_sync_st) == TRUE)
  {
    ...
    if(vv_item_total < p_vv_info[vv_id].vv_total)
    {
      ...
      for(i = 0; i < p_vv_info[vv_id].vv_total ; i++)
      {
        ...
        {
          mod_item = p_read[i];
          if(mod_item & 1 << DB_SEL_POS_IN_VIEW)
            mod_item &= ~(1 << DB_SEL_POS_IN_VIEW);
          if(mod_item & 1 << DB_DEL_POS_IN_VIEW)
            mod_item &= ~(1 << DB_DEL_POS_IN_VIEW);
          ret_index = db_add_remove_item_16v2(p_data,up_table_id,mod_item,FALSE);
    ...
    {
      if(vv_item_total > 0 &&
        ((add_flag == TRUE && del_flag == TRUE) ||
         (add_flag == FALSE && del_flag == FALSE) ||
         (up_table_id == 0)))
      {
        ...
        for(i = 0; i < p_vv_info[vv_id].vv_total ; i++)
        {
          ...
          if(IS_ACTIVE(p_read[i]) != 0)
          {
            ...
            if(cur_item & 1 << DB_DEL_POS_IN_VIEW)
              cur_item &= ~(1 << DB_DEL_POS_IN_VIEW);
            *(p_item_buf + i) = db_find_item_index_16v2(p_data,up_table_id,cur_item);
          }
        }
  ...
  else
  {
    ...
    for(; pkt_inx < p_view_info[up_table_id].packet_total; pkt_inx++)
    {
      ...
      for(pos = 0; pos < VIEW_BUNDLE_SIZE; pos++)
      {
        ...
        {
          for(flash_chk_cnt = 0 ; 
                flash_chk_cnt + inval_item_cnt < vv_item_total; flash_chk_cnt ++)
          {
                               ...
                               u16 *p_pos)
{
  ...
  *p_pos = 0xFFFF;
  ...
  //need check add_pos is overflow
  //Write data cache

  ret = db_add_element_16v2(p_data, table_id, &element_id, p_buffer, len);
  //ret = dm_write_node(table_id, element_id, pbuffer, len);
  if(ret == DB_SUC)
  {
...
void db_clean_16v2(void *p_data)
{
  ...
  for(table_id = 0; table_id < MAX_TABLE_NUM; table_id++)
  {
    ...
    if(p_view_info[table_id].p_sync_st != NULL)
      *p_view_info[table_id].p_sync_st = FALSE;
    p_view_info[table_id].item_count = 0;
  }
...
void db_init_database_16v2(void)
{
  ...
  for(cnt = 0; cnt < MAX_TABLE_NUM; cnt++)
  {
    p_table_info[cnt].block_id = INVALID_ID;
