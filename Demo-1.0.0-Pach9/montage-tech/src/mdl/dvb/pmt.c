...
s32 buf_len, u16 es_pid)
{
    ...
    {
        if(SYS_GET_LOW_HALF_BYTE(p_data[3]) <= 3)
        {
...
static inline u16  dump_cas_descriptor(u8 *p_data, pmt_t *p_pmt, u16 es_pid)
{
  ...
  p_pmt->ecm_cnt++;
  //MT_ASSERT(p_pmt->ecm_cnt <= MAX_ECM_DESC_NUM);

  return desc_len;
}

static inline u16  dump_program_info_desc(u8 *p_data, u8 info_len, pmt_t *p_pmt, BOOL dvbc)
{
  u8 cnt = 0;
...
static BOOL dump_pmt_desc_1(parse_desc_param_t *p_parse_info, BOOL dvbc)
{
  s16     es_info_len = p_parse_info->es_info_len;
  s16     desc_len    = p_parse_info->desc_len;
  s16     proc_len    = p_parse_info->proc_len;
  ...
  //  for(;prog_info_len > 0; prog_info_len -= desc_len, p_data += desc_len)
    ...
    while(proc_len < sec_len)
    {
        ...
        switch(p_data[0])
        {
           ...
           case DVB_STREAM_DATA_BROADCAST_SEC:
            {
              // Data broadcast sec
              pmt.es_id = element_pid;
              break;
...
void parse_pmt_1(handle_t handle, dvb_section_t *p_sec)
{
    ...
    while(proc_len < sec_len)
    {
        ...
        //parse descriptor
        is_video = dump_pmt_desc_1(&parse_info, FALSE);
