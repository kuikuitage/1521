...
static void parse_dsmcc_gi(service_t *p_svc, u8 *p_gi, u16 len)
{
  ...
  //for(i = 0; i < group_num; i++)
  {
    ...
    for(j = 0; j < descript_cnt; j++)
    {
      ...
      {
        MT_ASSERT(p_gi[offset + 1] == 9);
        MT_ASSERT(p_gi[offset + 2] == 1);
...
void parse_dsmcc_ddm(handle_t handle, dvb_section_t *p_sec)
{
  ...
  if(p_dsmcc == NULL)
  {
    ...
    alloc_t.size = sizeof(dsmcc_ddb_t);
    alloc_t.user_id = SYS_MODULE_DVBS;
    p_dsmcc = MEM_ALLOC(&alloc_t);
#endif
   
    p_dsmcc = mtos_malloc( sizeof(dsmcc_ddb_t));
    MT_ASSERT(p_dsmcc != NULL);
  }
  ...
  for(i = 0; i < dsi_info.group_num; i++)
  {
    ...
    for(j = 0; j < descript_cnt; j++)
    {
       ...
       descript_type = p_tem[len];
        if(SYS_HARDWARE_DESC == descript_type)
        {
            if(p_tem[len + 1] != 0x9  || p_tem[len + 2] != 1)
            {
              MDL_DBG_DSMCC(("p_tem[len + 1] != 0x9  || p_tem[len + 2] != 1, return!\n"));
              return;
            }
