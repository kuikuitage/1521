...
u16 event_id, u8 *p_buf)
{
  ...
  {
    p_sht_desc->name_len = MAX_EVT_NAME_LEN;
  }
  else
  {
    p_sht_desc->name_len = event_name_len;
  }  

  memcpy((u8 *)p_sht_desc->evt_nm, p_buf, p_sht_desc->name_len);
...
u16 event_id, u8 *p_buf)
{
  ...
  //Add \0 to string end
  if(p_sht_desc->name_len  > MAX_EVT_NAME_LEN)
  {
    p_sht_desc->name_len = MAX_EVT_NAME_LEN;
  }
...
ext_evt_txt_t *p_ext_txt, u16 event_id, u8 *p_buf)
{
  ...
  p_ext_desc->index = SYS_GET_HIGH_HALF_BYTE(p_buf[0]);
 ...
 //Parse item content loop
  while(item_desc_len > 0)
  {
    //Item name length
    item_name_len = p_buf[0];
    item_desc_len -- ;
    p_buf ++;
  
    //Item name information
    //MT_ASSERT(item_num <= MAX_ITEM_NUM);
    
    if(item_num < MAX_ITEM_NUM)
    {
      if(item_name_len < MAX_ITEM_DESC_LEN)
      {
        memcpy(p_ext_desc->item[item_num].item_name, p_buf, item_name_len);
      }
      else
      {
    ...
    MT_ASSERT(item_desc_len >= item_name_len);
    item_desc_len -= item_name_len;
  
    //Item cont len
    item_cont_len = p_buf[0];
    item_desc_len --;
    p_buf ++;
    ...
    if(item_num < MAX_ITEM_NUM)
    {
      if(item_cont_len < MAX_ITEM_CONT_LEN)
      {
        memcpy(p_ext_desc->item[item_num].item_cont, p_buf, item_cont_len);
      }
      else
      {
    ...
    p_buf += item_cont_len;
    MT_ASSERT(item_desc_len >= item_cont_len);
    item_desc_len -= item_cont_len;
    
  }
  
  MT_ASSERT(item_desc_len == 0);
        ...
        case DVB_DESC_CONTENT:
          {
            ...
            //MT_ASSERT(tot_cont_num < MAX_EVT_PER_SEC);
        ...
        case DVB_DESC_SHORT_EVENT:
          {
            ...
            //MT_ASSERT(tot_sht_txt_num < MAX_SHT_TXT_NUM);
            ...
            //Set language code       
            memcpy(p_eit_desc->lang_code, &p_inbuf[cnt + 1], 
        ...
        case DVB_DESC_EXT_EVENT:
          {
            ...
            //MT_ASSERT(tot_ext_txt_num < MAX_EXT_TXT_NUM);
            ...
            //MT_ASSERT(tot_ext_info_num < MAX_EXT_DESC_NUM); 
...
{  
  ...
  MT_ASSERT(eit.tot_sht_txt_num <= MAX_SHT_TXT_NUM);
