...
static sev_node_t *get_svc_node(void *p_data, u16 svc_id, u16 ts_id, u16 orig_nw_id)
{
  ...
  if (orig_nw_id == IGNORE_ID && ts_id == IGNORE_ID)
  {
  ...
  else if (orig_nw_id == IGNORE_ID && ts_id != IGNORE_ID)
  {
    p_net_node = p_epg_var->p_net_node_head;
    while (p_net_node != NULL)
    {
  ...
  sev_node_t *p_svc_node, ts_node_t *p_ts_node)
{
  ...
  MT_ASSERT(tab_cnt < TABLE_CNT(p_epg_var));
...
ext_evt_desc_t *p_ext_desc)
{
  ...
  for (item_cnt = 0; item_cnt < p_ext_desc->item_num; item_cnt ++)
  {
    ...
    dvb_to_unicode(p_ext_desc->item[item_cnt].item_cont, MAX_ITEM_CONT_LEN, 
      p_uni_ext_desc->item[item_cnt].item_cont, MAX_ITEM_CONT_LEN + 1);

    dvb_to_unicode(p_ext_desc->item[item_cnt].item_name, MAX_ITEM_CONT_LEN, 
...
static evt_node_t *create_evt_node(void *p_data, eit_t *p_eit_info, u8 evt_idx, u32 *p_ret)
{
  ...
  *p_ret = 0;
  ...
  for (sht_idx = 0; sht_idx < p_eit_info->tot_sht_txt_num; sht_idx ++)
  {
    ...
    if (p_eit_info->sht_txt[sht_idx].evt_id == p_eit_desc->evt_id
      && p_eit_info->sht_txt[sht_idx].txt_len != 0)
    {
      p_new_e_node->sht_txt_len = p_eit_info->sht_txt[sht_idx].txt_len + 1;
      ext_total_len += p_new_e_node->sht_txt_len * sizeof(u16);
      break;
    }
  }
  ...
  {
    memset(p_epg_var->p_ext_txt_buf, 0, MAX_EXT_TXT_BUF_LEN * sizeof(u8));
    ext_txt_len = 0;

    //Check extend text information
    for (cnt = 0; cnt < p_eit_info->tot_ext_txt_num; cnt ++)
    {
      if (cnt > MAX_EXT_TXT_NUM)
      {
        break;
      }
      
      if (p_eit_info->ext_txt[cnt].evt_id == p_eit_desc->evt_id
          && p_eit_info->ext_txt[cnt].txt_len != 0)
      {  
        strcat((char *)p_epg_var->p_ext_txt_buf, (char *)p_eit_info->ext_txt[cnt].ext_txt);
        
        ext_txt_len +=  p_eit_info->ext_txt[cnt].txt_len;
        MT_ASSERT(ext_txt_len <= MAX_EXT_TXT_BUF_LEN);
      }
    }

    if (ext_txt_len > 0)
    {
      ext_total_len += (ext_txt_len + 1) * sizeof(u16);
    }
    ...
    if (ext_desc_num > 0)
    {
      ext_total_len += ext_desc_num * sizeof(ext_evt_desc_uni_t);
    }
  }
  else
  {
  ...
  if (ext_total_len > 0)
  {
    //changed by anchen, why need the 4 bytes? see the reason lin928
    p_total_buffer = epg_malloc_nonfix(&p_epg_var->memp, ext_total_len +4);
    if (p_total_buffer == NULL)
    {
      epg_free_fix(&p_epg_var->evt_node_memf, p_new_e_node);
      p_new_e_node = NULL;
     // EPG_PRINT("NO non-fixed memory\n");
      return NULL;
    }

    p_new_e_node->shrt_ext_len = ext_total_len + 4;

    memset(p_total_buffer, 0, p_new_e_node->shrt_ext_len);
    ...
    else
    {
      ...
      buffer_addr += p_new_e_node->sht_txt_len * sizeof(u16);
      dvb_to_unicode(p_eit_info->sht_txt[sht_idx].txt,
        p_eit_info->sht_txt[sht_idx].txt_len,
        p_new_e_node->p_sht_txt, 
        p_new_e_node->sht_txt_len);
    }
    ...
    else
    {
      ...
      buffer_addr += (ext_txt_len + 1) * sizeof(u16);
      //Set extend txt information
      dvb_to_unicode(p_epg_var->p_ext_txt_buf,
        ext_txt_len, 
        p_new_e_node->p_ext_txt, 
        ext_txt_len + 1);
    }
                        ...
                        u16 max_net, u16 max_ts, u16 max_svc, u16 max_evt)
{
  ...
  if (p_epg_var->evt_info_priority == EPG_EVT_INFO_ALL)
  {
    if (p_epg_var->p_ext_txt_buf == NULL)
    {
      p_epg_var->p_ext_txt_buf = mtos_malloc(sizeof(u8) * MAX_EXT_TXT_BUF_LEN);
    }
    MT_ASSERT(p_epg_var->p_ext_txt_buf != NULL);
  }
  else
  {
    p_epg_var->p_ext_txt_buf = NULL;
  }
...
void epg_data_mem_release(void *p_data)
{
  ...
  if (p_epg_var->p_ext_txt_buf != NULL)
  {
    mtos_free(p_epg_var->p_ext_txt_buf);
    p_epg_var->p_ext_txt_buf = NULL;
  }
...
void epg_data_init_dy_policy(void *p_data, u16 dy_policy_cnt)
{
  ...
  if (dy_policy_cnt == 0)
  {
    ...
    MT_ASSERT(p_epg_var->p_dy_policy != NULL);
    ...
    MT_ASSERT(p_epg_var->p_dy_policy_intra != NULL);
 }
